# #12-s01 「能動的追加探索」「探索の十分性は Agent が判断」「4 path は起点 (上限ではない)」の明文化 (doc lint)
...
```text
$ grep -cE "4 path は探索の起点であり上限ではない|探索の十分性は Agent が判断|自らの気づきに基づく追加探索" \
...
> **4 path は探索の起点であり上限ではない**。Agent は 4 path 通過後でも、課題への根拠が不十分と判断した場合、自らの気づきに基づく追加探索 (別 search key の生成、別 path への切り替え、上位章 hop、関連 Conflict Review Item 再確認) を能動的に行う。探索の十分性は Agent が判断し、制約に必要な根拠が揃うまで継続する。

### `doc/EXTERNAL_DESIGN.ja.md` §8.3

> **4 path は探索の起点であり上限ではない**。Agent は 4 path 通過後でも、課題への根拠が不十分と判断した場合、自らの気づきに基づく追加探索 (...) を能動的に行う。探索の十分性は Agent が判断し、制約に必要な根拠が揃うまで継続する。CLI は path 数や hop 数の上限を強制しない。
...
Agent (LLM) は instruction-follower の性質から「4 path 通過 → 即終了」と機械的に解釈する余地がある。これを排除するため、能動的追加探索を **明示的に奨励** する文言を 3 ファイルに同時に入れる。ドリフト防止 (CLI 道具縛り + evidence_origin 縛り) は維持する。
