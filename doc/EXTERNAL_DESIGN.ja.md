...
- [x] Source Retrieval Index は section-level の Qdrant collection そのものである (chunk 分割した別 collection を持たない)
  - 実装: spec_grag/core.py:51 `CHUNK_LEVEL_ENABLED: bool = False` で chunk-level retrieval を case C-1 default で disable。`_chunk_level_enabled(config)` が `[vector_store].chunk_level_enabled` 上書きを許可。disable 時は spec_grag/core.py `_chunk_level_disabled_artifact_source_chunks` / `_chunk_level_disabled_artifact_retrieval_index_revision` が stub artifact (status="disabled") を書き込み、`build_source_chunks` / `upsert_qdrant_bge_m3_index_incremental` などの chunk 関連 helper は呼ばれない
  - 検証: tests/test_chunk_level_disabled.py の 6 件 (production default = False、config override、stub shape、chunk helper 非呼出 guard)。adjacent suite 88 passed (test_spec_core + test_chunk_level_disabled + test_section_manifest_audit + test_freshness + test_context_artifacts + test_release_readiness)
  - 注意: Qdrant `spec_grag_source` collection の物理削除は別 migration step (運用者判断で `curl -X DELETE http://localhost:6333/collections/spec_grag_source`)。chunk-level dormant code は撤去せずコメントアウト相当で残置 (ユーザー指示準拠)
