...
対象プロジェクトのルートに `.spec-grag/config.toml` を置く。設定は 1 key につき 1 行で示す。`<id>` は `[llm.providers.<id>]` で命名するユーザー定義 provider id (例: `codex`、`claude_typing`、`claude_judge`)。
...
| `[llm.providers.<id>]` | `command` | 必須 | — | 実行する CLI コマンド名または絶対パス (例: `codex`、`claude`)。`SPEC_GRAG_FAKE_PROVIDER` が truthy のときは無視される |
...
| `[llm.providers.<id>]` | `timeout_sec` | 任意 | `120` | 1 attempt あたりのタイムアウト秒数 |
| `[llm.providers.<id>]` | `max_retries` | 任意 | `1` | 初回失敗後の追加 retry 回数。`max_retries = 1` の場合、最大 attempt 数は初回 1 回 + retry 1 回の計 2 回 |
...
| `[limits]` | `llm_batch_concurrency` | 任意 | `1` | section_metadata / related_sections の batch 並列実行数 (1 = 逐次。Codex Pro 5x / Claude Max 5x 環境は 4-8 推奨) |
...
`[llm.providers.<id>]` は少なくとも 1 つ必須。`[llm.stage_routing]` 全 stage 未指定の場合、すべての stage が `[llm.providers]` の先頭定義を使う。`--llm-provider <id>` を CLI で明示すると、その provider id が全 stage を上書きする。silent fallback (provider 実行失敗後に別 provider に黙って切り替える) は行わない。
...
| `chapter_key_anchor` | 章単位 summary / key_topics / important_sections の合成 | 低〜中。haiku / mini 級で十分なことが多い |

stage_routing は明示指定方式であり、未指定の stage は `[llm.providers.<id>]` の先頭定義へフォールバックする。標準 template は `doc/CALIBRATION_MODEL_EFFORT.ja.md` の H-4 実測結果に基づき、4 つの stage すべてに provider を割り当てた状態で配布する。ユーザーは個別 entry を上書き、または削除して先頭 provider へ戻す形で運用する。
...
[llm.stage_routing]
# section_metadata / related_sections / chapter_key_anchor は先頭定義 (codex) のまま
conflict_review = "claude_judge"
```
...
`.spec-grag/config.toml` の設定値以外に、次の環境変数で挙動を上書きする経路がある。通常運用では設定ファイルを正本とし、環境変数は test / smoke / CI / setup 限定の補助に留める。
...
| 環境変数 | 用途 |
|---|---|
| `SPEC_GRAG_FAKE_PROVIDER` | truthy (`1` / `true` / `yes` / `on`) のとき、`[llm.providers.<id>]` の内容を無視して in-process FakeLlmProvider を使う。test / smoke で実 CLI を呼ばないために使う |
| `SPEC_GRAG_LLM_PROVIDER` | `spec-grag core` 実行時に使用する `[llm.providers.<id>]` の id を指定する。CLI `--llm-provider` 引数と同義で、両方指定した場合は CLI 引数が優先される |
| `SPEC_GRAG_QDRANT_URL` | Qdrant の接続先 URL を `[vector_store].url` の代わりに使用する。setup script や smoke / probe での接続先差し替え用 |
| `SPEC_GRAG_REAL_PROVIDER` | 実 LLM CLI (`codex` / `claude`) を呼ぶ test を有効にする gate。本運用では不要 |
| `SPEC_GRAG_REAL_SMOKE` | 実 LLM CLI + 実 retrieval (Qdrant + FlagEmbedding) を併せて使う smoke test を有効にする gate |
| `SPEC_GRAG_REAL_RETRIEVAL` | 実 Qdrant + 実 FlagEmbedding を呼ぶ retrieval test を有効にする gate |
| `CODEX_HOME` | Codex CLI が読む user 設定の root を上書きする (既定 `~/.codex`)。`spec-grag-setup-project --codex-install user` の配置先解決に使う |
...
`.env` は user 個別の上書き設定 (provider 切替、Qdrant URL 差し替え、real-test gate 等) を入れるため `.gitignore` 対象とする。共有用の雛形は `.env.example` を repo に commit する。
