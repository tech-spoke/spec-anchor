...
- [ ] Section ペア候補作成は `[conflict_candidate_detection]` の `small_section_all_pairs_threshold`、`section_pair_top_k`、`global_pair_cap`、`min_dense_score`、`allow_same_source_file_pair`、`allow_same_section_pair` で処理量と対象範囲を制御する。候補は保存されず、CoreResult の diagnostics に件数と mode が残る。
...
| [ ] | `[conflict_candidate_detection]` | `small_section_all_pairs_threshold` | 任意 | `12` | Section 数がこの値以下なら全 Section ペアを候補にする |
| [ ] | `[conflict_candidate_detection]` | `section_pair_top_k` | 任意 | `8` | retrieval_cap mode で 1 Section あたり候補に残す近傍 Section 数 |
| [ ] | `[conflict_candidate_detection]` | `global_pair_cap` | 任意 | `80` | retrieval_cap mode で通常候補として LLM judge に送る Section ペア数の上限 |
| [ ] | `[conflict_candidate_detection]` | `min_dense_score` | 任意 | `0.55` | retrieval_cap mode で候補として採用する最低 score |
...
enabled = true
small_section_all_pairs_threshold = 12
