...
- [ ] Chapter Key Anchor は LLM が章単位で生成する (input: 章 heading + 配下 section summary / search_keys / identifiers / related_sections + 関連 Core Concept; output: chapter_id / summary / key_topics / important_sections / notes / source_section_ids / generated_at)
  - 現状: spec_grag/core.py:2216 `_chapter_anchors` は LLM を経由しない機械集約 (summary を一部 section の summary 連結、key_topics を search_keys の先頭から拾うのみ)。Phase R-7 で LLM 生成 stage に置換予定 (doc/STORAGE_REDESIGN.ja.md §7.4 R-7)。
  - 残: 章単位 LLM call (heading + 配下 summary / search_keys / identifiers / related_sections を input、key_topics / important_sections / notes を output)、prompt_version 管理、cache 化 (key = chapter_id + 配下 section_hash 集合 + prompt_version)
...
- [ ] Section Summary / Section Search Keys / Section Identifiers / Related Sections / heading_path / source_hash / semantic_hash は、Qdrant `[vector_store].section_collection` の payload に格納する (1 section = 1 vector)
  - 現状: Qdrant `spec_grag_section` collection は section vector を保持し、payload に `summary / search_keys / identifiers / heading_path / source_hash / semantic_hash` を入れる経路は実装済み (spec_grag/retrieval_index.py:1043-1073 `build_section_payloads`)。ただし、Section Summary / Search Keys / Identifiers / Related Sections の **正本** は依然 `.spec-grag/context/section_metadata.json` 側にあり、Qdrant payload はそのコピー扱い (二重保管 / drift リスク)。
  - 残: Phase R-2 (読み取り経路を Qdrant payload に統一)、Phase R-3 (書き込み経路を Qdrant 直結、`related_sections` を `set_payload` で追加)、Phase R-5 (`section_metadata.json` 廃止)
- [ ] Source Retrieval Index は section-level の Qdrant collection そのものである (chunk 分割した別 collection を持たない)
  - 現状: chunk-level `spec_grag_source` collection はまだ standard 経路から呼ばれる (spec_grag/core.py の `_qdrant_upsert_with_partial_dispatch` 経由)。
  - 残: Phase R-5 で `spec_grag_source` を `delete_collection` し、`_qdrant_upsert_with_partial_dispatch` / `compute_chunk_diff` / `upsert_qdrant_bge_m3_index_incremental` / `source_chunks.json` を撤去
...
| flag | section_metadata cache | pair typing cache | embedding (Qdrant section collection) |
|---|---|---|---|
| (none) | reuse (hash 一致時) | reuse (hash 一致時) | reuse (hash 一致時) |
| `--all` | clear → 再生成 | clear → 再評価 | reuse (hash 一致時) |
| `--rebuild` | clear → 再生成 | clear → 再評価 | full recreate (Qdrant collection 再作成) |

非対称の根拠は決定論性である。BGE-M3 embedding は同 section embedding text + 同 model で完全同一 vector を返すため、`--all` で recreate しても情報利得はなく時間と計算資源だけ消費する。一方 LLM の section metadata 生成と pair relation typing は非決定論的であり、`--all` の「保存判定を信用しない」意図に従って毎回 LLM を再呼出する。Qdrant の vector が破損している、collection schema を移行した、その他の経路で index が壊れた疑いがある場合は `--rebuild` を使い、section-level collection を完全に作り直す。
...
- [ ] `spec-grag inject "<task>"` の gate probe は freshness report と pending conflict を返す
  - 現状: `spec-grag inject "<task>"` 経路は存在するが、Phase R-6 で §8.4 表通りの output 構造 (freshness report + pending conflict + `needs_agent_constraints` フラグ) に整形する。
- [ ] `spec-grag inject-search "<query>"` (top-K section payload を返す hybrid retrieval API) を提供する
  - 残: Phase R-6
- [ ] `spec-grag inject-section "<id>" [<id>...]` (id 指定 payload 一括 lookup) を提供する
  - 残: Phase R-6
- [ ] `spec-grag inject-chapters` (chapter_anchors.json 全体) を提供する
  - 残: Phase R-6
- [ ] `spec-grag inject-purpose` (purpose_file + concept_file の全文) を提供する
  - 残: Phase R-6
- [ ] `spec-grag inject-conflicts` (resolved + stale でない items) を提供する
  - 残: Phase R-6
- [ ] `spec-grag inject "<task>" --constraints '<JSON>'` で Agent が組んだ制約を検証する
  - 残: Phase R-6
