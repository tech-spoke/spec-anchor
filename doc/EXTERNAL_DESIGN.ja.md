...
| `<project_root>/docs/core/purpose.md` (or `[core].purpose_file` で指定された file) 不在で `spec-anchor core` を実行 | CLI exit code 1。stdout JSON (§11.1.1): `status="failed"`、`diagnostics.config_error={"reason_code":"config_error", "message":"required core file not found: {purpose_path}", "exception_type":"FileNotFoundError"}`、`warnings=[<same message>]`、`freshness_report.status="failed"`、`freshness_report.blocking_reasons=["failed_required_artifact"]`、`freshness_report.diagnostics.failed_required_artifacts=["source_specs"]`。例: `message="required core file not found: docs/core/purpose.md"` |
| `<project_root>/docs/core/concept.md` (or `[core].concept_file` で指定された file) 不在で `spec-anchor core` を実行 | CLI exit code 1。stdout JSON: 上記 Purpose 不在と同じ shape、`message="required core file not found: {concept_path}"` (例: `"required core file not found: docs/core/concept.md"`) |
| `[sources].include` の glob にマッチする Source Specs が 0 件で `spec-anchor core` を実行 | CLI exit code 1。stdout JSON: `status="failed"`、`diagnostics.config_error={"reason_code":"config_error", "message":"sources.include matched no files: {glob}", "exception_type":"ConfigError"}`、`updated_sources=[]`、`freshness_report.diagnostics.failed_required_artifacts=["source_specs"]`。`[sources].include` 自体が空 list / 非 list の場合は `message="sources.include must be a non-empty list"` または `"sources.include must contain non-empty strings"` |
...
| `freshness.json` が `status="blocked"` で `blocking_reasons` に `"dirty_or_stale_source"` / `"stale_config_or_schema"` / `"watcher_running"` / `"watcher_queue_pending"` のいずれかを含む状態で `spec-anchor inject-search` / `inject-section` / `inject-chapters` / `inject-purpose` / `inject-conflicts` のいずれかを実行 | CLI exit code 0。stdout JSON (§11.1.2 A): `command="/spec-inject <subcommand>"`、`status="blocked"`、`should_stop=true`、`stops=true`、`blocked=true`、`can_continue=false`、`blocking_reasons=<freshness.json と同じ>`、`recommended_next_action=` `"run /spec-core before inject"` (`dirty_or_stale_source`) / `"run /spec-core --all before inject"` (`stale_config_or_schema`) / `"wait for watcher completion before inject"` (`watcher_running` / `watcher_queue_pending`)、`pending_conflict_items=[]` |
| 同上の `freshness.json` 状態で `spec-anchor realign --answer-json '<json>'` を実行 | CLI exit code 0。stdout JSON: 上記と同 shape (`command="/spec-realign"`、`recommended_next_action` の `before inject` → `before realign` に置換) |
...
| `freshness.json` が `status="failed"` (= 直前 `/spec-core` で `failed_required_artifact`) の状態で `spec-anchor inject-*` / `realign` を実行 | CLI exit code 0 (`inject-*`) または 0 (`realign` は exit 1 を返さず 0、JSON で `status="failed"` を表現)。stdout JSON (§11.1.2 A): `status="failed"`、`blocking_reasons=["failed_required_artifact"]`、`recommended_next_action="run /spec-core or /spec-core --all before inject"` (`realign` は `before realign`) |
| 配置先 path が存在しない / directory でない状態で `spec-anchor-setup-project --target <path>` を実行 | CLI exit code 1。stdout JSON (§11.1.3): `status="error"`、`exit_code=1`、`target=<path>`、`applied=false`、`created=[]`、`updated=[]`、`skipped=[]`、`conflicts=[]`、`diagnostics=[{"reason_code":"target_not_found", "message":"target directory not found: {path}"}]` または `[{"reason_code":"target_is_not_directory", "message":"target path is not a directory: {path}"}]` |
...
| Qdrant service が `localhost:6333` で起動していない状態で `spec-anchor-setup-system [--check-only]` を実行 | CLI exit code 0 (warning level、`status` は `"ok"` or `"degraded"`)。stdout JSON (§11.1.4): `production_readiness.status="blocked"`、`production_readiness.blocking_reasons=["qdrant_service_unavailable"]`、`production_readiness.checks=[{"name":"qdrant_service", "status":"failed", "reason_code":"qdrant_service_unavailable"}, ...]`、`providers=[{"name":"qdrant", "kind":"vector_store_service", "available":false, "url":<probed URL>, "error":"URLError: ..." (or "OSError: ...")}, ...]` |
| `FlagEmbedding` / `qdrant_client` の Python package が import 不可で `spec-anchor-setup-system` を実行 | CLI exit code 0。stdout JSON: `production_readiness.status="blocked"`、`production_readiness.blocking_reasons=["flagembedding_package_unavailable"]` / `["qdrant_client_package_unavailable"]`、`providers=[{"name":"FlagEmbedding", "kind":"embedding_provider", "available":false, "version":null, "required":false}, ...]` |
| `codex` / `claude` CLI が `PATH` 上に無い状態で `spec-anchor-setup-system` を実行 | CLI exit code 0。stdout JSON: `production_readiness.blocking_reasons=["agent_cli_codex_unavailable"]` / `["agent_cli_claude_unavailable"]`、`agent_cli_entries.codex.cli.path=null` / `agent_cli_entries.claude.cli.path=null`、`agent_cli_entries.<agent>.cli.version=null` |
| `spec-anchor` / `spec-anchor-watch` / `spec-anchor-setup-project` / `spec-anchor-setup-system` / `spec-anchor-slash` の console script のいずれかが `PATH` 上に無い状態で `spec-anchor-setup-system` を実行 | CLI exit code 0 (warning)。stdout JSON: `production_readiness.blocking_reasons=["console_script_<name>_unavailable"]`、`console_scripts=[{"name":<name>, "available":false, "path":null}, ...]` |
...
slash command (`/spec-core` / `/spec-inject` / `/spec-realign`) と Codex skill (SPEC-anchor skill) は、Agent CLI (Claude Code / Codex) が利用者の発火を受けて内部で `spec-anchor` CLI を子プロセスとして実行し、その JSON 戻り値 (§11.1.1〜§11.1.4) を読み取って Agent が自然言語で利用者に伝達する構成である。

「入力」は利用者が Agent CLI に与える発火条件 (cwd の状態 + 環境 + 利用者 prompt) を、「出力」は Agent CLI が内部で実行する `spec-anchor` CLI コマンドと、その JSON を受けて template (`spec_anchor/templates/.claude/commands/*.md` / `.codex/skills/spec-anchor/SKILL.md`) の手順に従って Agent が利用者に提示する情報を記述する。Agent CLI (Claude Code / Codex) が自然言語生成で構成する最終文言は LLM 出力に依存するため、本契約では「Agent が伝達すべき情報内容」を template の責務範囲として固定する。

| 入力 | 出力 |
|---|---|
| `<project_root>` で利用者が `/spec-core` (引数なし) または `/spec-core --all` / `--rebuild` を発火 | Agent は `spec-anchor core [--all] [--rebuild]` を実行。CLI 出力 (§11.1.5 の `spec-anchor core` 該当行) を読み取り、`status="updated"` なら更新サマリ (updated_sections 件数 / chapter_anchors 件数 / pending_conflict_count) を伝達。`status="failed"` なら `diagnostics.<context>.message` と `warnings` の各文字列、必要なら `freshness_report.diagnostics.failed_required_artifacts` を伝達し、復旧手順 (例: `spec-anchor-setup-project --target <path>` 実行 / Purpose / Core Concept 作成 / Qdrant 起動 / `spec-anchor core --rebuild`) を利用者に提示。`status="degraded"` なら `failed_sections` 件数と `warnings` を伝達 |
| `<project_root>` で利用者が `/spec-inject "<task>"` または `/spec-inject` (会話区間から推論) を発火 | Agent は §8.3 の 4 path Agentic Search を実行する前に、最初の `spec-anchor inject-*` 呼び出し (通常 `inject-search` または `inject-conflicts`) の CLI 出力で `should_stop=true` を観測したら停止指示を利用者に伝達。`status="blocked"` で `blocking_reasons` に dirty/stale/watcher 系がある場合: `recommended_next_action` の文字列 (`"run /spec-core before inject"` 等) と blocking 理由を利用者に伝達し、`/spec-core` または watcher 完了待ちを提案。`blocking_reasons=["pending_conflict"]` の場合: `pending_conflict_items` の各 item を整形 (conflict_id / severity / claims / why_conflicting / decision_options / recommended_next_action) して利用者に提示し、判断を求める。`status="error"` の場合: `error.message` を伝達し、復旧手順を提案。stop しない場合: §8.3 の Agentic Search を進め、§8.5 の 4 区分 (今回守る制約 / 今回見るべき対象 / 関連先として確認したもの / 不確実性・人間確認) で制約セットを利用者に提示。raw JSON を貼らない |
| `<project_root>` で利用者が `/spec-realign "<task>"` を発火し、合わせて answer candidate (回答案) を提示 | Agent は §9 の手順で `spec-anchor realign --answer-json '<json>'` (`--answer-file` / `--answer-text` も可) を実行。CLI 出力で `should_stop=true` / `status="blocked"` / `status="failed"` / `status="error"` の場合は `/spec-inject` と同様の停止伝達 (blocking_reasons / pending_conflict_items / recommended_next_action / error.message)。停止しない場合: CLI JSON の `answer` field (RealignResult、§9.3 の 4 区分) を整形して利用者に提示。raw JSON は貼らない。constraint と answer が衝突する場合は `competing_conflicts` を「競合 / 不確実性 / 人間レビューが必要な点」セクションに必ず含める |
| 利用者が `spec-anchor-watch [--once]` を Agent CLI 経由でなく直接 shell から起動 (slash command / skill 経由の運用は想定しない) | 本レイヤー対象外。CLI 表 (§11.1.5) を参照 |
| 利用者が `spec-anchor-setup-project` / `spec-anchor-setup-system` を Agent CLI 経由でなく直接 shell から起動 (初期化作業のため slash command / skill レイヤーから発火する想定はしない) | 本レイヤー対象外。CLI 表 (§11.1.5) を参照 |
