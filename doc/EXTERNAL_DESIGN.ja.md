...
各 Section の `source_section_id` は、`<file_path>#<heading_slug>` 形式の識別子である。`source_section_id` は、`.spec-anchor/config.toml` の `[sources].include` に一致する Source Specs 全体で一意でなければならない。SPEC-anchor はこの一意性を、Source Retrieval Index と Related Sections が同じ Section を参照し続けるための契約として扱う。
