...
| [ ] | `freshness.json` が `status="fresh"` で課題に関係する pending な Conflict Review Item があり利用者が `/spec-inject` を発火 | Agent は §8.3 の Agentic Search で制約を集め、CLI 出力の `pending_conflict_items` の各 item を §2.8 の構造 (conflict_id / severity / claims / why_conflicting / why_llm_cannot_decide / source_refs / recommended_next_action) で利用者に提示する。制約情報 + 矛盾情報を提示してから停止し、矛盾の却下は強制しない。各 item の `recommended_next_action` は default `"Ask a human to decide this conflict."`<br><br>出力文言例:<br>## 今回守る制約<br>- ... (Agentic Search で集めた制約)<br><br>## 人間判断が必要な矛盾 (2 件)<br>1. `conflict_id=cnf_001` (severity=high): claims=「A は同期処理」vs「A は非同期処理」、why_conflicting=「§3.2 と §5.1 が矛盾」、why_llm_cannot_decide=「両方の根拠が同等で LLM が判断できない」、source_refs=`docs/spec/api.md#0003`, `docs/spec/queue.md#0001`、recommended_next_action=`Ask a human to decide this conflict.`<br>2. `conflict_id=cnf_002` (severity=medium): ...<br>矛盾の意味を会話で伝えてください。矛盾ではない場合は `spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` で却下できます。 |
...
| [ ] | `freshness.json` が `status="fresh"` で課題に関係する pending な Conflict Review Item があり利用者が `/spec-realign` を発火 | Agent は §8.3 の Agentic Search で制約を集め、CLI 出力の `pending_conflict_items` を §2.8 の構造 (conflict_id / severity / claims / why_conflicting / why_llm_cannot_decide / source_refs / recommended_next_action) で利用者に提示してから停止する。`/spec-realign` でも矛盾時は答案を生成せず (答案つきの `spec-anchor realign` を呼ばない)、提示停止になる<br><br>出力文言例:<br>## 今回守る制約<br>- ...<br><br>## 人間判断が必要な矛盾 (1 件)<br>1. `conflict_id=cnf_001` (severity=high): claims=「retry は 3 回」vs「retry は 5 回」、why_conflicting=「§4.1 と §6.2 が矛盾」、why_llm_cannot_decide=「両方が現行 Source Specs に存在」、source_refs=`docs/spec/policy.md#0004`, `docs/spec/retry.md#0001`、recommended_next_action=`Ask a human to decide this conflict.`<br>矛盾の意味を会話で伝えてから回答へ進んでください。答案は生成していません。矛盾ではない場合は `spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` で却下できます。 |
