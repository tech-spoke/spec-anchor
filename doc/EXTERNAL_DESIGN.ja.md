...
⑤ では衝突の件数だけを伝えてはいけない。各衝突について、論点の 1 行見出し、対立する主張とその出典、なぜ衝突しているか、なぜ機械では決められないか、重要度 (高 / 中 / 低)、関係する仕様、採用候補、推奨される次の操作、再参照用の衝突 ID を、人間向けの見出しで展開する。

#### 8.7.3 正常完了時の表示

正常完了でも内部の指標名や状態名をそのまま貼らない。

- [ ] `/spec-core`: 「保持物の更新が完了しました」とともに、更新があった仕様 (変更が無ければ「変更ありませんでした」)、人間判断が必要な衝突 (あれば 8.7.2 で展開)、過去の衝突解消判断のうち仕様変更で見直し余地が出たものの再確認候補 (過去の判断は 採用 / 却下 / 修正 へ翻訳)、次の操作を表示する。
