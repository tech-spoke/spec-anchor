...
```json
{
  ...
  "production_readiness": {
    ...
    "blocking_reasons": ["qdrant_service_unavailable" | "flagembedding_package_unavailable" | "qdrant_client_package_unavailable" | "agent_cli_codex_unavailable" | "agent_cli_claude_unavailable" | "console_script_<name>_unavailable" | ...],
...
各エラー行で、利用者は §11.1 の JSON 構造のどのフィールドを観測すれば状態を特定できるかを「観測フィールド」列に明示する。「主要メッセージ」列は実装が `warnings` / `recommended_next_action` / `error.message` / `diagnostics.<context>.message` に書き込む文字列を template 形式 (`{var}` は実行時に置換される値) で示す。利用者はこれらを stdout の JSON から読み取り、Agent CLI / shell 側で表示する。

| 状態 | 対応コマンド | CLI exit code | 観測フィールド | 主要メッセージ (実装出力) |
|---|---|---|---|---|
| `.spec-anchor/config.toml` が見つからない | `/spec-core` / `/spec-inject` / `/spec-realign` / `spec-anchor-watch` | `/spec-core`: 1、`/spec-realign`: 1、`/spec-inject`: 0、`spec-anchor-watch`: 0 | `/spec-core` / `/spec-realign`: `diagnostics.config_error.reason_code = "config_error"`。`/spec-inject` / `spec-anchor-watch`: §11.1.2 B の `error.code = "command_error"`、`error.type = "ConfigError"` | `diagnostics.config_error.message` / `error.message` = `".spec-anchor/config.toml not found under {root}"`。`/spec-inject` / `/spec-realign` の `recommended_next_action` は呼ぶ前に解決すべきなので発火経路は `error.message` 主体 (gate 経路でなく例外経由) |
| Purpose が見つからない | `/spec-core` | 1 | `diagnostics.config_error.reason_code = "config_error"`、`warnings` に同 message | `diagnostics.config_error.message` / `warnings[i]` = `"required core file not found: {purpose_path}"` (例: `"required core file not found: docs/core/purpose.md"`)。利用者は Purpose ファイルを作成して再実行 |
| Core Concept が見つからない | `/spec-core` | 1 | 同上 | `"required core file not found: {concept_path}"` (例: `"required core file not found: docs/core/concept.md"`) |
| Source Specs が見つからない | `/spec-core` | 1 | `diagnostics.config_error.reason_code = "config_error"`、`freshness_report.diagnostics.failed_required_artifacts = ["source_specs"]` | `diagnostics.config_error.message` / `warnings[i]` = `"sources.include matched no files: {glob}"` または `"sources.include must be a non-empty list"` |
| Section Metadata 更新に一部失敗 | `/spec-core` | 0 (`status = degraded` は failed ではない) | `status = "degraded"`、`diagnostics.section_metadata_generation.failed_sections = [{section_id, reason_code, ...}, ...]`、`freshness_report.diagnostics.degraded_optional_artifacts` に `"section_metadata"` | `warnings[i]` に section ごとの message (例: `"section_metadata generation failed for {section_id}: {reason}"`)。`recommended_next_action` = `"continue inject with warnings"` (degraded は continue 可) |
| Chapter Key Anchor 更新に一部失敗 | `/spec-core` | 1 | `status = "failed"`、`diagnostics.chapter_anchors.status = "failed"`、`diagnostics.chapter_anchors.failed_chapter_ids = [...]`、`diagnostics.chapter_anchors.failure_reasons_by_chapter` | `warnings[i]` = `"Chapter Anchors LLM generation failed for {N} chapter(s); canonical chapter_anchors.json is not updated. Run /spec-core --all to retry."`。下流の `/spec-inject` / `/spec-realign` 側の `recommended_next_action` = `"run /spec-core or /spec-core --all before inject"` |
| Related Sections の retrieval backend に到達できない (Qdrant を期待した設定で初期化失敗) | `/spec-core` | 1 | `status = "failed"`、`related_sections_status = "failed"`、`diagnostics.related_sections.qdrant_backend_failure.failure_reason`、`freshness_report.diagnostics.failed_required_artifacts` に `"related_sections"` | `warnings[i]` = `"Related Sections retrieval backend failure: {failure_reason}; canonical related_sections artifact is not updated. Restore Qdrant connectivity and run /spec-core --rebuild."`。下流の `recommended_next_action` = `"run /spec-core or /spec-core --all before inject"`。Qdrant 未設定の InMemory 構成は対象外 (warning なし) |
| embedding / retrieval index 更新に失敗 | `/spec-core` | 1 | `status = "failed"`、`retrieval_index_status = "failed"`、`freshness_report.diagnostics.failed_required_artifacts` に `"retrieval_index"` | `warnings[i]` = `"Source Retrieval Index update failed"` または verification 不整合時は `"Source Retrieval Index verification detected inconsistency; run /spec-core --rebuild"`。下流の `recommended_next_action` = `"run /spec-core or /spec-core --all before inject"` |
| dirty / stale / watcher 系 blocking reason がある | `/spec-inject` / `/spec-realign` | 両方 0 (JSON で報告) | §11.1.2 A: `status = "blocked"`、`should_stop = true`、`blocking_reasons` に `"dirty_or_stale_source"` / `"stale_config_or_schema"` / `"watcher_running"` / `"watcher_queue_pending"` のいずれか | `recommended_next_action` = `"run /spec-core before inject"` (`dirty_or_stale_source`) / `"run /spec-core --all before inject"` (`stale_config_or_schema`) / `"wait for watcher completion before inject"` (`watcher_running` / `watcher_queue_pending`)。`realign` の場合は `inject` → `realign` に置換 |
| `pending_conflict` だけが blocking reason として残っている | `/spec-inject` / `/spec-realign` | 両方 0 (JSON で報告) | §11.1.2 A: `status = "blocked"`、`blocking_reasons = ["pending_conflict"]`、`pending_conflict_items = [{conflict_id, severity, claims, decision_options, ...}, ...]` | `recommended_next_action` = `"resolve pending Conflict Review Items"`。各 Conflict Review Item の `recommended_next_action` field は LLM 生成の場合 `"Ask a human to decide this conflict."` が default |
| watcher running / queued changes が残っている | `/spec-inject` / `/spec-realign` | 両方 0 (JSON で報告) | §11.1.2 A: `blocking_reasons` に `"watcher_running"` または `"watcher_queue_pending"` | `recommended_next_action` = `"wait for watcher completion before inject"` (`realign` の場合は `realign` に置換) |
| Project Setup Script の target が存在しない | `spec-anchor-setup-project` | 1 (`status = "error"` / `"failed"`) | §11.1.3: `diagnostics` に `reason_code = "target_not_found"` または `"target_is_not_directory"`、`conflicts` は空 | `diagnostics[i].message` = `"target directory not found: {path}"` または `"target path is not a directory: {path}"` |
| Project Setup Script の配置先に既存ファイルがある | `spec-anchor-setup-project` | 1 (`status = "conflict"`、`--force` で 0 に戻る) | §11.1.3: `status = "conflict"`、`conflicts = [{path, reason, diff}]`、reason は `"would_overwrite_existing_file"` / `"destination_exists_and_is_not_file"` / `"existing_file_is_not_utf8_text"` のいずれか | reason ごとの自動 message なし (利用者は `conflicts[i].path` と `conflicts[i].diff` (= unified diff) を表示し、`--force` を判断する)。Purpose / Core Concept は `protected_core_paths` 扱いで `--force` でも上書きしない |
| System Setup Script の依存ツールが見つからない | `spec-anchor-setup-system` | warning レベル: 0、`failed` / `error` レベル: 1 | §11.1.4: `production_readiness.status = "blocked"`、`production_readiness.blocking_reasons` に `"qdrant_service_unavailable"` / `"flagembedding_package_unavailable"` / `"qdrant_client_package_unavailable"` / `"agent_cli_<name>_unavailable"` / `"console_script_<name>_unavailable"` のいずれか、`providers` / `console_scripts` の各 entry の `available: false` で詳細 | 各 check の `reason_code` を表示。Qdrant の場合は `providers[<qdrant>].error` に urlopen の例外メッセージ (`"URLError: ..."` 等)。FlagEmbedding / qdrant_client パッケージは `find_spec` 結果から「Python パッケージ <name> が import できない」を Agent CLI 側で構成する |
| System Setup Script の Agent CLI 認識性チェックで Codex skill / Claude command が認識されない | `spec-anchor-setup-system` | 0 (warning のみ、`status` は `"ok"` or `"degraded"`) | §11.1.4: `agent_cli_entries.codex.project_skill_path` / `agent_cli_entries.claude.project_command_path` に setup-project の出力先、`diagnostics` に warning entry、`providers` の Agent CLI entry に `version` 情報 | warning entry の `message` に「skill / command が認識されない」旨と、`agent_cli_entries.<agent>` の path / version を含む。手作業対応手順 (`spec-anchor-setup-project --force` 等) を Agent CLI 側で構成する |
