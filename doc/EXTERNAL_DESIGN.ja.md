...
| 状態 | 意味 |
|---|---|
| `dirty` | Source specs に semantic change があるが、GRAG / embedding / concept index / sidecar に反映されていない |
...
JSON、pending state、CLI の状態遷移コマンドは、エージェントと SPEC-grag が確認状態を保存・再開・検証するための機械インターフェースである。ユーザー向けの確認インターフェースはチャット上の自然言語確認であり、ユーザーが内部 JSON を直接編集・確認することを前提にしない。

Concept diff は同時に複数生成してはいけない。pending Concept diff は、特定の `concept.md` base hash に対する単一の承認対象である。pending Concept diff が残っている間に Source specs が追加変更された場合、SPEC-grag は新しい Concept diff を重ねて作らず、変更 section を queued change として保存する。pending が accept / reject / revise / apply などで解消された後、queued change を最新 Source specs と最新 Concept を前提に再評価し、まだ必要な場合だけ次の Concept diff を 1 件生成する。

SPEC-grag は、Concept diff 生成の効率化のために provisional concept cache を持ってよい。これは LLM が観測した未承認 Concept 候補、supporting section、semantic hash、confidence、rejected / suppressed 履歴などを保存する内部 cache である。ただし、provisional concept cache は承認済み Concept ではない。InjectionContext、Answer、Conflict 確定、production readiness の根拠として採用してはいけない。用途は差分検出、重複提案抑制、queued change 再評価、LLM extraction 再実行削減に限定する。
...
Concept diff が出力された場合、エージェントはユーザーに hunk 単位で accept / reject / 修正指示を確認する。確認なしに Concept を書き換えてはいけない。reject する場合は Concept 更新案を採用しない。修正指示を受け取った場合は、必要に応じて Concept 更新候補を再生成する。
...
Source specs 全体に対する未承認 Conflict 候補は、確定 Conflict として扱ってはいけない。関連する未承認 Conflict 候補は `ReviewNotes` に入れ、承認済み Conflict だけを関連タスクの `ConflictNotes` に反映する。
...
Conflict 候補の承認・拒否・保留は `/spec-core` 相当の確認で扱う。`Answer` 生成 phase は Conflict 候補を新たに承認せず、承認済み Conflict を `ConflictNotes` として、未承認 Conflict 候補を `ReviewNotes` として扱う。
...
| `[watcher]` | `enabled` / `debounce_ms` / `state_file` / `queue_file` | 任意 | Source specs 変更検知と background incremental の設定 |
...
| Conflict 候補が未承認 | 確定 Conflict として扱わず、関連する InjectionContext / Answer では ReviewNotes として提示する |
...
| production で dirty / pending / stale | 自動更新、自動承認、fallback を行わず fail-fast する |
