...
| 制約検証 | `spec-grag inject --constraints '<JSON>'` | validated constraints + injectable_context |
...
`spec-grag realign` は §8.4 の `inject` / `realign` 共通フラグ (`--constraints*`) に加え、回答候補の入力フラグを持つ。
