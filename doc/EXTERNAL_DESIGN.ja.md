...
| `[llm.stage_routing]` | `section_metadata` / `related_sections` / `conflict_review` / `chapter_key_anchor` | 任意 | stage 別に LLM provider を振り分ける。各 key の値は `[llm.providers.<id>]` で定義済み provider id。未指定 stage は `[llm.providers.<id>]` の先頭定義へフォールバック。`--llm-provider` を CLI で明示すると、その provider id が全 stage を上書きする |
| `[retrieval]` | `dense_top_k` / `sparse_top_k` / `rank_fusion` / `section_collection` / `section_dense_threshold` / `section_candidate_top_k` / `section_final_top_n` | 任意 | hybrid retrieval の取得幅と section-level 候補絞り込み閾値。`section_collection` は section-level retrieval 用 Qdrant collection 名 (1 section = 1 vector、payload に summary / search_keys / identifiers / related_sections / heading_path を含む) |
| `[embedding]` | `provider` / `model` / `dense_enabled` / `sparse_enabled` | 必須 | embedding provider。標準は `flagembedding` + `BAAI/bge-m3` |
| `[vector_store]` | `provider` / `url` | 必須 | vector store。標準は Qdrant |
...
Codex 用 skill と Claude 用 command は `spec-grag core` を `--llm-provider` 引数なしで実行し、`[llm.stage_routing]` に従って stage 別に provider を選ばせる。`--llm-provider` を明示すると `[llm.stage_routing]` が上書きされ、その provider id が全 stage に適用されるため、provider 障害時の手動 fallback など特別な事情がない限り指定しない。direct CLI / watcher / 手動実行も同様で、`--llm-provider` 未指定なら stage_routing が、`[llm.stage_routing]` 未指定の stage は `[llm.providers.<id>]` の先頭定義が選ばれる。silent fallback（provider 実行失敗後に別 provider に黙って切り替える）は行わない。
