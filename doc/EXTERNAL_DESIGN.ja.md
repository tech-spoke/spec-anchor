...
Related Sections は **conflict 判定を持たない**。relation_hint は `depends_on / impacts / prerequisite / same_policy / see_also` のみ。`conflicts_with` は本 stage では確定させない。仕様上の矛盾候補抽出は、Related Sections とは別に §2.11 の Section Pair Conflict Detection が扱う。
...
- [ ] LLM が矛盾兆候を見つけた場合、Related Sections の選定や schema は変わらない。仕様上の矛盾候補は §2.11 Section Pair Conflict Detection が Source Specs の Section ペアから抽出する。

### 2.8 Conflict Review Item

Conflict Review Item は、Purpose、Core Concept、または Source Specs の根拠同士が同時に満たせない疑いがあり、LLM が既存根拠だけでは解消できない場合に、人間へ判断を求める項目である。Conflict Review Item の作成対象は、§2.11 Section Pair Conflict Detection が選んだ Source Specs の Section ペアである。
...
保存される item は次のフィールドを持つ。`resolution` は却下済み item で必要な場合に付く。
...
- [ ] LLM が矛盾を最終裁定して状態を自動で変えることが発生しない (矛盾を「解決済み」としてマークする状態・操作は存在しない。人間が取れる操作は説明と却下の 2 つだけ。§7.2.2)

- [ ] status が `pending` の Conflict Review Item は freshness の停止理由ではない (`blocking_reasons` に矛盾起因の理由は積まれない)。課題に関係する pending な矛盾がある場合、`/spec-inject` / `/spec-realign` は制約情報 + 矛盾情報を提示してから停止する (詳細な提示停止挙動は §8.6 / §11.1.5 / §11.2 を参照)。
...
### 2.11 Section Pair Conflict Detection

Section Pair Conflict Detection は、Source Specs の Section 同士を比べ、同時に満たせない可能性がある組み合わせだけを Conflict Review Item にする仕組みである。

`/spec-core` は、まず LLM を使わずに Section ペア候補を作る。小さい Source Specs 集合では Section ペアを広く列挙し、大きい Source Specs 集合では section-level retrieval と上限設定で判定対象を絞る。この候補は保存されず、full run または変更がある incremental run のたびにメモリ上で作り直される。

候補作成の後、LLM を使うのは Section ペアの conflict judge だけである。judge は対象の 2 Section とその出典を読み、既存根拠だけでは解消できないと判断した場合だけ Conflict Review Item を作る。Related Sections の relation_hint は Conflict Review Item の入力にしない。

変更のない incremental run では、Section ペア候補の作成と LLM judge の両方を実行しない。この場合、前回の Conflict Review Items が保持される。

- [ ] `/spec-core` は Source Specs の Section から Section ペア候補を作る。候補作成は LLM を呼ばない。
- [ ] Section ペア候補は `.spec-anchor/context/` や `.spec-anchor/state/` に保存されない。保存されるのは Conflict Review Items だけである。
- [ ] LLM を呼ぶ conflict 判定は Section ペア conflict judge の 1 段だけである。
- [ ] 変更のない incremental run では、Section ペア候補作成と LLM judge をスキップし、前回の Conflict Review Items を保持する。
- [ ] Detection の有効 / 無効は `[conflict_candidate_detection].enabled` で制御する。既定値は `true` である。
...
- [ ] `/spec-core` は Section Metadata / Related Sections / Chapter Key Anchor / Source Retrieval Index / Conflict Review Items を生成する (各保持物の所在は §4.1)。
...
`/spec-inject` (各 `inject-*` サブコマンド: `inject-search` / `inject-section` / `inject-chapters` / `inject-purpose`) と `/spec-realign` は、保持物が最新でない場合は停止し、理由と対処方法を表示する (freshness gate)。各コマンドは内部で gate を通すので、Agent が事前に別の probe コマンドを呼ぶ必要はない。
...
| [ ] | 人間判断待ちの Conflict Review Item がある | freshness gate は停止せず、制約情報と併せて提示する | 必要なら Conflict Review Item を却下する、または Source Specs を修正する |
...
- [ ] Source Specs / Purpose / Core Concept の変更後に `/spec-core` または `spec-anchor-watch` が保持物を更新した場合、SPEC-anchor は既存の人間判断待ち Conflict Review Item を現在の Source Specs で再評価する。根拠 Section が変更されて矛盾が解消した場合、または根拠 Section が削除・改名されてペアが消えた場合、その項目は矛盾レビュー一覧から取り除かれる (以後提示されない)。これは人間の却下とは別で、ソース修正による自動解消であり、項目は残さず削除する。根拠 Section が変更されていない項目は、再評価だけでは取り除かれない。
...
Search Keys は根拠にしない。Section Summary と Chapter Key Anchor は検索・理解の補助であり、制約として採用する場合は Purpose、Core Concept、または Source Specs の根拠を確認する。
...
- [ ] Section ペア候補は `.spec-anchor/context/` に格納されない。full run または変更がある incremental run のたびにメモリ上で作り直される。
...
- [ ] Section ペア候補作成専用の状態記録ファイルは存在しない。変更のない incremental run かどうかは `/spec-core` が現在の Section hash と Purpose / Core Concept の hash を比較して判断する。

**LLM 応答 cache (`.spec-anchor/cache/`)**:

- [ ] section_metadata / related_sections / chapter_anchors の LLM 応答が `.spec-anchor/cache/` 配下にキャッシュされる。
...
- [ ] Section ペア候補をメモリ上で作る (候補自体は保持しない)。
...
- [ ] 今回の課題に必要な制約を最終生成しない (`spec-anchor inject-*` (`inject-search` / `inject-section` / `inject-chapters` / `inject-purpose`) は retrieval payload / 章 anchor path / Purpose 全文を返すのみで、constraint statement の最終生成は Agent / LLM の責務である。CLI 出力に fabricated な `constraints[]` / `statement` field が現れない)。
...
/spec-core --all
  = LLM 由来 cache (section_metadata / related_sections / chapter_anchors) をクリアして再評価
...
| 確認 | flag | LLM 由来 cache (section_metadata / related_sections / chapter_anchors) | embedding (Qdrant section collection) |
|---|---|---|---|
...
| 確認 | フラグ | 内容 |
|---|---|---|
| [ ] | `--all` / `-a` | LLM 由来 cache (section_metadata / related_sections / chapter_anchors) をクリアして再評価する。embedding は hash 一致時に再利用 |
...
| [ ] | `--dismiss-conflict <conflict_id>` | 指定した Conflict Review Item を「矛盾ではない」として却下する (`--reason` 必須) |
| [ ] | `--reason <text>` | `--dismiss-conflict` の却下理由。空文字は受け付けない |
...
[ ] /spec-core
      ...
      [ ] Section ペア候補をメモリ上で作る
      [ ] Section ペア conflict judge で、LLM が解決できない conflict を Conflict Review Item として記録する
...
CoreResult
  ...
  [ ] section_pair_candidate_generation_status
  [ ] potential_conflicts
...
- [ ] `potential_conflicts` は Section ペア conflict judge が「矛盾ではない」または「既存根拠から優先関係が明確」と判断できた候補を warning として保持する。
- [ ] `section_pair_candidate_generation_status` は Section ペア候補作成の最終状態を示す。`disabled` / `skipped_unchanged` / `success` / `blocked` / `failed` のいずれかの値を取る。
- [ ] CoreResult の `diagnostics.section_pair_candidate_generation` は、少なくとも `status`、`section_count`、`mode` (`all_pairs` または `retrieval_cap`)、`generated_count`、`truncated_count`、`recheck_count` を返す。
...
矛盾に対して人間が取れる操作は、説明 (状態を変えない) と却下 (以後提示しない) の 2 つだけである。詳細は §2.8 と §7.2.2 を参照。
...
- [ ] `/spec-core` は、ソース変更後の再評価で矛盾が解消した (または根拠 Section が消えた) pending item を矛盾レビュー一覧から取り除き、その件数 `auto_resolved_conflict_count` / id 一覧 `auto_resolved_conflict_ids[]` で返す。これは人間の却下 (項目は `dismissed` として残り、根拠ハッシュ変化で再浮上しうる) とは別で、ソース修正による自動解消であり、項目は残さず削除する。
- [ ] Section ペア候補作成は `[conflict_candidate_detection]` の `conflict_candidate_mode`、`section_pair_top_k`、`global_pair_cap`、`min_dense_score`、`allow_same_source_file_pair`、`allow_same_section_pair` で処理量と対象範囲を制御する。候補ペア総数が `global_pair_cap` 以下なら全 Section ペアを judge し、超える場合は retrieval で上位だけ judge する(単一 budget で切り替わり、Section 数による段差は無い)。候補は保存されず、CoreResult の diagnostics に件数と mode が残る。
- [ ] Conflict Review Item の `conflict_id` は Section ペア ID である。人間が `spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` を実行すると、その Section ペア単位で却下される。
...
会話区間の解釈と中心課題の特定は Agent / LLM の責務である (§5.3)。CLI は会話区間も課題プロンプトも消費しない。Agent / LLM が解釈した課題に基づいて、§8.3 の 3 path から必要な保持物を CLI に問い合わせ、制約を生成する。入力に含まれる会話区間の扱い、生テキスト投入の制限、根拠の区別は §3.4 を参照。
...
| [ ] | `conflict_review_items.json` | dismissed / pending Conflict Review Items |

### 8.3 Agent / LLM が行う作業 (3 path)

Agent / LLM は、課題の性質に応じて次の 3 path を組み合わせて使う。`evidence_origin` の enum (`Purpose` / `Core Concept` / `Source Specs`) を 3 path がそれぞれカバーする。各 path は必須ではなく許可で、Agent が選んで使い分ける。

**3 path は探索の起点であり上限ではない**。Agent は 3 path 通過後でも、課題への根拠が不十分と判断した場合、自らの気づきに基づく追加探索 (別 search key の生成、別 path への切り替え、上位章への hop) を能動的に行う。探索の十分性は Agent が判断し、制約に必要な根拠が揃うまで継続する。CLI は path 数や hop 数の上限を強制しない。ただし根拠は引き続き `evidence_origin` ∈ {Purpose / Core Concept / Source Specs} に縛られ、CLI 道具 (`spec-anchor inject-*`) を介さずにいきなり Source Specs を grep する経路は引き続き禁止する (ドリフト防止: 検索の起点は必ず CLI 経由)。Source Specs ファイル本文の `Read` は、CLI で section_id を特定した後の補助確認としてのみ許可される。
...
Agentic Search は Agent / LLM の責務である。CLI は検索結果、payload、章 anchor、Purpose / Core Concept、pending Conflict Review Items の通知を返すだけであり、探索方針を自律的に決めない。
...
- [ ] `spec-anchor inject-*` には自動探索 / 多段 traversal を実行する CLI コマンドは含まれない (CLI は単発の retrieval / payload lookup / 章 anchor 取得 / Purpose 取得のみ提供し、Section 間の再帰的 lookup は Agent 側が行う)。

### 8.4 CLI が提供する操作

CLI は外部契約として次の参照操作を提供する。

- [ ] 各 `inject-*` コマンドは内部で freshness gate / watcher gate を通す (§3.3 / §6.3)。gate が blocked / failed の場合、各コマンドは Agentic Search を実行せず、停止指示と理由 (`should_stop=true`、`blocking_reasons`、`recommended_next_action`) を返す。pending Conflict Review Items は停止理由ではなく、fresh な場合に情報として `pending_conflict_items` / `pending_conflict_count` に載る。Agent は別途事前 probe を実行する必要はない。
...
- [ ] `spec-anchor inject-search` / `inject-section` / `inject-chapters` / `inject-purpose` には CLI 固有フラグはない。`/spec-realign` の CLI フラグは §9.4 を参照。
...
constraint
  [ ] statement: 今回守る制約
  [ ] evidence_origin: Purpose | Core Concept | Source Specs
  [ ] evidence_ref: 文書 path、source_section_id、source span、または Core Concept の項目
...
- [ ] `/spec-inject` は、検索キー、Section Summary、Related Sections だけを根拠として制約を確定しない。制約として使う場合は、Purpose、Core Concept、Source Specs の該当箇所を根拠として示す。
...
### 8.6 pending Conflict Review Items の出力

- [ ] freshness report が `status = fresh` で、CLI が `pending_conflict_items[]` を返す場合、Agent は通常の制約セットを生成したうえで、人間判断が必要な conflict も併せて提示する。pending Conflict Review Items は freshness の blocker ではないが、課題に関連する場合は回答へ進む前に人間判断が必要な事項として扱う (詳細な CLI / Agent 出力契約は §11.1.5 / §11.2 を参照)。

Agent CLI が提示する出力は、次を必ず含む。各 conflict item は §2.8 の構造に従う。`recommended_next_action` の default は `Ask a human to decide this conflict.`。
...
### 8.7 人間向け表示契約 (停止時・正常完了時・リトライ)

スラッシュコマンドが利用者へ返す応答は、利用者が SPEC-anchor の内部構造を知らない前提で読める言葉で書く。CLI が返す内部の状態名やフィールド名をそのまま会話へ貼ってはいけない。表示の具体フォーマットは各コマンドテンプレ (`.claude/commands/spec-*.md` と `.codex/skills/spec-anchor/SKILL.md`) が保持し、本節はその契約を定める。

#### 8.7.1 停止時の表示カテゴリ

コマンドが処理を進められず停止したとき、状況を次の利用者視点カテゴリへ写像し、該当カテゴリの見出しと「次の操作」だけを表示する。該当しないカテゴリは表示しない。

- [ ] ① 初期設定が完了していない: 設定ファイル、Purpose / Core Concept、または対象仕様が見つからない。`spec-anchor-setup-project` の実行やファイル作成を促す。
...
#### 8.7.2 人間判断が必要な衝突の本文展開

⑤ では衝突の件数だけを伝えてはいけない。各衝突について、論点の 1 行見出し、対立する主張とその出典、なぜ衝突しているか、なぜ機械では決められないか、重要度 (高 / 中 / 低)、関係する仕様、推奨される次の操作、再参照用の衝突 ID を、人間向けの見出しで展開する。

#### 8.7.3 正常完了時の表示

正常完了でも内部の指標名や状態名をそのまま貼らない。

- [ ] `/spec-core`: 「保持物の更新が完了しました」とともに、更新があった仕様 (変更が無ければ「変更ありませんでした」)、人間判断が必要な衝突 (あれば 8.7.2 で展開)、過去の却下判断のうち仕様変更で見直し余地が出たものの再確認候補、次の操作を表示する。
- [ ] `/spec-inject`: §8.5 の 5 セクションで表示し、各制約の根拠種別・参照補助・適用範囲・確認すべき点を人間向けの見出しで示す。
- [ ] `/spec-realign`: §9.3 の 4 区分で表示し、制約は `/spec-inject` と同じ人間向け見出しで示す。

#### 8.7.4 回答候補の形式不備とリトライ

`/spec-realign` に渡した回答候補の形式が不備のとき、コマンドはどの項目が期待とどう違うかを項目単位で返す。これを受けて Agent は、指摘された項目だけを直して 1 回だけ再実行する。それでも整わない場合は ⑥ として扱い、最後に送った回答候補と「期待された形式との差分」を人間語で併記して表示する。リトライの途中経過や内部の項目名は利用者へ出さない。
...
- [ ] **`/spec-realign` 実行の trace 監査** (検証単位): Agent CLI の tool call trace に §8.3 の 3 path のいずれかの呼び出し連鎖 + `spec-anchor realign --answer-json '<json>'` が観測され、最終出力に §9.3 の 4 区分が含まれる。
...
| [ ] | `[conflict_candidate_detection]` | `enabled` | 任意 | `true` | Section Pair Conflict Detection を有効にするか |
| [ ] | `[conflict_candidate_detection]` | `conflict_candidate_mode` | 任意 | `"budget"` | `budget`=候補ペア総数が `global_pair_cap` 以下なら全 Section ペア、超なら retrieval で絞る。`exhaustive`=cap 無視で全 Section ペア(検証用) |
| [ ] | `[conflict_candidate_detection]` | `section_pair_top_k` | 任意 | `8` | retrieval で絞る際、1 Section あたり候補に残す近傍 Section 数 |
| [ ] | `[conflict_candidate_detection]` | `global_pair_cap` | 任意 | `80` | LLM judge に送る通常候補 Section ペア数の上限。この値で全ペア列挙 / retrieval 絞り込みが切り替わる(単一の budget) |
| [ ] | `[conflict_candidate_detection]` | `min_dense_score` | 任意 | `0.55` | retrieval で候補として採用する dense cosine の最低 score |
| [ ] | `[conflict_candidate_detection]` | `judge_batch_size` | 任意 | `5` | 1 LLM call でまとめて判定する Section ペア数。`<=1` で 1 ペア 1 call。parse 失敗 / 件数不一致時は該当 batch を 1 ペアずつ再判定 |
| [ ] | `[conflict_candidate_detection]` | `allow_same_source_file_pair` | 任意 | `true` | 同じ Source Specs file 内の Section ペアを候補対象にするか |
| [ ] | `[conflict_candidate_detection]` | `allow_same_section_pair` | 任意 | `true` | 同じ Section 同士のペアを候補対象にするか |
...
`[llm.providers.<id>]` と `[llm.stage_routing]` は `/spec-core` が保持物生成 (section_metadata / related_sections / conflict_review / chapter_key_anchor) で直接 spawn する LLM の設定である。Section ペア候補作成は LLM を呼ばないため provider routing 対象外である。`/spec-inject` / `/spec-realign` の会話区間解釈、Agentic Search、制約生成、回答生成を行う Agent / LLM は Agent CLI 側で動くため、これらの設定の対象外である。

- [ ] `[llm.providers.<id>]` と `[llm.stage_routing]` は `/spec-core` の保持物生成 stage (section_metadata / related_sections / conflict_review / chapter_key_anchor) でのみ参照される (`/spec-inject` / `/spec-realign` の Agent / LLM 動作には影響しない、tool call trace に `[llm.providers]` 由来の subprocess 起動が現れない)。
- [ ] Section ペア候補作成は LLM を呼ばないため `[llm.stage_routing]` の対象外であり、provider routing 解決もされない。
...
`/spec-core` の各 LLM stage は認知負荷が異なるため、stage 別に LLM provider を切り替える仕組みを持つ。許可される stage key は次の 4 つに固定する。Section ペア候補作成は LLM を呼ばないため stage_routing の対象外である。
...
- [ ] 許可外の stage key (例: 誤記の `conflict_reveiw`) は設定エラーとして reject する (許可される stage key は `section_metadata` / `related_sections` / `conflict_review` / `chapter_key_anchor` の 4 つ固定)。
...
# 各 LLM stage がどの provider を使うかを指定する。
# Section ペア候補作成は LLM を呼ばないため routing 対象外。
[llm.stage_routing]
...
enabled = true
conflict_candidate_mode = "budget"
...
- `/spec-inject` (内部実行: `spec-anchor inject-search` / `inject-section` / `inject-chapters` / `inject-purpose`、§8)
...
```json
{
  ...
  "blocked": true,
  "can_continue": false,
  "blocking_reasons": ["dirty_or_stale_source" | "watcher_running" | "watcher_queue_pending" | "stale_config_or_schema" | "failed_required_artifact"],
  "warnings": [<string or object>],
...
`pending_conflict_items` は freshness の停止理由ではなく、注入すべき情報である。保持物が最新で freshness が `fresh` であっても、課題に関係する pending な矛盾があれば `pending_conflict_items` / `pending_conflict_count` として返る。`blocking_reasons` には矛盾起因の理由は積まれない。
...
| [ ] | Section Metadata の LLM 生成が一部の section で失敗 `spec-anchor core` を実行 | CLI exit code 1。stdout JSON: `status="failed"`、`failed_sections=[{section_id, reason_code, ...}, ...]`、`updated_sections=[<成功 section>]`、`diagnostics.section_metadata_generation={..., "failed_sections":[...], "freshness_status":"failed"}`、`freshness_report.status="failed"`、`freshness_report.blocking_reasons=["failed_required_artifact"]`、`freshness_report.diagnostics.failed_required_artifacts=["section_metadata"]`、`warnings=[<section ごとの失敗説明>]`。Section Metadata の生成失敗は全失敗・部分失敗を問わず必須保持物の失敗として扱い、後続の `/spec-inject` / `/spec-realign` を停止させる |
...
| [ ] | `freshness.json` が `status="blocked"` で `blocking_reasons` に `"dirty_or_stale_source"` / `"stale_config_or_schema"` / `"watcher_running"` / `"watcher_queue_pending"` のいずれかを含む状態で `spec-anchor inject-search` / `inject-section` / `inject-chapters` / `inject-purpose` のいずれかを実行 | CLI exit code 0。stdout JSON (§11.1.2 A): `command="/spec-inject <subcommand>"`、`status="blocked"`、`should_stop=true`、`stops=true`、`blocked=true`、`can_continue=false`、`blocking_reasons=<freshness.json と同じ>`、`recommended_next_action=` `"run /spec-core before /spec-inject"` (`dirty_or_stale_source`) / `"run /spec-core --all before /spec-inject"` (`stale_config_or_schema`) / `"wait for watcher completion before /spec-inject"` (`watcher_running` / `watcher_queue_pending`)。`pending_conflict_items` は本 path では output に含まれない (null / 省略) |
...
| [ ] | `freshness.json` が `status="fresh"` で課題に関係する pending な Conflict Review Item があり `spec-anchor inject-*` を実行 | CLI exit code 0。stdout JSON (§11.1.2 A): `status="fresh"`、`should_stop=false`、`blocking_reasons=[]`、`pending_conflict_items=[{conflict_id, status:"pending", severity, section_pair, conflict_points, why_conflicting, why_llm_cannot_decide, source_refs, recommended_next_action:"Ask a human to decide this conflict."}, ...]`、`pending_conflict_count=<件数>` (矛盾は停止理由ではなく注入情報。Agentic Search は実行可能。Agent 側が制約 + 矛盾を提示してから停止する) |
...
| [ ] | Setup-project 適用後で Agent CLI が skill / command を認識しない状態で `spec-anchor-setup-system --run-smoke` を実行 (Codex skill path mismatch / Claude command file 不在等) | CLI exit code 0 (warning のみ)。stdout JSON: `status="ok"`、`production_readiness.status="ready"` (Agent CLI 認識性は readiness check に含めず warning 扱い)、`diagnostics=[{"reason_code":<具体>, "message":<warning 文>, "agent_cli_entries":{...}}, ...]`、`agent_cli_entries.codex.project_skill_path=<期待 path>` / `agent_cli_entries.claude.project_command_path=<期待 path>`、`providers[<codex/claude>].version=<取得した CLI version>` |
...
| [ ] | Section Metadata の LLM 生成が一部 section で失敗した状態で利用者が `/spec-core` を発火 | Agent は `spec-anchor core` を実行。CLI 出力の `status="failed"`、`failed_sections` 件数と `diagnostics.section_metadata_generation.failed_sections[i].reason_code` / `warnings` を利用者に伝達。利用者には「Section Metadata の生成に失敗したため停止状態であり、失敗 section を再生成するまで `/spec-inject` / `/spec-realign` は実行できない」と提示<br><br>出力文言例:<br>`/spec-core` は 12 section 中 2 section の Section Metadata 生成に失敗しました。失敗 section: `docs/spec/api.md#0003-payments` (reason: llm_timeout), `docs/spec/auth.md#0001-session` (reason: schema_validation_error)。失敗 section があるため `/spec-inject` / `/spec-realign` は停止します。失敗 section を再生成するには `/spec-core --all` を実行してください。 |
...
| [ ] | `freshness.json` が `status="fresh"` で課題に関係する pending な Conflict Review Item があり利用者が `/spec-inject` を発火 | Agent は §8.3 の Agentic Search で制約を集め、CLI 出力の `pending_conflict_items` の各 item を §2.8 の構造 (conflict_id / severity / conflict_points / why_conflicting / why_llm_cannot_decide / source_refs / recommended_next_action) で利用者に提示する。制約情報 + 矛盾情報を提示してから停止し、矛盾の却下は強制しない。各 item の `recommended_next_action` は default `"Ask a human to decide this conflict."`<br><br>出力文言例:<br>## 今回守る制約<br>- ... (Agentic Search で集めた制約)<br><br>## 人間判断が必要な矛盾 (2 件)<br>1. `conflict_id=section_pair:sha256:v1:...` (severity=high): 左の抜粋=「A は同期処理」、右の抜粋=「A は非同期処理」、why_conflicting=「§3.2 と §5.1 が矛盾」、why_llm_cannot_decide=「両方の根拠が同等で LLM が判断できない」、source_refs=`docs/spec/api.md#0003`, `docs/spec/queue.md#0001`、recommended_next_action=`Ask a human to decide this conflict.`<br>2. `conflict_id=section_pair:sha256:v1:...` (severity=medium): ...<br>矛盾の意味を会話で伝えてください。矛盾ではない場合は `spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` で却下できます。 |
...
| [ ] | `freshness.json` が `status="fresh"` で課題に関係する pending な Conflict Review Item があり利用者が `/spec-realign` を発火 | Agent は §8.3 の Agentic Search で制約を集め、CLI 出力の `pending_conflict_items` を §2.8 の構造 (conflict_id / severity / conflict_points / why_conflicting / why_llm_cannot_decide / source_refs / recommended_next_action) で利用者に提示してから停止する。`/spec-realign` でも矛盾時は答案を生成せず (答案つきの `spec-anchor realign` を呼ばない)、提示停止になる<br><br>出力文言例:<br>## 今回守る制約<br>- ...<br><br>## 人間判断が必要な矛盾 (1 件)<br>1. `conflict_id=section_pair:sha256:v1:...` (severity=high): 左の抜粋=「retry は 3 回」、右の抜粋=「retry は 5 回」、why_conflicting=「§4.1 と §6.2 が矛盾」、why_llm_cannot_decide=「両方が現行 Source Specs に存在」、source_refs=`docs/spec/policy.md#0004`, `docs/spec/retry.md#0001`、recommended_next_action=`Ask a human to decide this conflict.`<br>矛盾の意味を会話で伝えてから回答へ進んでください。答案は生成していません。矛盾ではない場合は `spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` で却下できます。 |
