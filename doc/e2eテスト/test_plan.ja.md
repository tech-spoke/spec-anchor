...
`STATUS` 列の凡例: `done` = ✅ 化完了 / `in-progress` = 着手済 / `todo` = 未着手 / `partial` = 一部のみ ✅

| STATUS | Phase | 対象 | 件数 (完/全) | profile | evidence folder | commit |
|---|---|---|---:|---|---|---|
| done | P0 | エビデンス harness + §11 既存 ✅ 44 確認 | 44/44 | `none` / `fake` | (前セッション) | 7397617 |
| done | P1 §5 | 責務境界 | 15/15 | `none` / `fake` | `P1-section-5` | fc48a7f (+ 9e939f6 fix) |
| done | P1 §10 | 設定ファイル | 69/69 | `none` / `fake` | `P1-section-10-final` | 57f7fdd |
| done | P2 | §6 コマンド体系 | 37/37 | `none` / `fake` / `real-smoke` | `P2-section-6-final` | da7fd59 |
| done | P3 | §7 /spec-core | 118/118 | `fake` (real Qdrant + BGE-M3) | `P3-section-7-final` | (this commit) |
| todo | P3a | §2 + §3 + §4 構造 / 動作モデル / 保持物 | 0/82 | `fake` + `local-service` | — | — |
| todo | P4 | §3.3 freshness gate (再検証) | 0/19 | `fake` | — | — |
| todo | P4a | trace 監査 harness (T-trace-harness) | — | `real-smoke` | — | — |
| todo | P5 | §8 /spec-inject | 0/66 | `fake` + `real-smoke` | — | — |
| todo | P6 | §9 /spec-realign | 0/18 | `fake` + `real-smoke` | — | — |
| todo | P7 | 全 `real-smoke` 系統合 | 0/(80) | `real-smoke` | — | — |

**累計進捗**: ✅ 283 / [ ] 約 170 (約 62%)。最終更新: 2026-05-22 (P3 完了時点)。

**進捗の正本**: 件数とフォルダは本表で管理する。各 [ ] → ✅ 遷移時に本表の対応行を更新する (CLAUDE.md ルール 10 / 12 準拠)。spec doc の ✅ マーク数と本表の累計件数は一致させる。
