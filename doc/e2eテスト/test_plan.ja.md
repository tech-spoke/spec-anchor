...
最終更新: 2026-05-22 (第三回改定: spec doc 本文の verification ✅ 全件 revert で baseline ゼロ。これまでの P0 §11.2 19 + 過去 phase 累計の ✅ をすべて `[ ]` に戻し、信用できる N/M evidence を harness `42854c8` 以降で積み直す方針)。
...
- [ ] **scenario 1: `/spec-inject` inject-section 代表経路**
...
- [ ] **scenario 2: `/spec-inject` inject-chapters 代表経路**
...
- [ ] **scenario 3: `/spec-inject` inject-purpose 代表経路**
...
P7 は P3b + P5 + P6 完了後に着手する (チェーンを踏むには `/spec-inject` `/spec-realign` の hybrid 実装が先に必要)。
P3b は P7 とは独立に **P4 / P5 / P6 着手前の必須ゲート** として先行実施する (§4.2.4)。
