...
| Phase | 対象 | 件数概算 | profile | 理由 |
|---|---|---:|---|---|
| P0 | エビデンス harness 整備 + §11 既存 ✅ 44 件の遡及検証 | 44 | `none` / `fake` | `spec_anchor/testing/evidence.py` + conftest hook 実装、§11 既 ✅ test に SPEC_REF docstring 追加、evidence_map.jsonl 初回生成 |
| P1 | §5 + §10 (config / responsibility) | 83 | `none` / `fake` | 依存なし、静的契約。table-driven で短期間に処理可能 |
| P2 | §6 setup script | 37 | `fake` | P3 以降の前提を作る |
| P3 | §7 /spec-core | 118 | `fake` + `local-service` | 保持物正本。最大件数だが §2 / §3 / §4 と連動して進められる |
| P3a | §2 + §3 + §4 (構造 / 動作モデル / 保持物) | 82 | `fake` + `local-service` | P3 と並走 (同じ artifact を別観点で検証) |
| P4 | §3.3 freshness gate (再検証) | (含 §3.3 19) | `fake` | P3 完了後の状態遷移検証 |
| P4a | trace 監査 harness 整備 (T-trace-harness) | (harness) | `real-smoke` | `spec_anchor/testing/trace.py` 実装、§6.7 のタスク。P5 着手前に完了 |
| P5 | §8 /spec-inject | 66 | `fake` + `real-smoke` | trace 監査を含む |
| P6 | §9 /spec-realign | 18 | `fake` + `real-smoke` | 同上 |
| P7 | 全 `real-smoke` 系統合 | (80) | `real-smoke` | Agent 文言確認、trace 監査の本検証 |
