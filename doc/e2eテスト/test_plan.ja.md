...
|---|---|---:|---:|---:|---:|---:|---|---|
| P0 | §11.2 (§11.1.5 23 件は別 Phase へ移管) | 0/42 | 0/42 | 0/42 | **19/42** ※1 | 0 | (前セッション、P0 は §11 を unchanged 扱い) | 7397617 |
...
※1: P0 §11.2 の prod E2E `19/42` は **spec doc 本文の ✅ マーク件数** であり、field 級 (`evidence_map.jsonl` の `production_e2e_verified` entry) では 0 件。P0 commit `7397617` の commit message が「Pre-existing 44 `✅` in §11 unchanged」と明示している通り、これらは harness 以前の author-assertion 由来で、`8d71934 ✅ now means production_e2e_verified` の relabel ルールを厳格適用すれば revert 候補。本セッションは §11.1.5 23 件のみ revert したため、§11.2 19 件は保留状態。
...
- spec doc ✅ 級 prod E2E: **19** 項目 (P0 §11.2 のみ。field 級証明なし。①§11.2 を §11.1.5 と同様に `[ ]` revert するか、②real Codex/Claude で再 verify して field 級に昇格するか、の 2 択を別 task で判断)
...
備考: spec doc 冒頭の凡例 (L12 / L14 / L16 / L18 / L31) にも `✅` の文字が現れるが、これは legend 定義としての出現であり検証単位ではない。`grep -c "✅"` は legend 込みで 24 件 (legend 5 + §11.2 19) を返すが、累計件数は本文の検証単位 19 件で数える。

spec doc (`EXTERNAL_DESIGN.ja.md`) 本文の `✅` マークは **`production_e2e_verified` を達成した項目** のみ (新基準、ただし P0 §11.2 19 件は author-assertion 由来で field 級証明なし、§11.1.5 の revert と同じ判断を別 task で行う予定)。P1 §5 / P1 §10 / P2 §6 / P3 §7 / §11.1.5 の計 262 件は (2026-05-22 セッションで `[ ]` に戻したため) spec doc 上 `[ ]` のまま残る。これらの hybrid / unit レベルの進捗は本表 §4.2.2 の per-level 列で読む。
...
最終更新: 2026-05-22 (P3 hybrid field 級記録 81/118 達成 + P0 §11.2 prod E2E は spec doc ✅ 19 件のみで field 級は 0/42、§4.2.2 表を `✓` 表記から N/M 表記へ書き換え、P3b / P7 を §4.2.2 表から除外して scenario checklist 管理へ移行)。
