...
`✅` は **`production_e2e_verified` を達成した項目** に限り付与する。`hybrid_verified` までしか達成していない項目は `[ ]` のまま残す。これ以前の作業で hybrid 段階のまま `✅` を付けていた §5 / §6 / §7 / §10 の項目は、2026-05-22 セッションで `[ ]` に一旦戻した (件数は本表 §4.2.2 件数列の `0/M` に反映)。`✅` を再付与するのは P7 (横串 production E2E phase, §4.2.4 参照) で当該項目を real Codex / Claude 経由で踏み直した時のみとする。
...
|---|---|---:|:-:|:-:|:-:|:-:|:-:|---|---|
| P0 | harness + §11 既存 ✅ 確認 (現行 42 件 / 旧 44 件) | 42/42 | ✓ | ✓ | ✓ | ✓ | ✓ | (前セッション) | 7397617 |
...
- production_e2e_verified: **42** 項目 (P0 §11 のみ。spec doc 本文の `✅` 42 件と一致)

備考: spec doc 冒頭の凡例 (L11 / L25) にも `✅` の文字が現れるが、これは legend 定義としての出現であり検証単位ではない。`grep -c "✅"` は legend 込みで 44 件を返すが、累計件数は本文の検証単位 42 件で数える。

spec doc (`EXTERNAL_DESIGN.ja.md`) 本文の `✅` マークは **`production_e2e_verified` を達成した項目** のみ。P1 §5 / P1 §10 / P2 §6 / P3 §7 の計 239 件は (2026-05-22 セッションで `[ ]` に戻したため) spec doc 上 `[ ]` のまま残る。これらの hybrid / unit レベルの進捗は本表 §4.2.2 の per-level 列で読む。
