...
| **P3b** | **/spec-core real-smoke gate (P4/P5/P6 着手前の必須ゲート)** | 0/TBD | 0/TBD | 0/TBD | 0/TBD | 0 | — | — |
...
| P7 | P1-P6 横串 real production E2E (再定義: §4.2.5 参照) | 0/TBD | 0/TBD | 0/TBD | 0/TBD | 0 | — | — |
...
最終更新: 2026-05-22 (P3 hybrid field 級記録 81/118 達成 + P0 §11.2 prod E2E は spec doc ✅ 19 件のみで field 級は 0/42、§4.2.2 表を `✓` 表記から N/M 表記へ書き換え)。
...
- `/spec-core` (incremental / `--all` / `--rebuild` の代表 3 経路) を tmp_path project に対して実行
- 生成 artifact が `.spec-anchor/state/*.json` と `.spec-anchor/context/*.json` に書き出される
- `freshness_report.status == "fresh"` (blocked / failed に落ちていない)
- `retrieval_index_status == "success"` (skipped / failed に落ちていない)
- `related_sections_status == "success"` (failed / blocked に落ちていない)
- `core_progress.json` の `stages[].llm_provider` が `fake` ではなく real provider id (`codex` / `claude_*`) を記録している
- fake / stub / in-memory fallback に落ちていないこと (`diagnostics` 各 stage の `provider` 列で確認)

P3b 完了条件:

- 上記が代表 3 経路すべてで成功する
- 同じ入力で 2 回回したとき 2 回目が `skipped_unchanged` に正しく落ちる (idempotency)
- 失敗系として Qdrant 停止状態で 1 経路を踏み、`retrieval_index_status == "failed"` と復旧手順 (`/spec-core --rebuild`) を JSON で返すこと
...
- `SPEC_ANCHOR_FAKE_LLM` および `SPEC_ANCHOR_FAKE_RETRIEVAL` を unset
- real Codex CLI / real Claude CLI / real Qdrant / real BGE-M3 が応答する状態
- P3b 経由で生成された artifact を入力として `/spec-inject` (`inject-section` / `inject-chapters` / `inject-purpose` の代表経路、少なくとも 1 種ずつ)
- `/spec-realign` (代表課題 1 件で `/spec-core` 生成 artifact を根拠に answer を整形できること)
- 生成 artifact (`.spec-anchor/state/*.json` / `.spec-anchor/context/*.json`) が `/spec-inject` `/spec-realign` で根拠として読まれること (chain 検証)
...
- Agent CLI (Claude Code / Codex) の trace に `inject-search` → `inject-section` 等の期待 tool call 順序が記録されていること

完了条件:

- 上記代表経路がすべて成功する
- artifact が `/spec-core` 完了後に変更されず、`/spec-inject` `/spec-realign` で参照できる (chain consistency)
...
**進捗の正本**: 件数とフォルダは本表で管理する (CLAUDE.md ルール 10 / 12 準拠)。spec doc の `✅` マークは **`production_e2e_verified` を達成した項目のみ** を示す。`implemented` / `unit_verified` / `hybrid_verified` / `real_smoke_verified` の進捗は本表 §4.2.2 の各列でのみ管理し、spec doc には `✅` を付けない。
