...
**spec doc (`EXTERNAL_DESIGN.ja.md`) の `✅` マークの意味 (2026-05-22 改定)**:
`✅` は **`production_e2e_verified` を達成した項目** に限り付与する。`hybrid_verified` までしか達成していない項目は `[ ]` のまま残す。これ以前の作業で hybrid 段階のまま `✅` を付けていた §5 / §6 / §7 / §10 の項目は、2026-05-22 セッションで `[ ]` に一旦戻した (件数は本表 §4.2.2 件数列の `0/M` に反映)。同セッションで `§11.1.5` の 23 件も `[ ]` に戻した (理由: P0 commit `7397617` は §11 を "unchanged" として保持しただけで実 verification をしておらず、harness 以前の author-assertion `✅` だったため新基準に整合させた。新規 Phase で改めて踏み直す)。`✅` を再付与するのは P7 (横串 production E2E phase, §4.2.5 参照) または §11.1.5 専用 Phase で当該項目を real Codex / Claude 経由で踏み直した時のみとする。
...
凡例 (`unit` / `hybrid` / `real-smoke` / `prod E2E` 各列、2026-05-22 改定):

各 cell は **`N/M` 形式** で表記する。`✓` / `部分` / `—` は使わない (legend の曖昧さによる誤読を防ぐため、過去 cell も書き換えた。経緯は §4.2.2 後段の Phase 別注記を参照)。

- `M` (分母) — spec doc 本文の検証単位件数 (`[ ]` + 該当 phase の spec doc ✅ の合計)
- `N` (分子) — `doc/e2eテスト/evidence/<dir>/evidence_map.jsonl` に「`verification_level: <該当 level>` AND `result: passed`」で記録された unique (spec_section, spec_line) の件数
- `N=0` — その検証レベルで field 級記録が 1 件もない (= 実体上未検証 / 過去は無記録)
- `N=M` — 全件 field 級記録あり
- `NA` — そのレベルが技術的に適用外 (例: config loader 単体に real LLM は不要)
- 旧 harness 時代に test を passed 記録したが `verification_level` field 自体を書いていない entry は **本表の N/M に含めない**。代わりに最右の `field 未付与 passed` 列で件数を併記する (情報損失防止のため別管理)

**`prod E2E (✅)` 列の特例**: spec doc 本文の ✅ マーク件数を表示する (例: P0 §11.2 = 19/42)。spec doc ✅ は本来 `evidence_map.jsonl` の `production_e2e_verified` 件数と一致すべきだが、harness 以前の author-assertion (P0 §11 既存 ✅) は field 級証明を持たない。乖離は §4.2.3 末尾の注記で個別管理する。

#### 4.2.2 Phase 別検証状態

「件数」列は **spec doc の `✅` (production E2E 完了)** の数を示す。hybrid 以下で踏んだ件数は本表の各 verification level 列で読む。

| Phase | 対象 | unit (field) | hybrid (field) | real-smoke (field) | prod E2E (✅) | field 未付与 passed | evidence | commit |
|---|---|---:|---:|---:|---:|---:|---|---|
| P0 | §11.2 (§11.1.5 23 件は別 Phase へ移管、§11.2 19 件も 2026-05-22 第三回改定で revert) | 0/42 | 0/42 | 0/42 | 0/42 | 0 | (前セッション、P0 は §11 を unchanged 扱い、本セッションで全 revert) | 7397617 |
| P1 §5 | 責務境界 | 0/15 | 0/15 | 0/15 | 0/15 | 15 | `P1-section-5` | fc48a7f (+ 9e939f6 fix) |
| P1 §10 | 設定ファイル | 0/69 | NA/69 | NA/69 | 0/69 | 69 | `P1-section-10-final` | 57f7fdd |
| P2 §6 | コマンド体系 | 0/37 | 0/37 | 0/37 | 0/37 | 37 | `P2-section-6-final` | da7fd59 |
| P3 §7 | /spec-core | **37/118** | **81/118** | 0/118 | 0/118 | 118 | `P3-section-7-final` + `P3-relabel-*` | 56ddb61 (hybrid), ea332da (relabel), 42854c8 (verification_level harness) |
| P3a | §2 / §3 / §4 構造 / 動作モデル / 保持物 | 0/82 | 0/82 | 0/82 | 0/82 | 0 | — | — |
| P4 | §3.3 freshness gate (再検証) | 0/19 | 0/19 | 0/19 | 0/19 | 0 | — | — |
| P4a | trace 監査 harness (T-trace-harness) | NA | NA | NA | NA | 0 | — | — |
| P5 | §8 /spec-inject | 0/66 | 0/66 | 0/66 | 0/66 | 0 | — | — |
| P6 | §9 /spec-realign | 0/18 | 0/18 | 0/18 | 0/18 | 0 | — | — |
| **§11.1.5** | **CLI error contract (transverse: `/spec-core` / `/spec-inject` / `/spec-realign` / `spec-anchor-watch`)** | 0/23 | 0/23 | 0/23 | 0/23 | 0 (先行セッション uncommitted impl のみ) | — | — |

**本表に含めない Phase**: P3b (`/spec-core` real-smoke gate) と P7 (横串 real production E2E) は spec doc 検証行 (`[ ]`) を直接数える counting unit と適合しない (代表 scenario 単位で進捗を管理する phase であり、1 scenario が複数 spec 行に同時 touch するため M が定まらない)。進捗は §4.2.4 (P3b) / §4.2.5 (P7) の scenario checklist で個別管理する。

※1: P0 §11.2 の prod E2E は 2026-05-22 第三回改定で 19/42 → 0/42 に revert した。理由: P0 commit `7397617` の commit message が「Pre-existing 44 `✅` in §11 unchanged」と明示している通り、§11 全体の ✅ は harness 以前の author-assertion 由来で field 級証明が無い。`8d71934 ✅ now means production_e2e_verified` の relabel ルールを §11.1.5 だけでなく §11.2 にも厳格適用した結果、spec doc 本文の verification ✅ は 0 件 (凡例 6 件は記号の定義文として残置、verification mark ではない)。

「field 未付与 passed」列の数は、`doc/e2eテスト/evidence/` 配下の旧 harness 時代の evidence_map.jsonl 内で `result: passed` だが `verification_level` field 自体を持たない entry の件数。`verification_level` field を出力する harness は commit `42854c8` で初めて追加されたため、それ以前の Phase (P1/P2/P3 の最初の commit 時) の evidence は全て field 未付与。各 N/M cell には含めない (情報損失防止のため別列で残す)。これらを field 級記録に格上げするには既存 test に `verification_level` marker を遡及付与して再実行する必要があり、`doc/TODO.ja.md` の `T-retroactive-verification-level` として独立 task 化した。
...
- **§11.1.5 の impl 部分**: 先行セッションが `spec_anchor/{core.py,inject.py,realign.py,cli.py}` に 3 関数 (`_detect_qdrant_backend_failure` / `_config_missing_error_result` / `_realign_exit_code` + watcher ConfigError JSON 化) を working tree に追加済 (未 commit)。これは §11.1.5 表 23 行のうち config 不在系 + Qdrant 到達不能系 + realign exit code 系を実装する。残り (Section Metadata / Chapter Key Anchor LLM 失敗、setup-project / setup-system 系、PATH 不在系) は未着手。unit / hybrid / real-smoke / production E2E 全て未実施。

#### 4.2.3 累計進捗 (検証レベル別)

`✅ 283` のような単一の累計表記は使わない (production readiness レベルが誤読される)。検証レベル別に分解する。
**`production_e2e_verified` 件数は spec doc 本文 (§1 以降) の `✅` マーク数と一致する** (この行が正本)。

各レベルの集計基準を厳格化した (2026-05-22 第二回改定)。**field 級** = `evidence_map.jsonl` 内 `verification_level: <該当 level>` AND `result: passed` の unique (spec_section, spec_line) 件数。**spec doc ✅ 級** = spec doc 本文の `✅` マーク件数 (P0 §11.2 19 件のみ、harness 以前の author-assertion で field 級証明なし)。

- field 級 unit_verified: **37** 項目 (P3 §7 のうち `tests/test_spec_core_acceptance.py` で `verification_level="hybrid_verified"` 付与済 marker のうち unit カバーする部分が evidence 化された分)
- field 級 hybrid_verified: **81** 項目 (P3 §7。`P3-relabel-verify` evidence + 2026-05-22 累積 evidence)
- field 級 real_smoke_verified: **0** 項目
- field 級 production_e2e_verified: **0** 項目
- spec doc ✅ 級 prod E2E: **0** 項目 (2026-05-22 第三回改定で §11.2 19 件も `[ ]` に revert したため、verification ✅ は spec doc 本文から消失。凡例 6 件は記号の定義文で verification mark ではない)
- 旧 harness 「field 未付与 passed」(N/M に含めない参考値): P1 §5=15、P1 §10=69、P2 §6=37、P3 §7=118 (合計 239 件)。これらを field 級に格上げするには `T-retroactive-verification-level` 参照

備考: spec doc 冒頭の凡例 (L12 / L14 / L16 / L18 (2 件) / L31) にも `✅` の文字が現れるが、これは legend 定義としての出現であり検証単位ではない。`grep -c "✅"` は 6 件 (全て legend) を返すが、累計件数は本文の検証単位 0 件で数える。

spec doc (`EXTERNAL_DESIGN.ja.md`) 本文の `✅` マークは **`production_e2e_verified` を達成した項目** のみ (新基準)。P0 §11.2 19 件 + §11.1.5 23 件 + P1 §5 15 + P1 §10 69 + P2 §6 37 + P3 §7 118 の計 281 件は全て (2026-05-22 セッションで `[ ]` に戻したため) spec doc 上 `[ ]` のまま残る。これらの hybrid / unit レベルの進捗は本表 §4.2.2 の per-level 列で読む。verification ✅ をゼロから積み直すための baseline 状態。

**本累計の対象外** (scenario ベースの phase): P3b (5 scenario) と P7 (6 scenario) は spec doc 検証行 (`[ ]`) を直接 N/M で数える counting unit と適合しない。進捗は §4.2.4 (P3b checklist) / §4.2.5 (P7 checklist) で個別管理し、本累計には積まない。

最終更新: 2026-05-23 (第三回改定の spec doc 本文 baseline ゼロは維持。P3b / P7 は scenario ベースのため本累計に積まず、§4.2.4 / §4.2.5 の checklist と evidence map で完了を管理する)。

#### 4.2.4 P3b: /spec-core real-smoke gate (P4/P5/P6 着手前の必須ゲート)

`/spec-inject` と `/spec-realign` は `/spec-core` が生成した artifact に依存する。
`/spec-core` の real LLM 経路が壊れていた場合、P5 / P6 を fake / hybrid でいくら積んでも、後で破綻が表面化する。
よって `/spec-core` 単独の real-smoke を **P4 / P5 / P6 着手前の必須ゲート (P3b)** とする。
...
- tmp_path project に対して実行

P3b scenario checklist (進捗管理単位、各 scenario の合計 = 5 件):

- [x] **scenario 1: incremental 経路**
  - 実行: `spec-anchor core` (flag なし)
  - 検証: `freshness_report.status == "fresh"`、`retrieval_index_status == "success"`、`related_sections_status == "success"`、`core_progress.json` の `stages[].llm_provider` が real provider id (`codex` / `claude_*`)、`diagnostics` の各 stage `provider` 列に fake / stub / in-memory fallback が現れない
- [x] **scenario 2: `--all` 経路**
  - 実行: `spec-anchor core --all` で LLM 由来 cache クリア + 全 section 再評価
  - 検証: scenario 1 と同じ + `mode == "full"`
- [x] **scenario 3: `--rebuild` 経路**
  - 実行: `spec-anchor core --rebuild` で Qdrant collection drop + recreate
  - 検証: scenario 2 と同じ + `core_progress.json` の `stages.section_collection_upsert.action == "upserted_full"`
- [x] **scenario 4: idempotency**
  - 実行: scenario 1 と同じ入力で 2 回目を実行
  - 検証: 2 回目の `retrieval_index_status == "skipped_unchanged"`、`related_sections_status == "skipped_unchanged"`、artifact が変更されない
- [x] **scenario 5: 失敗系 (Qdrant 不到達)**
  - 実行: Qdrant service を停止した状態で scenario 1 を実行
  - 検証: `status == "failed"`、`retrieval_index_status == "failed"`、`warnings[]` に復旧手順 (`/spec-core --rebuild` 指示) を含む

P3b 完了条件:

- 上記 5 scenario が全て合格する
- evidence: `doc/e2eテスト/evidence/<日付>-P3b/evidence_map.jsonl` に 5 件の `verification_level: "real_smoke_verified"` entry を記録 (本 phase の進捗は本書 §4.2.2 の N/M ではなく本 checklist で読む)
- 完了証跡: `doc/e2eテスト/evidence/2026-05-23-P3b/evidence_map.jsonl` に 5 件の `verification_level: "real_smoke_verified"` / `result: "passed"` entry を記録した。

**P3b が未完了の状態で P4 / P5 / P6 を完了扱いにしない**。
P3 §7 (hybrid) が通っているだけでは `/spec-core` の real LLM 経路の健全性は担保されない。

#### 4.2.5 P7 の再定義: 横串 real production E2E phase
...
前提: P3b (`/spec-core` real-smoke gate) が完了していること (§4.2.4)。P7 は P3b を前提に inject / realign チェーンを通す。

前提条件 (全 scenario 共通):

- `SPEC_ANCHOR_FAKE_LLM` および `SPEC_ANCHOR_FAKE_RETRIEVAL` を unset
- real Codex CLI / real Claude CLI / real Qdrant / real BGE-M3 が応答する状態
- P3b 完了済 (artifact が生成済) であること

P7 scenario checklist (進捗管理単位、各 scenario の合計 = 6 件):

- [x] **scenario 1: `/spec-inject` inject-section 代表経路**
  - 実行: P3b 生成 artifact を入力に `/spec-inject` で `spec-anchor inject-section <id>` 呼び出しを 1 件
  - 検証: 返却 payload に Section Metadata (summary / search_keys / identifiers / related_sections / heading_path) が揃う、Agent CLI 出力が §8.5 の 5 セクション構造
- [x] **scenario 2: `/spec-inject` inject-chapters 代表経路**
  - 実行: `/spec-inject` で `spec-anchor inject-chapters` 呼び出し 1 件
  - 検証: 返却 path が `chapter_anchors.json` の絶対 path、Agent が `Read` で必要箇所を抽出し制約に組み込む trace 観測
- [x] **scenario 3: `/spec-inject` inject-purpose 代表経路**
  - 実行: `/spec-inject` で `spec-anchor inject-purpose` 呼び出し 1 件
  - 検証: 返却 `purpose` field に Purpose 全文、`core_concept_path` に Core Concept の絶対 path
- [x] **scenario 4: `/spec-realign` 正常経路**
  - 実行: 代表課題 1 件 + `agent_answer` 4 区分構造を `--answer-json` で渡す
  - 検証: RealignResult の `answer` 4 区分が出力される、artifact が P3b 完了後変更されていない (chain consistency)
- [x] **scenario 5: chain consistency**
  - 実行: scenario 1-4 を 1 連続セッションで実行
  - 検証: 全 scenario で `freshness_report.status` が `fresh` / `degraded` のいずれか (`blocked` / `failed` に落ちない)、`retrieval_index_status` / `related_sections_status` が `success` (fallback / degraded / stale に落ちない)
- [x] **scenario 6: 失敗系 1 経路**
  - 実行: Qdrant 停止 / config 不在 / pending conflict のいずれか 1 つを発生させて `/spec-inject` または `/spec-realign` を実行
  - 検証: CLI が JSON `error` または `blocking_reasons` を Agent が解釈できる形で返す、Agent が利用者に §11.2 の停止時出力構造で復旧手順を伝達

P7 完了条件:

- 上記 6 scenario が全て合格する
- evidence: `doc/e2eテスト/evidence/<日付>-P7/evidence_map.jsonl` に 6 件の `verification_level: "production_e2e_verified"` entry を記録 (chain consistency と失敗系の確認が含まれる)
- 完了証跡: `doc/e2eテスト/evidence/2026-05-23-P7/evidence_map.jsonl` に 6 件の `verification_level: "production_e2e_verified"` / `result: "passed"` entry を記録した。

P7 は P3b + P5 + P6 完了後に着手する (チェーンを踏むには `/spec-inject` `/spec-realign` の hybrid 実装が先に必要)。2026-05-23 に完了済み。
P3b は P7 とは独立に **P4 / P5 / P6 着手前の必須ゲート** として先行実施する (§4.2.4)。2026-05-23 に完了済み。

**進捗の正本**:

- spec doc 検証行 (`[ ]` / `✅`) ベースの phase (P0 / P1 / P2 / P3 / P3a / P4 / P5 / P6 / §11.1.5) の進捗は §4.2.2 N/M 表で管理 (CLAUDE.md ルール 10 / 12 準拠)
- scenario ベースの phase (P3b 5 scenario / P7 6 scenario) の進捗は §4.2.4 / §4.2.5 の checklist で管理 (本表には含めない)
- spec doc の `✅` マークは **`production_e2e_verified` を達成した項目のみ** を示す。`implemented` / `unit_verified` / `hybrid_verified` / `real_smoke_verified` の進捗は §4.2.2 / §4.2.4 / §4.2.5 で管理し、spec doc には `✅` を付けない
...
```json
{"spec_section": "§8.4", "spec_line": 791, "checkbox_text": "section-level hybrid retrieval | spec-anchor inject-search \"<query>\" | top-K の Section payload (...)", "test_id": "tests/test_spec_inject.py::test_inject_search_returns_topk", "profile": "fake", "method": "入出力比較", "verification_level": "hybrid_verified", "result": "passed", "duration_sec": 0.42, "executed_at": "2026-05-22T10:30:15+09:00"}
{"spec_section": "§3.3", "spec_line": 245, "checkbox_text": "Source Specs が変更されたが /spec-core で更新されていない | 停止する | /spec-core を実行する", "test_id": "tests/test_freshness.py::test_dirty_source_blocks_inject", "profile": "fake", "method": "入出力比較", "verification_level": "unit_verified", "result": "passed", "duration_sec": 0.18, "executed_at": "2026-05-22T10:30:18+09:00"}
```
...
- `method`: `入出力比較` / `artifact 内容確認` / `Agent 出力文言確認` / `tool call trace 監査`
- `verification_level`: `unit_verified` / `hybrid_verified` / `real_smoke_verified` / `production_e2e_verified` (§4.2.1 定義)。**spec doc `✅` 化の判定はこのフィールドを見る** (§8.1)
...
def test_inject_search_returns_topk():
    ...
    PROFILE: fake
    METHOD: 入出力比較
    VERIFICATION_LEVEL: hybrid_verified
    """
    ...
```
...
- `VERIFICATION_LEVEL`: **必須**。`unit_verified` / `hybrid_verified` / `real_smoke_verified` / `production_e2e_verified` のいずれか (§4.2.1 定義)。test が実際に踏む経路に応じて選ぶ。`production_e2e_verified` のみが spec doc `✅` の付与根拠になる (§8.1)

`@pytest.mark.parametrize` 行ごとに verification_level が異なる場合は `pytest.param(..., marks=[pytest.mark.spec_ref(section, line, profile=..., method=..., verification_level=...)])` の形で per-row 指定する (docstring 級の宣言を override する)。
...
`✅` は `production_e2e_verified` を達成した項目のみに付与する (§4.2.1 / 外部設計書 凡例)。
`[ ]` を `✅` に変更する条件は次のすべて:
...
- **同じ entry に `verification_level: "production_e2e_verified"` が記録されている** (`unit_verified` / `hybrid_verified` / `real_smoke_verified` のみの passed は `✅` 化の根拠にしない)
- spec doc の `[ ]` を `✅` に置き換える commit message に、エビデンス folder path と該当 test ID 一覧、および当該 evidence の `verification_level` を明示する (§7.5 形式)

production E2E が技術的に適用外の項目 (例: §10 設定ファイルの config loader 単体テスト) は、`✅` 化せずに本表で `prod E2E = NA` として個別に approved_exception を残し、test_plan §10 (残 TODO) に exception 理由を記載する。`NA` だからといって `✅` を機械的に付与しない。
...
- ✅ test docstring 規約: `SPEC_REF: §<章> L<行>`, `PROFILE:`, `METHOD:`, `VERIFICATION_LEVEL:` 必須 (2026-05-22 改定で VERIFICATION_LEVEL 追加)
- ✅ §11 既存 ✅ 44 件の遡及検証は P0 で実施
- ✅ Codex CLI の session log 所在 / 形式: `codex exec --json` の stdout JSONL を証跡正本とし、最終 message は `-o <path>` の text file に保存する。P7 証跡は `doc/e2eテスト/evidence/2026-05-23-P7/stdout/*.stdout.jsonl` と `artifacts/*.last-message.txt`。
- ✅ Claude Code の `--print` mode trace: `claude -p --verbose --output-format stream-json --include-partial-messages` で tool call trace を stdout JSONL に残す。既存証跡は `doc/e2eテストCODEX実施用/evidence/20260523-101202-codex-e2e-inject-output/stdout/claude-spec-inject-8-5-v2.stdout`。
- ✅ `real-smoke` profile はローカル手動実行を既定とし、CI では明示 opt-in のみ実行する。理由は実 Codex / Claude CLI、実 Qdrant、BGE-M3 cache、外部 credential に依存するため。
- ✅ `local-service` profile の Qdrant version は 1.17.1 をこの検証時点の baseline とする。`doc/e2eテスト/evidence/2026-05-23-P3b/artifacts/setup-system-check-only.json` で `qdrant_server_version="1.17.1"`、`qdrant_client` version `1.17.1` を確認した。
- ✅ `SPEC_ANCHOR_E2E_EVIDENCE_DATE` default は pytest 実行時の local date (`YYYY-MM-DD`) とする。Phase 固定が必要な場合は `SPEC_ANCHOR_E2E_EVIDENCE_DATE=<phase-tag>` で override する (`spec_anchor/testing/evidence.py` の `_resolve_default_target_dir`)。
