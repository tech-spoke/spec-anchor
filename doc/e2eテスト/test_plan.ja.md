...
| Phase | 対象 | 件数 (✅/全) | impl | unit | hybrid | real-smoke | prod E2E | evidence | commit |
|---|---|---:|:-:|:-:|:-:|:-:|:-:|---|---|
| P0 | harness + §11 既存 ✅ 確認 (§11.2 19 件のみ; §11.1.5 23 件は別 Phase へ移管) | 19/42 | ✓ | ✓ | ✓ | ✓ | 部分 | (前セッション) | 7397617 |
| P1 §5 | 責務境界 | 0/15 | ✓ | ✓ | — | — | — | `P1-section-5` | fc48a7f (+ 9e939f6 fix) |
| P1 §10 | 設定ファイル | 0/69 | ✓ | ✓ | NA | NA | — | `P1-section-10-final` | 57f7fdd |
| P2 §6 | コマンド体系 | 0/37 | ✓ | ✓ | 部分 | 部分 | — | `P2-section-6-final` | da7fd59 |
| P3 §7 | /spec-core | 0/118 | ✓ | ✓ | ✓ | — | — | `P3-section-7-final` | 56ddb61 (hybrid), ea332da (relabel) |
| **P3b** | **/spec-core real-smoke gate (P4/P5/P6 着手前の必須ゲート)** | 0/(TBD) | — | — | — | — | — | — | — |
...
| P7 | P1-P6 横串 real production E2E (再定義: §4.2.5 参照) | 0/(TBD) | — | — | — | — | — | — | — |
| **§11.1.5** | **CLI error contract (transverse: `/spec-core` / `/spec-inject` / `/spec-realign` / `spec-anchor-watch`)** | 0/23 | 部分 | — | — | — | — | — | (先行セッション uncommitted) |
...
- implemented: **281** 項目 (P0 42 + P1 §5 15 + P1 §10 69 + P2 §6 37 + P3 §7 118)
- unit_verified: **281** 項目 (上記と同じ集合が単体レベルで通った)
- hybrid_verified: **160** 項目 (P0 42 + P3 §7 118。P3 は LLM stub 含む)
- real_smoke_verified: **42** 項目 (P0 のみ。P2 §6 setup-system probe は「部分」扱いで累計には積まない)
- production_e2e_verified: **19** 項目 (P0 §11.2 のみ。spec doc 本文の `✅` 19 件と一致)
...
spec doc (`EXTERNAL_DESIGN.ja.md`) 本文の `✅` マークは **`production_e2e_verified` を達成した項目** のみ。P1 §5 / P1 §10 / P2 §6 / P3 §7 / §11.1.5 の計 262 件は (2026-05-22 セッションで `[ ]` に戻したため) spec doc 上 `[ ]` のまま残る。これらの hybrid / unit レベルの進捗は本表 §4.2.2 の per-level 列で読む。

最終更新: 2026-05-22 (P3 hybrid 完了 + spec doc honest-relabel 完了時点、real-smoke / production E2E は P0 のみ達成)。
