...
**Phase 1 入り口の優先順序（pivot 決定を尊重した fallback ladder、§3.9 参照）**:

```
1. 案 B（第一選択）
   = GRAG 本来の使い方。SchemaLLMPathExtractor / kg_extractors / PropertyGraphIndex
     の標準フローに乗り、その中で使う LLM backend を「Claude/Codex CLI subprocess」
     に差し替えるだけ。Extractor で使う LLM を Claude/Codex に差し替えるだけの構成
   ↓ 案 B が技術的に成立しないと実証できた場合のみ
2. 案 C（fallback 1）
   = kg_extractors orchestration + CustomPGExtractor + ImplicitPathExtractor。
     SchemaLLMPathExtractor は使わず spec-grag 側に抽出を寄せるが、
     LlamaIndex の TransformComponent flow には乗る
   ↓ 案 B / 案 C 両方が成立しないと実証できた場合のみ
3. GRAG 撤回（最終手段）
   = pivot 撤回、自前 canonical graph（SQLite / NetworkX / FAISS）。
     LlamaIndex は optional adapter または完全に外す
```

→ **Phase 1 最初の spike は案 B（`CodexCLIAdapter(LLM)` の実装と SchemaLLMPathExtractor 駆動）**。「方向 1 / 方向 2 の対称 2 択」ではなく、案 B から始めて行き詰まったら次に降りる優先順序。

> Claude の前案（§3.9 旧版「方向 1 / 方向 2 の対称 2 択」）も誘導の一形態だった: pivot 決定（GraphRAG 採用）からの距離を無視して GRAG 撤回を同列の選択肢として並べていた（memory `feedback_path_a_local_optimum.md` の「対称提示も誘導」/ `feedback_grag_purpose_drift.md` の「pivot 距離無視」参照）。
...
### 3.9 Phase 1 入り口 — 優先順序付き fallback ladder（案 B → 案 C → GRAG 撤回）

> **重要**: 旧版（2026-04-28 改訂初版）では「方向 1 vs 方向 2 の対称 2 択」と書いていたが、これは **pivot 決定（GraphRAG 採用、commit b45d95f）からの距離を無視した並列提示**で、GRAG 撤回を GRAG 採用と同列の選択肢として並べていた。これも誘導の一形態。本節は pivot 決定を尊重した優先順序に書き直したもの。

ユーザー決定（2026-04-28）に基づき、Phase 1 入り口は **案 B → 案 C → GRAG 撤回** の 3 段階 fallback ladder。**「対称比較」ではなく「上から順に試して、行き詰まったら次に降りる」**。

#### 第一選択: 案 B（GRAG 本来の使い方）

**理由**: SchemaLLMPathExtractor / kg_extractors / PropertyGraphIndex は、文書から entity / relation / path 候補を抽出して graph に組み立てる **GRAG の中核機能**。これを使うのが LlamaIndex 採用の自然な構成であり、その中で使う LLM backend を「OpenAI API / Anthropic API」ではなく「Claude/Codex CLI subprocess」に差し替えるだけ。

```
[章 markdown]
  ↓ PropertyGraphIndex.from_documents
[SchemaLLMPathExtractor]
  ↓ schema 強制 prompt を組み立て、CodexCLIAdapter に投げる
[CodexCLIAdapter(LLM)] ← spec-grag が実装する subprocess wrapper
  ↓ subprocess: claude --print --output-format json --json-schema (またはCodex)
[Claude/Codex CLI が応答 JSON を返す]
  ↓ adapter: CompletionResponse 形式に変換
[SchemaLLMPathExtractor: pydantic strict validation + retry]
  ↓
[PropertyGraphIndex が graph_store / vector_store に投入]
  ↓
spec-grag Orchestrator: Concept 承認・未承認遮断・safe delete・
                        4 軸付与・InjectionContext / RealignResult
```
...
| 担当 | 責務 |
|---|---|
| SchemaLLMPathExtractor | 抽出スキーマ・prompt・pydantic validation・retry を持つ「抽出の主体」|
| CodexCLIAdapter（spec-grag が実装） | Extractor が利用する LLM backend。prompt を受けて応答を返すだけ |
| PropertyGraphIndex | Extractor 結果を graph に組み立てる |
| spec-grag Orchestrator | schema 定義、Concept 承認、未承認遮断、4 軸評価、Conflict 昇格、InjectionContext 構築 |

**Phase 1 ステップ 0 で詰める**:
- `CodexCLIAdapter(LLM)` の最小 method 一覧と subprocess 統合 spike（spike 05 として実装）
- SchemaLLMPathExtractor 日本語プロンプト化と spec-grag Core schema を Literal 型で表現する方法（spike 06）
- vector_store の VECTOR_SOURCE_KEY 連結問題を正規パターンで解消（spike 03 の 0 件問題）

**案 B が成立する条件**:
- CodexCLIAdapter が `LLM` interface の最低限の method を充足し、SchemaLLMPathExtractor が抽出を完了する
- pydantic ValidationError 時の retry が現実的に動く（無限ループ / rate limit / 認証切れの吸収）
- vector_store 連結が動作し、retrieval が機能する

**案 B が成立しない兆候**（→ 案 C に降りる判定基準）:
- adapter の sync/async/streaming 整合が CLI subprocess の挙動と根本的に合わない
- SchemaLLMPathExtractor の LlamaIndex 内蔵 retry が CLI subprocess の失敗パターン（認証切れ / rate limit）を扱えない
- LlamaIndex `Settings.llm` 解決パスが多すぎて adapter 差し込みが confusing になる

#### 第二選択（fallback 1）: 案 C（案 B が成立しなかった場合）

**理由**: 案 B で `LLM` adapter 統合が困難でも、`kg_extractors` orchestration は使える。SchemaLLMPathExtractor を諦めて `CustomPGExtractor(TransformComponent)` を spec-grag が実装し、その内部で Claude/Codex CLI を直接呼ぶ。LlamaIndex の chunk 化 / progress / async pipeline は活用できる。

```
[章 markdown]
  ↓ PropertyGraphIndex.from_documents(
      kg_extractors=[ImplicitPathExtractor(), CustomPGExtractor()])
[CustomPGExtractor: spec-grag が TransformComponent を継承]
  ↓ subprocess で Claude/Codex CLI を呼ぶ（adapter 経由ではない）
  ↓ JSON parse + spec-grag Validator（自前 retry）
[graph_store に投入]
```

**Phase 1 ステップ 1 で詰める**（案 B が成立しなかった場合のみ）:
- CustomPGExtractor の TransformComponent 適合
- LlamaIndex の `Settings.llm` 解決を avoid する設計
- prompt 工学・retry・chunk 制御を spec-grag 側で実装

**案 C が成立しない兆候**（→ GRAG 撤回に降りる判定基準）:
- TransformComponent flow が spec-grag の Concept 承認・未承認遮断と整合できない
- LlamaIndex の chunk 化が日本語仕様書の章構造と合わない
- vector_store 連結が案 B と同様に解決できない

#### 第三選択（最終手段）: GRAG 撤回（案 B / 案 C 両方が成立しなかった場合のみ）

**理由**: 案 B / 案 C どちらも実証で成立しないなら、LlamaIndex を core dependency にする根拠が消える。pivot 決定を撤回し、自前 canonical graph（SQLite / NetworkX / FAISS）に切り替える。

```
- canonical graph: SQLite / JSONL / NetworkX 等
- vector store: FAISS / Chroma / SQLite-vec（直接利用）
- LlamaIndex: 完全に外す、または optional adapter に格下げ
- 抽出: Claude/Codex CLI subprocess（案 A の subprocess 部分を流用）
- LightRAG / HippoRAG: optional 波及先 retriever
```

**Phase 2 以降で詰める**（GRAG 撤回が確定した場合のみ）:
- 自前 graph store の永続化形式
- 自前 retriever の fusion 設計
- pivot 撤回の memory / DESIGN.ja.md 更新

#### 共通評価基準（§3.8 を使う）

各段階での「成立 / 成立しない」判定は **§3.8 の 5 観点（保存管理 / 明示 edge 探索 / stale edge 除去 / 暗黙的波及先発見 / GRAG 構築支援）** に基づく。spike で実証した上で「成立しない」と判定できた時のみ次段階に降りる（spike なしで「面倒そう」「不確実そう」では降りない、これが私の局所最適化バイアスを抑える歯止め）。
...
### 観点 B: Phase 1 entry plan — **済（2026-04-28、案 B 第一選択 + fallback ladder）**

ユーザー決定（2026-04-28）に基づき、Phase 1 は **案 B（GRAG 本来の使い方）から spike 開始**。実証で成立しないと判定された場合のみ案 C に降り、それでも成立しないなら GRAG 撤回。詳細は §3.9。

最初の spike は:

- **spike 05**: `CodexCLIAdapter(LLM)` 実装（subprocess wrapper、最小 method 一覧、structured output 対応）
...
これらが成立すれば案 B 確定、§1.1〜§1.9 仮分担マトリクスを案 B 前提で再評価。

### 観点 C: 確定設計含意（R1-R5）の段階別適用 — 案 B 確定後

- 案 B 確定なら R1 / R2 / R3 / R4 / R5 すべて適用、DESIGN.ja.md §1.4 / §1.9 に正式反映
- 案 C に降りた場合は R2 (kg_extractors) / R3 (load_index_from_storage) を再評価、R1 / R4 / R5 はそのまま適用
- GRAG 撤回まで降りた場合のみ R1 / R2 / R3 を撤回し、DESIGN.ja.md §1.4 を「Python + 自前 canonical graph」に書き換え
