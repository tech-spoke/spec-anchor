...
`related_sections` の `possible_conflict` は完全廃止する。実装順は、まず SpecClaim 経路 (Phase 1-3) を実装し、既知 conflict fixture が SpecClaim 経路で拾えることを Phase 4 で確認した後、Phase 5 で Related Sections schema / prompt / output、`spec_anchor/core.py` の routing、`spec_anchor/conflict_review.py` の relation_hint 整合 filter、関連 tests、`doc/EXTERNAL_DESIGN.ja.md` / `doc/DESIGN.ja.md` の `possible_conflict` 契約をすべて削除する。production config に legacy mode は残さない。
...
| `.spec-anchor/state/core_progress.json` | `/spec-core` 実行ごとの stage diagnostics。`claim_retrieval` の truncation、`spec_claims` の validation failure、`conflict_candidate_triage` の `send_to_review` 件数を記録する | 実装完了報告、監査 |
...
`candidate_uid` は sorted `claim_uid` pair から作る。route は `candidate_uid` に入れない。
...
`route` は単一 string とする。初期実装では `claim_retrieval_llm_triage` だけが有効値である。複数 retrieval channel (dense / sparse_key / conflict_probe) から同じ pair が候補化された事実は `retrieval_sources[]` に記録し、`route` には含めない。

`triage` は `claim_retrieval_llm_triage` route の candidate で必須とする。`triage = null` の candidate を `.spec-anchor/context/conflict_candidate_pairs.jsonl` に保存してはいけない。Conflict Review に送れるのは、`triage.send_to_review = true` の candidate だけである。
...
claim pair の dedup は `claim_uid` の sorted tuple で行う。`left_*` と `right_*` はこの sorted tuple に従う。同一 pair が複数 retrieval channel (dense / sparse_key / conflict_probe) から来た場合、`retrieval_sources[]` に追加して集約し、reason を潰さない。
...
Phase 5 (旧経路削除) では、Related Sections の schema / prompt / output から `possible_conflict` field を完全削除する。`spec_anchor/core.py` の `possible_conflict=true` 由来 routing と、`spec_anchor/conflict_review.py` の relation_hint 整合 filter (= `possible_conflict` を前提に relation_hint で再 filter する処理) も同じ commit で削除する。production config に legacy mode key を追加しない。旧 revision の `/spec-core` 保持ファイルとの一回限り recall 比較は `doc/性能測定/spec_claim_migration_comparison.md` の手順に従う。
...
- 保存する内容: SpecClaim 集合指紋、claim uid 集合、claim hash 集合、retrieval hash 集合、candidate uid 集合、claim retrieval 設定指紋、LLM triage の prompt version、schema version、model、effort、candidate 上限、`truncated_candidate_sources`、`truncated_pair_count`
...
- 不一致時の fallback 条件: SpecClaim 集合、claim retrieval 設定、triage prompt version、schema version、model、effort、candidate 上限のいずれかが変わった場合、必要な claim pair の retrieval と LLM triage を再実行する。

`.spec-anchor/state/core_progress.json`:

- 保存する内容: `/spec-core` 実行ごとの stage status、elapsed、action、diagnostics。`claim_retrieval` の truncation は `stages.claim_retrieval.diagnostics` に記録する。SpecClaim validation failure は `stages.spec_claims.diagnostics` に記録する。`conflict_candidate_triage` の `send_to_review` 件数と `triage = null` 件数は `stages.conflict_candidate_triage.diagnostics` に記録する。
...
- `claim_retrieval_llm_triage` route 以外の triage cache 経路は持たない。新規 route を追加する場合は、本設計書の §6 と §7.3 を先に更新する。
...
- retrieval_sources 別内訳: dense / sparse_key / conflict_probe (任意、retrieval recall 分析時)
...
- 候補 pair に `retrieval_sources[]` を保存する。同一 pair が複数 channel から来た場合は集約する。

### Phase 3: LLM triage 追加

- 少数 claim pair だけに LLM triage を実行する。
- 出力は `send_to_review`、`reason`、`confidence` に限定する。
- conflict 確定や人間判断 item 作成は行わない。
- `triage = null` の record は `.spec-anchor/context/conflict_candidate_pairs.jsonl` に保存せず、診断目的でも Conflict Review に送らない。
- SpecClaim 経路で `send_to_review = true` の candidate が Conflict Review に渡る test を追加する。

### Phase 4: 既知 fixture 検証

- 既知 conflict fixture が SpecClaim 経路 (Phase 1-3) で `send_to_review = true` として Conflict Review に届くことを確認する。
- 実 Codex / Claude CLI、Qdrant、FlagEmbedding BGE-M3 を使う `/spec-core` 実行で確認する。
- 任意のサブステップとして、削除前 revision の `/spec-core` 保持ファイルを退避してから、新 SpecClaim 経路の結果と一回限り比較する。手順と結果は `doc/性能測定/spec_claim_migration_comparison.md` (Phase 5 着手前に新規作成) に記録する。
- 比較が必要な場合、本作業のためだけに production config を変更しない。比較は revision 切替と保持ファイル退避で行う。

### Phase 5: `possible_conflict` 完全削除

Phase 4 で SpecClaim 経路の recall が許容範囲であることを確認した後、次をすべて削除する。

- `spec_anchor/related_sections.py` の schema、prompt、output から `possible_conflict` field
- `spec_anchor/llm_provider.py` の `possible_conflict` schema 定義
- `spec_anchor/core.py` の `possible_conflict=true` 由来 conflict review routing
- `spec_anchor/conflict_review.py` の `possible_conflict` 前提の relation_hint 整合 filter (現状 L367-421 付近)
- `[limits].conflict_pair_max_per_section` 設定 key と関連処理
- `tests/` 配下の `possible_conflict` 経路 routing test
- `doc/EXTERNAL_DESIGN.ja.md` / `doc/DESIGN.ja.md` の `possible_conflict` 契約記述

検証として、次の test を追加または更新する。

- Related Sections output に `possible_conflict` field が存在しないこと
- Related Sections prompt が conflict 判定を要求しないこと
- `spec_anchor/core.py` が Related Sections output から `possible_conflict` を読まないこと
- SpecClaim candidate pair の `send_to_review = true` だけが Conflict Review に渡ること

Phase 5 完了後、次の grep を実行し、想定 hit 例外リスト以外で 0 件であることを確認する。

```text
git grep -nE "possible_conflict" spec_anchor tests
git grep -nE "legacy_related_possible_conflict" spec_anchor tests doc
git grep -nE "legacy_possible_conflict_mode" spec_anchor tests doc
git grep -nE "conflict_pair_max_per_section" spec_anchor tests doc
```

想定 hit 例外:

- `doc/性能測定/spec_claim_migration_comparison.md` の比較記録 (Phase 4 で作成した一回限り計測ドキュメント)
- 本ドキュメント `doc/SPEC_CLAIM_CONFLICT_CANDIDATE_DESIGN.ja.md` §18 廃止・置換された設計判断 (歴史記録)
- 本ドキュメント §12 と §14.2 の本検証コマンド自身

それ以外の hit がある場合は、Phase 5 削除漏れとして再対応する。本ドキュメント全体の archive 化または `possible_conflict` 言及の削除は、Phase 5 とは別の整理 task とする。
...
- 単一 route: `conflict_candidate_pairs.jsonl` の各 candidate は単一 `route` を持つ。複数 retrieval channel 由来の事実は `retrieval_sources[]` に保存する。`route` に legacy 値や複数値を入れない。(`SCD-032`)
- Conflict Review 入力境界: Conflict Review は SpecClaim pair、`evidence`、`triage` を入力にする。Related Sections の `relation_hint` や `possible_conflict` を Conflict Review の入力にしない。(`SCD-033`)
...
Phase 1 (`spec_claims.jsonl` 追加) の完了条件:
...
Phase 2 (Claim Retrieval 追加) の完了条件:
...
Phase 4 (既知 fixture 検証) の完了条件:

- 実 Codex / Claude CLI、Qdrant、FlagEmbedding BGE-M3 を含む `/spec-core` 実行で、既知 conflict fixture が SpecClaim 経路から `send_to_review = true` として Conflict Review に届くことを確認する。
- 任意で旧 revision の `/spec-core` 保持ファイルとの一回限り recall 比較を実施した場合、手順と結果を `doc/性能測定/spec_claim_migration_comparison.md` に記録する。
- 実 Codex / Claude CLI、Qdrant、FlagEmbedding BGE-M3 を含む確認が未実行の場合、未実行範囲を完了扱いせず、残 TODO として残す。
- production config に legacy mode key を追加していないことを確認する。

Phase 5 (`possible_conflict` 完全削除) の完了条件:

- `spec_anchor/related_sections.py` の schema、prompt、output から `possible_conflict` field を削除した。
- `spec_anchor/llm_provider.py` の `possible_conflict` schema 定義を削除した。
- `spec_anchor/core.py` の `possible_conflict=true` 由来 conflict review routing を削除した。
- `spec_anchor/conflict_review.py` の relation_hint 整合 filter を削除した。
- `[limits].conflict_pair_max_per_section` 設定 key と関連処理を削除した。
- `tests/` 配下の `possible_conflict` 経路 routing test を削除し、Related Sections output に `possible_conflict` がないこと等を確認する test を追加した。
- `doc/EXTERNAL_DESIGN.ja.md` / `doc/DESIGN.ja.md` の `possible_conflict` 契約記述を削除した。
- `git grep -nE "possible_conflict|legacy_related_possible_conflict|legacy_possible_conflict_mode|conflict_pair_max_per_section" spec_anchor tests doc` の hit が、Phase 5 の想定 hit 例外 (本ドキュメント §12 末尾参照) 以外で 0 件である。
...
| --- | --- | --- | --- |
| §2.7 Related Sections | Related Sections に `possible_conflict: true` を立て、最終判定を Conflict Review Item へ委ねる | Related Sections の schema / prompt / output から `possible_conflict` field を完全削除する。Related Sections は Agentic Search 補助に限定し、Conflict Candidate Detection の主経路にも legacy 経路にもしない | Phase 5 |
| §4.1 保持物の物理配置 | `.spec-anchor/context/section_metadata.jsonl`、Related Sections、Conflict Review Items が中心 | `.spec-anchor/context/spec_claims.jsonl`、必要な場合の `.spec-anchor/context/spec_claim_retrieval_index.jsonl`、`.spec-anchor/context/conflict_candidate_pairs.jsonl`、`.spec-anchor/state/spec_claims_state.json`、`.spec-anchor/state/conflict_candidate_pairs_state.json` を追加する | Phase 1 -> Phase 3 |
| §7.4 `/spec-core` 出力の `potential_conflicts` | Related Sections の `conflicts_with` 由来の conflict 候補を保持する | `conflict_candidate_pairs.jsonl` と LLM triage の `send_to_review = true` 集合に意味を寄せる。旧 `potential_conflicts` field は Phase 5 で完全削除する | Phase 3 -> Phase 5 |
| §2.8 Conflict Review Item / §7.4 Conflict 判定 | `relation_hint = conflicts_with` 相当または衝突しやすい語を共有する high-risk pair を conflict 判定 stage へ送る | SpecClaim retrieval と LLM triage に差し替える。Related Sections 由来 pair は Conflict Review に送らない | Phase 2 -> Phase 5 |
| §10.2 `[llm.stage_routing]` 許可 stage key | `section_metadata`、`related_sections`、`conflict_review`、`chapter_key_anchor` | `spec_claims`、`claim_retrieval`、`conflict_candidate_triage` の扱いを追加する。LLM を呼ばない stage は provider routing 対象外であることを明記する | Phase 1 -> Phase 3 |
| §10.2 `[limits]` table と config 例 | `[limits].conflict_pair_max_per_section = 8` が conflict 判定 stage の上限 | `[limits].conflict_pair_max_per_section` を完全削除する。新経路の処理量制御は `per_claim_top_k`、`per_section_top_k`、`per_target_top_k`、`global_candidate_top_k`、`triage_max_pairs` に移す | Phase 5 |

`doc/DESIGN.ja.md` へ反映するときの書き換え対象一覧:

| 箇所 | 現在の記述 | 書き換え方針 | Phase |
| --- | --- | --- | --- |
| §1 implementation tracker (Phase E 完了 task) | `possible_conflict` フラグを立てる Phase E が完了 task として記録されている | Phase 5 で `possible_conflict` 完全削除に対応する task に書き換える。Phase E 完了 task はそのまま残してよいが、後続の Phase 5 削除 task を追記する | Phase 5 |
| §5.7 conflict 判定 stage 対象 | `possible_conflict = true` フラグが立った pair と高リスク pair が対象 | SpecClaim pair の `triage.send_to_review = true` に置き換える。Related Sections の `possible_conflict` と `conflict_pair_max_per_section` への言及を削除する | Phase 3 -> Phase 5 |
| §5.8 Conflict 判定 / `possible_conflict` schema | `possible_conflict: true` を referral signal とする説明、`possible_conflict     # bool、Conflict Review pipeline への referral signal` の schema 例 | `conflict_candidate_pairs.jsonl` の `triage.send_to_review = true` を referral signal にする説明に置き換える。`possible_conflict` field の schema 例を削除する | Phase 5 |
| §5.8 末尾 `relation_hint = conflicts_with` 除外説明 | LLM が矛盾兆候を見つけた場合 `possible_conflict: true` フラグだけ立て、Conflict Review pipeline が独立判定 | Related Sections は conflict 判定を持たないと書き換える。Conflict Review pipeline は SpecClaim pair の `triage.send_to_review = true` を入力にする | Phase 5 |
| Conflict Review pipeline 入力 | `possible_conflict: true` pair を CLI が Conflict Review に渡す | SpecClaim pair、`evidence`、`triage` を入力にする。`relation_hint` 整合 filter (`conflict_review.py` L367-421) も Phase 5 で削除する | Phase 5 |
...
Conflict Review の入力は SpecClaim pair、evidence、triage result である。
Related Sections の relation_hint や possible_conflict を入力にしない。
...
Related Sections は conflict 判定を持たない。
Related Sections の出力に conflict referral signal を含めない。
...
| SCD-013 | `candidate_uid` は sorted claim pair から作り、route は単一 `route`、候補化元は `retrieval_sources[]` に分ける | 同じ claim pair が複数 retrieval channel から来ても同一 candidate として扱うため |
...
| SCD-032 | `possible_conflict` を完全廃止する。legacy mode を production config に残さない | SpecClaim 経路完成後の commit で削除し、recall 比較は一回限りの計測タスクとして `doc/性能測定/spec_claim_migration_comparison.md` に分離する。production に legacy 名残を残さないため |
| SCD-033 | Conflict Review の入力は SpecClaim pair / evidence / triage result とする | Related Sections の `relation_hint` や `possible_conflict` を Conflict Review に再混入させないため |

## 18. 廃止・置換された設計判断

次の SCD は、`possible_conflict` 完全廃止判断 (SCD-032 / SCD-033) に伴い、active 採用判断から外した。番号は欠番のまま残し、renumber しない。

| ID | 元の判断 | 廃止理由 | 置換先 |
| --- | --- | --- | --- |
| SCD-004 | `possible_conflict` は旧経路 signal に降格する | legacy mode を production config に残さない判断に変更 | SCD-032 |
| SCD-014 | `legacy_related_possible_conflict` は通常 diagnostics only とする | legacy 経路を production 機能として保持しない判断に変更 | SCD-032 |
| SCD-024 | legacy 比較 mode は `disabled / diagnostics_only / compare_with_triage` とする | legacy mode を完全廃止し、recall 比較は一回限り計測タスクに分離 | SCD-032 |
| SCD-030 | `legacy_related_possible_conflict` diagnostics は `.spec-anchor/state/core_progress.json` と CoreResult diagnostics に記録する | legacy route の diagnostics を持たない判断に変更 | SCD-032 |
