...
| ID | 区分 | 重大度 | 状態 | 扱い |
| --- | --- | --- | --- | --- |
| CDX-001 | 手抜き / 指示違反 | 中 | 開放 | `prior_state` dead 引数。「開放中」に詳細 |
| CDX-002 | 設計バグ (実証済み) | 最高 | 開放 | apply 前 / 後 fingerprint 乖離。real LLM Run 2 で 1 section 変更にも全 4 section re-embed を実測。「開放中」に詳細 |
| CDX-003 | 名称誤誘導 | 中 | 開放 | partial 成功時の `action="fallback_rebuilt"`。「開放中」に詳細 |
| CDX-004 | trade-off 未文書化 | 中 | 開放 | 空 list `sections_to_delete=[]` 渡しで B-3a stale delete が黙って無効化。「開放中」に詳細 |
| CDX-005 | 越権 (Agent が rule book を拡張) | 低 | 開放 | AGENTS.md +2 行は Codex prompt の指示範囲外。「開放中」に判断 task として残す |
| CDX-006 | 検証穴 | 高 | 開放 | apply 前後 fingerprint 乖離を再現する test なし。CDX-002 修正と同時に追加。「開放中」に詳細 |
| CDX-007 | 名称誤誘導 (軽微) | 低 | 開放 | partial run で `section_count: 51` が `sections_upserted_count: 1` と並ぶ。「開放中」に詳細 |

## 開放中

優先順位 (上から順に着手):

1. **CDX-002**: real LLM で B-3b partial 化が機能するよう、fingerprint 計算対象から `related_sections` を除外する (B 案)。B-3b の本来合格基準 (1 section 変更で `sections_upserted_count == 1` になる) は、CDX-002 修正後に再評価する
2. **CDX-006**: apply 前後 fingerprint 乖離を再現する end-to-end test を追加 (CDX-002 修正と同時)
3. **CDX-003 / CDX-001 / CDX-007**: action 名修正、dead 引数削除、diagnostics field 名整理 (CDX-002 と同じ commit で吸収可能)
4. **CDX-004**: 空 list `sections_to_delete=[]` 渡し時の trade-off を文書化または設計再考
5. **CDX-005**: AGENTS.md の Agent 由来追記 +2 行を revert するか保持するかを人間が判断
6. **B-3b**: CDX-001〜CDX-007 全件解消後に、B-3b 本来の合格条件 (50 section fixture で stage wall time + 5 秒以内、または改訂された比率ベース基準) を再評価
7. **B-4**: `--verify-index` 明示検証 flag (B-3b と並行可)
8. **B-5**: section_metadata / related_typing cache の現状確認 (調査のみ。B-3 と独立に実施可)
9. **B-6**: 大規模 spec での Qdrant scroll 計測 (主犯ではないため低優先)
10. **AUD-006 / AUD-007**: Chapter Anchors / Related Sections fallback の freshness 反映 (外部契約上の freshness 表現確定が必要、判断要)
...
- 検証条件 A〜D すべて満たす
- 本項を `doc/TODO.ja.md` から削除し、`doc/監査/IMPLEMENTATION_DISPOSITION_2026-05-13.ja.md` (または後続の disposition 文書) に CDX-002 disposition を追記
...
### CDX-004: `sections_to_delete=[]` で B-3a stale delete が黙って無効化される件の文書化または設計再考
...
### CDX-005: AGENTS.md +2 行の Agent 由来追記を保持するか revert するかの判断
...
- 判断結果が AGENTS.md に反映されている
- 判断理由が `doc/監査/IMPLEMENTATION_DISPOSITION_2026-05-13.ja.md` または後続 disposition 文書に CDX-005 として記録される
...
### CDX-007: `section_count` field の誤誘導 (軽微)
