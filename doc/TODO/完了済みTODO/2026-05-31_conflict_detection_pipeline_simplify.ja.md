...
**最終更新**: 2026-05-31
**ステータス**: 実装・実機検証完了（#1〜#4・#6 完了、#5 実装完了、#7 計測完了、**#8 auto 解消 → item 削除 実装・テスト・doc 完了**）。**production E2E 取り直し（現コード = batch+budget-first+#8 後）を 2026-05-31 実施・5 ステップ全 PASS**（検出 / 人間 dismiss / reopen / auto 解消→削除 / 修正失敗→pending 維持。証跡 `doc/e2eテスト/evidence/2026-05-31-conflict-auto-resolve-delete/`（FINDINGS + CoreResult JSON + 生 stdout）、計測は `doc/性能測定/METRICS.md` 第13回「production E2E 取り直し」節）。**CLOSE（2026-05-31 オーナー承認）**: production E2E 取り直し証跡（FINDINGS）をレビューし close 判断。残っていた非ブロッキング follow-up（#9-s10 evidence 再生成 / `min_dense_score` 実 Qdrant キャリブレーション）は本 close 時点で非ブロッキングと判断し未実施。必要になれば別 TODO 化する（min_dense_score は section > 13 の retrieval_cap mode でのみ効くため、現状の小規模サンプルでは未行使）。コミット列 2285c49→ce69820→e44a558→73ca0c7→3aebc46→feba231→992eca5→21c6384→96b5822→…→d14bdaf（#8 + budget close）
