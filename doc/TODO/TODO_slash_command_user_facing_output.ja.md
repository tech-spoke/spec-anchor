...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #7-s01 | doc lint: §8.5 本文に内部 field 名 / enum 値 (`should_stop` / `blocking_reasons` / `dirty_or_stale_source` 等) が含まれない (grep) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #7-s02 | doc lint: §8.5 のカテゴリマップ表 (6 + ◇ + ✕) が #1 の最終マップと一致、#2/#3/#4 のテンプレ語彙が §8.5 と整合 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #7-s03 | doc lint: §8.5 のリトライポリシー記述と #6 のテンプレ手順が整合 (1 回試行 + 失敗時 1 リトライ + ⑥ 落とし) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #10-s01 | `spec-anchor-setup-project` 直後の `.claude/commands/spec-inject.md` が `spec_anchor/templates/.claude/commands/spec-inject.md` と一致 (file diff) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #10-s02 | `spec-anchor-setup-project` 直後の `.claude/commands/spec-realign.md` がテンプレ版と一致 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #10-s03 | `spec-anchor-setup-project` 直後の `.claude/commands/spec-core.md` がテンプレ版と一致 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #10-s04 | `.codex/skills/spec-anchor/SKILL.md` install 後の語彙整理が最新と一致 (Codex 不使用のため file diff のみ。実行検証なし) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
| #10 | #10-s01 | setup-project 直後の `.claude/commands/spec-inject.md` がテンプレ版と一致 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #10 | #10-s02 | setup-project 直後の `.claude/commands/spec-realign.md` がテンプレ版と一致 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #10 | #10-s03 | setup-project 直後の `.claude/commands/spec-core.md` がテンプレ版と一致 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #10 | #10-s04 | `.codex/skills/spec-anchor/SKILL.md` install 後の語彙整理が最新と一致 (file diff のみ) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
