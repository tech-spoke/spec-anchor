...
|---|---|---|---|---|---|---|
| 1 | T-stop-category-mapping | CLI 停止状態を 6 カテゴリ + ◇ + ✕ へ確定マップ | 未着手 | カテゴリ表確定、外部設計書 §8.5 反映準備 | 2026-05-29 | — |
| 2 | T-stop-output-template-3files | 3 コマンドテンプレに停止時出力フォーマットと禁止用語リストを追加 | 未着手 | spec-inject.md / spec-realign.md / spec-core.md の編集 | 2026-05-29 | — |
| 3 | T-pending-conflict-body-expansion | pending Conflict Review Item の本文展開を必須化 | 未着手 | 内部 field 名 label を人間向け見出しへ置換、テスト追加 | 2026-05-29 | — |
| 4 | T-needs-agent-answer-hide | `needs_agent_answer` をユーザーに見せず Agent 自動再実行で吸収 | 未着手 | `/spec-realign` テンプレに自動再実行経路を契約化 | 2026-05-29 | — |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #2-s01 | ① 初期設定未完了 (config.toml 不在) を代表コマンド /spec-core で表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2-s02 | ② 外部サービス必要 (Qdrant 接続失敗) を代表コマンド /spec-inject で表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2-s03 | ③ 保持物更新必要 (dirty_or_stale_source) を代表コマンド /spec-inject で表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2-s04 | ④ 保持物の更新中・待機 (watcher_running) を代表コマンド /spec-inject で表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2-s05 | ⑥ ツール側のエラー (想定外 Python 例外) を代表コマンド /spec-core で表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2-s06 | ◇ 情報通知 (degraded_optional_artifact 単独) で続行可能を確認 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2-s07 | 3 コマンド一貫性: ③ dirty_or_stale_source 同条件で /spec-core / /spec-inject / /spec-realign が同テンプレ表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2-s08 | 禁止用語横断チェック: #2-s01〜#2-s07 出力に内部 field 名 / enum 値 / パイプライン段階名が含まれない |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #3-s01 | pending conflict 1 件 / 単一 claim pair (主張 A / B 形式) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #3-s02 | pending conflict 1 件 / 3 件以上の claims (主張 A / B / C 連続形式) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #3-s03 | pending conflict 複数件 (2 件以上、見出し連番 1./2./...) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #3-s04 | pending conflict + dirty_or_stale_source 混在 (③ と ⑤ の両方表示) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #3-s05 | 3 コマンド一貫性: pending conflict が /spec-core / /spec-inject / /spec-realign で同フォーマット表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #4-s01 | /spec-realign 答案なし呼び出し → Agent 自動再実行 → 整形済み RealignResult。出力に `needs_agent_answer` / `answer candidate` / `stop_reason` の語が含まれない |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #4-s02 | Agent 内部での自動再実行が利用者に見えない (再実行の進捗ログや「次は答案を作ります」等のメタ説明が含まれない) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
|---|---|---|---|---|---|---|---|
| #2 | #2-s01 | ① 初期設定未完了 (config.toml 不在) を代表コマンド /spec-core で表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2 | #2-s02 | ② 外部サービス必要 (Qdrant 接続失敗) を代表コマンド /spec-inject で表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2 | #2-s03 | ③ 保持物更新必要 (dirty_or_stale_source) を代表コマンド /spec-inject で表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2 | #2-s04 | ④ 保持物の更新中・待機 (watcher_running) を代表コマンド /spec-inject で表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2 | #2-s05 | ⑥ ツール側のエラー (想定外 Python 例外) を代表コマンド /spec-core で表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2 | #2-s06 | ◇ 情報通知 (degraded_optional_artifact 単独) で続行可能を確認 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2 | #2-s07 | 3 コマンド一貫性: ③ dirty_or_stale_source 同条件で /spec-core / /spec-inject / /spec-realign が同テンプレ表示 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #2 | #2-s08 | 禁止用語横断チェック: #2-s01〜#2-s07 出力に内部 field 名 / enum 値 / パイプライン段階名が含まれない |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #3 | #3-s01 | pending conflict 1 件 / 単一 claim pair (主張 A / B 形式) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #3 | #3-s02 | pending conflict 1 件 / 3 件以上の claims (主張 A / B / C 連続形式) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #3 | #3-s03 | pending conflict 複数件 (2 件以上、見出し連番) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #3 | #3-s04 | pending conflict + dirty_or_stale_source 混在 (③ と ⑤ の両方表示) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #3 | #3-s05 | 3 コマンド一貫性: pending conflict が /spec-core / /spec-inject / /spec-realign で同フォーマット |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #4 | #4-s01 | /spec-realign 答案なし呼び出し → Agent 自動再実行 → 整形済み RealignResult。出力に `needs_agent_answer` / `answer candidate` / `stop_reason` の語が含まれない |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #4 | #4-s02 | Agent 内部での自動再実行が利用者に見えない (再実行進捗ログやメタ説明が含まれない) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
