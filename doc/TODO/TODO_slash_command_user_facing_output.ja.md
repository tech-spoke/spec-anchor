...
| 12 | T-explicit-free-agentic-search | Agent の能動的 Agentic Search 余地を `/spec-inject` / `/spec-realign` テンプレと外部設計書 §8.3 で明文化 | 未着手 | テンプレ + §8.3 改訂 + #10 連動でテンプレ同期 | 2026-05-29 | — |
| 13 | T-japanese-only-user-facing | 利用者向け本文を日本語に統一 (英語自然文を Agent が翻訳) | 未着手 | 3 テンプレに翻訳契約追記 + #2 禁止用語リスト拡張 + #3-s01〜s05 / #8-s04 計 7 snapshot 再生成 + #10 連動 | 2026-05-29 | — |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #2-s01 | ① 初期設定未完了 (config.toml 不在) を代表コマンド /spec-core で表示 | tests/e2e/snapshots/#2-s01_stop_setup_missing_config_spec_core.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2-s02 | ② 外部サービス必要 (Qdrant 接続失敗) を代表コマンド /spec-inject で表示 | tests/e2e/snapshots/#2-s02_stop_qdrant_unavailable_spec_inject.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2-s03 | ③ 保持物更新必要 (dirty_or_stale_source) を代表コマンド /spec-inject で表示 | tests/e2e/snapshots/#2-s03_stop_dirty_source_spec_inject.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2-s04 | ④ 保持物の更新中・待機 (watcher_running) を代表コマンド /spec-inject で表示 | tests/e2e/snapshots/#2-s04_stop_watcher_running_spec_inject.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2-s05 | ⑥ ツール側のエラー (想定外 Python 例外) を代表コマンド /spec-core で表示 | tests/e2e/snapshots/#2-s05_stop_tool_error_spec_core.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2-s06 | ◇ 情報通知 (degraded_optional_artifact 単独) で続行可能を確認 | tests/e2e/snapshots/#2-s06_info_degraded_optional_continue.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2-s07 | 3 コマンド一貫性: ③ dirty_or_stale_source 同条件で /spec-core / /spec-inject / /spec-realign が同テンプレ表示 | tests/e2e/snapshots/#2-s07_stop_dirty_three_commands_consistency.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2-s08 | 禁止用語横断チェック: #2-s01〜#2-s07 出力に内部 field 名 / enum 値 / パイプライン段階名が含まれない | tests/e2e/snapshots/#2-s08_forbidden_terms_cross_check.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #3-s01 | pending conflict 1 件 / 単一 claim pair (主張 A / B 形式) | tests/e2e/snapshots/#3-s01_pending_conflict_single_pair.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #3-s02 | pending conflict 1 件 / 3 件以上の claims (主張 A / B / C 連続形式) | tests/e2e/snapshots/#3-s02_pending_conflict_three_claims.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #3-s03 | pending conflict 複数件 (2 件以上、見出し連番 1./2./...) | tests/e2e/snapshots/#3-s03_pending_conflict_multiple.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #3-s04 | pending conflict + dirty_or_stale_source 混在 (③ と ⑤ の両方表示) | tests/e2e/snapshots/#3-s04_pending_conflict_with_dirty_source.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #3-s05 | 3 コマンド一貫性: pending conflict が /spec-core / /spec-inject / /spec-realign で同フォーマット表示 | tests/e2e/snapshots/#3-s05_pending_conflict_three_commands.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #4-s01 | /spec-realign 答案なし呼び出し → Agent 自動再実行 → 整形済み RealignResult。出力に `needs_agent_answer` / `answer candidate` / `stop_reason` の語が含まれない | tests/e2e/snapshots/#4-s01_realign_auto_rerun_clean.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #4-s02 | Agent 内部での自動再実行が利用者に見えない (再実行の進捗ログや「次は答案を作ります」等のメタ説明が含まれない) | tests/e2e/snapshots/#4-s02_realign_auto_rerun_no_meta.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #5-s01 | 不正答案 (final 区分なし) → CLI が `error.code="missing_final_section"` / `error.field` / `error.expected` を含む JSON 返却 | snapshots/#5-s01_realign_error_missing_final_section.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #5-s02 | 不正答案 (constraints の evidence_origin 不正値) → CLI が `error.code="invalid_evidence_origin"` / `error.field="constraints[N].evidence_origin"` を返す | snapshots/#5-s02_realign_error_invalid_evidence_origin.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #5-s03 | 不正答案 (support_refs 型違反) → CLI が `error.code="invalid_support_refs_type"` を返す | snapshots/#5-s03_realign_error_invalid_support_refs_type.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #5-s04 | 正常答案 → error block が含まれない RealignResult 返却 | snapshots/#5-s04_realign_valid_no_error_block.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #6-s01 | 構造化失敗 (1 回目) → CLI error 詳細を Agent が読んで修正 → 2 回目で成功 → 整形済み RealignResult | snapshots/#6-s01_retry_success_after_fix.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #6-s02 | 構造化失敗 (1 回目 + リトライ) → 再失敗 → ⑥ ツール側エラー表示。出力に「最後の答案 JSON」と「CLI error 詳細」が併記される | snapshots/#6-s02_retry_exhausted_tool_error.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #7-s01 | doc lint: §8.5 本文に内部 field 名 / enum 値 (`should_stop` / `blocking_reasons` / `dirty_or_stale_source` 等) が含まれない (grep) | snapshots/#7-s01_design_no_internal_field_names.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #7-s02 | doc lint: §8.5 のカテゴリマップ表 (6 + ◇ + ✕) が #1 の最終マップと一致、#2/#3/#4 のテンプレ語彙が §8.5 と整合 | snapshots/#7-s02_design_category_map_consistency.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #7-s03 | doc lint: §8.5 のリトライポリシー記述と #6 のテンプレ手順が整合 (1 回試行 + 失敗時 1 リトライ + ⑥ 落とし) | snapshots/#7-s03_design_retry_policy_consistency.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #9-s01 | `spec-anchor core` stdout が valid JSON 単体 (`json.loads(stdout)` 成功) | tests/e2e/snapshots/#9-s01_core_stdout_single_json.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #9-s02 | `spec-anchor inject-search "query"` stdout が valid JSON 単体 | tests/e2e/snapshots/#9-s02_inject_search_stdout_single_json.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #9-s03 | `spec-anchor inject-section <id>` stdout が valid JSON 単体 | tests/e2e/snapshots/#9-s03_inject_section_stdout_single_json.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #9-s04 | `spec-anchor inject-chapters` stdout が valid JSON 単体 | tests/e2e/snapshots/#9-s04_inject_chapters_stdout_single_json.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #9-s05 | `spec-anchor inject-purpose` stdout が valid JSON 単体 | tests/e2e/snapshots/#9-s05_inject_purpose_stdout_single_json.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #9-s06 | `spec-anchor inject-conflicts` stdout が valid JSON 単体 | tests/e2e/snapshots/#9-s06_inject_conflicts_stdout_single_json.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #10-s01 | `spec-anchor-setup-project` 直後の `.claude/commands/spec-inject.md` が `spec_anchor/templates/.claude/commands/spec-inject.md` と一致 (file diff) | snapshots/#10-s01_template_spec_inject_matches_project.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #10-s02 | `spec-anchor-setup-project` 直後の `.claude/commands/spec-realign.md` がテンプレ版と一致 | snapshots/#10-s02_template_spec_realign_matches_project.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #10-s03 | `spec-anchor-setup-project` 直後の `.claude/commands/spec-core.md` がテンプレ版と一致 | snapshots/#10-s03_template_spec_core_matches_project.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #10-s04 | `.codex/skills/spec-anchor/SKILL.md` install 後の語彙整理が最新と一致 (Codex 不使用のため file diff のみ。実行検証なし) | snapshots/#10-s04_codex_skill_vocabulary_aligned.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
|---|---|---|---|---|---|---|---|
| #2 | #2-s01 | ① 初期設定未完了 (config.toml 不在) を代表コマンド /spec-core で表示 | snapshots/#2-s01_stop_setup_missing_config_spec_core.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2 | #2-s02 | ② 外部サービス必要 (Qdrant 接続失敗) を代表コマンド /spec-inject で表示 | snapshots/#2-s02_stop_qdrant_unavailable_spec_inject.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2 | #2-s03 | ③ 保持物更新必要 (dirty_or_stale_source) を代表コマンド /spec-inject で表示 | snapshots/#2-s03_stop_dirty_source_spec_inject.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2 | #2-s04 | ④ 保持物の更新中・待機 (watcher_running) を代表コマンド /spec-inject で表示 | snapshots/#2-s04_stop_watcher_running_spec_inject.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2 | #2-s05 | ⑥ ツール側のエラー (想定外 Python 例外) を代表コマンド /spec-core で表示 | snapshots/#2-s05_stop_tool_error_spec_core.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2 | #2-s06 | ◇ 情報通知 (degraded_optional_artifact 単独) で続行可能を確認 | snapshots/#2-s06_info_degraded_optional_continue.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2 | #2-s07 | 3 コマンド一貫性: ③ dirty_or_stale_source 同条件で /spec-core / /spec-inject / /spec-realign が同テンプレ表示 | snapshots/#2-s07_stop_dirty_three_commands_consistency.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #2 | #2-s08 | 禁止用語横断チェック: #2-s01〜#2-s07 出力に内部 field 名 / enum 値 / パイプライン段階名が含まれない | snapshots/#2-s08_forbidden_terms_cross_check.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #3 | #3-s01 | pending conflict 1 件 / 単一 claim pair (主張 A / B 形式) | snapshots/#3-s01_pending_conflict_single_pair.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #3 | #3-s02 | pending conflict 1 件 / 3 件以上の claims (主張 A / B / C 連続形式) | snapshots/#3-s02_pending_conflict_three_claims.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #3 | #3-s03 | pending conflict 複数件 (2 件以上、見出し連番) | snapshots/#3-s03_pending_conflict_multiple.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #3 | #3-s04 | pending conflict + dirty_or_stale_source 混在 (③ と ⑤ の両方表示) | snapshots/#3-s04_pending_conflict_with_dirty_source.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #3 | #3-s05 | 3 コマンド一貫性: pending conflict が /spec-core / /spec-inject / /spec-realign で同フォーマット | snapshots/#3-s05_pending_conflict_three_commands.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #4 | #4-s01 | /spec-realign 答案なし呼び出し → Agent 自動再実行 → 整形済み RealignResult。出力に `needs_agent_answer` / `answer candidate` / `stop_reason` の語が含まれない | snapshots/#4-s01_realign_auto_rerun_clean.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #4 | #4-s02 | Agent 内部での自動再実行が利用者に見えない (再実行進捗ログやメタ説明が含まれない) | snapshots/#4-s02_realign_auto_rerun_no_meta.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #5 | #5-s01 | 不正答案 (final 区分なし) → CLI が `error.code="missing_final_section"` / `error.field` / `error.expected` を返す | snapshots/#5-s01_realign_error_missing_final_section.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #5 | #5-s02 | 不正答案 (constraints の evidence_origin 不正値) → CLI が `error.code="invalid_evidence_origin"` / `error.field` を返す | snapshots/#5-s02_realign_error_invalid_evidence_origin.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #5 | #5-s03 | 不正答案 (support_refs 型違反) → CLI が `error.code="invalid_support_refs_type"` を返す | snapshots/#5-s03_realign_error_invalid_support_refs_type.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #5 | #5-s04 | 正常答案 → error block が含まれない RealignResult | snapshots/#5-s04_realign_valid_no_error_block.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #6 | #6-s01 | 構造化失敗 (1 回目) → CLI error 詳細を Agent が読んで修正 → 2 回目で成功 | snapshots/#6-s01_retry_success_after_fix.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #6 | #6-s02 | 構造化失敗 → リトライ → 再失敗 → ⑥ 表示。出力に最後の答案と error 詳細が併記 | snapshots/#6-s02_retry_exhausted_tool_error.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #6 | #6-s03 | (削除: #5 完了済みのため本シナリオは不要) | — | — | — | — | 削除 |
| #7 | #7-s01 | doc lint: §8.5 本文に内部 field 名 / enum 値が含まれない (grep) | snapshots/#7-s01_design_no_internal_field_names.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #7 | #7-s02 | doc lint: §8.5 のカテゴリマップ表が #1 最終マップと一致、#2/#3/#4 テンプレ語彙と §8.5 整合 | snapshots/#7-s02_design_category_map_consistency.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #7 | #7-s03 | doc lint: §8.5 のリトライポリシー記述と #6 のテンプレ手順が整合 | snapshots/#7-s03_design_retry_policy_consistency.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
...
| #9 | #9-s04 | `spec-anchor inject-chapters` stdout が valid JSON 単体 | snapshots/#9-s04_inject_chapters_stdout_single_json.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #9 | #9-s05 | `spec-anchor inject-purpose` stdout が valid JSON 単体 | snapshots/#9-s05_inject_purpose_stdout_single_json.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #9 | #9-s06 | `spec-anchor inject-conflicts` stdout が valid JSON 単体 | snapshots/#9-s06_inject_conflicts_stdout_single_json.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
...
| #10 | #10-s04 | `.codex/skills/spec-anchor/SKILL.md` install 後の語彙整理が最新と一致 (file diff のみ) | snapshots/#10-s04_codex_skill_vocabulary_aligned.md | `[✓]` | `[✓]` | `[✓]` | 未確認 |
| #12 | #12-s01 | doc lint: spec-inject.md / spec-realign.md / EXTERNAL_DESIGN.ja.md §8.3 に「能動的追加探索」「Agent が判断」「4 path は起点 (上限ではない)」を奨励する文言が grep でヒット | (実装完了時) | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #12 | #12-s02 | doc lint: 既存記述 (「path は必須ではなく許可」「Agent が選んで使い分ける」「evidence_origin 縛り」) と新規追加記述が矛盾しない (両立記述) | (実装完了時) | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #12 | #12-s03 | spec_anchor/templates/.claude/commands/spec-inject.md / spec-realign.md がプロジェクト直下版と一致 (file diff、#10 連動) | (実装完了時) | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #13 | #13-s01 | 再生成された #3-s01〜s05 / #8-s04 計 7 snapshot で `recommended_next_action` が日本語訳されている (`Ask a human to decide this conflict.` → 日本語へ置換) | tests/e2e/snapshots/#3-* + #8-s04 (再生成) | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #13 | #13-s02 | doc lint: 3 コマンドテンプレに「日本語訳契約」が grep でヒット | (実装完了時) | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #13 | #13-s03 | snapshot 全件横断: Agent 整形済み snapshot (#5 / #9 系を除く) に日本語以外の自然文が含まれない (`Ask` / `decide` / `Please` / `Resolve` 等が 0 件) | (実装完了時) | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #12-s01 | doc lint: `.claude/commands/spec-inject.md` / `.claude/commands/spec-realign.md` / `doc/EXTERNAL_DESIGN.ja.md` §8.3 に「能動的追加探索」「Agent が判断」「4 path は起点 (上限ではない)」を奨励する文言が grep でヒット | (実装完了時) | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #12-s02 | doc lint: 「path は必須ではなく許可」「Agent が選んで使い分ける」既存記述と新規追加記述が矛盾しない (両立記述で、ドリフト防止系の「evidence_origin 縛り」も維持) | (実装完了時) | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #12-s03 | `spec_anchor/templates/.claude/commands/spec-inject.md` / `spec-realign.md` がプロジェクト直下版と一致 (file diff、#10 連動) | (実装完了時) | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #13-s01 | 再生成された #3-s01〜s05 / #8-s04 計 7 snapshot で `recommended_next_action` が日本語訳されている (`Ask a human to decide this conflict.` が「人間判断で衝突を解消してください。」等に置換) | tests/e2e/snapshots/#3-* + #8-s04 (再生成) | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #13-s02 | doc lint: 3 コマンドテンプレに「日本語訳契約」が grep でヒット (「日本語訳」「日本語以外の自然文」「Agent は日本語訳に置き換える」のいずれか) | (実装完了時) | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #13-s03 | snapshot 全件横断: Agent 整形済み snapshot (CLI raw JSON snapshot である #5 / #9 系を除く) に日本語以外の自然文が含まれない (`Ask` / `decide` / `Please` / `Resolve` 等の英語フレーズが 0 件) | (実装完了時) | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
