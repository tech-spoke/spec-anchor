...
| 5 | T-realign-cli-error-detail | `structure_realign_answer` のエラーを field 単位に詳細化 | 未着手 | error code 細分化、CLI 出力 schema 拡張、テスト | 2026-05-29 | — |
| 6 | T-realign-retry-with-feedback | 構造化失敗時の 1 回リトライ + feedback 経路を契約化 | 未着手 | テンプレ記述、T-5 完了後に feedback 経路接続 | 2026-05-29 | — |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #5-s01 | 不正答案 (final 区分なし) → CLI が `error.code="missing_final_section"` / `error.field` / `error.expected` を含む JSON 返却 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #5-s02 | 不正答案 (constraints の evidence_origin 不正値) → CLI が `error.code="invalid_evidence_origin"` / `error.field="constraints[N].evidence_origin"` を返す |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #5-s03 | 不正答案 (support_refs 型違反) → CLI が `error.code="invalid_support_refs_type"` を返す |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #5-s04 | 正常答案 → error block が含まれない RealignResult 返却 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
| シナリオ ID | 概要 | エビデンス | pytest | LLM 自己確認 | 完了 | 人間レビュー |
|---|---|---|---|---|---|---|
| #6-s01 | 構造化失敗 (1 回目) → CLI error 詳細を Agent が読んで修正 → 2 回目で成功 → 整形済み RealignResult |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #6-s02 | 構造化失敗 (1 回目 + リトライ) → 再失敗 → ⑥ ツール側エラー表示。出力に「最後の答案 JSON」と「CLI error 詳細」が併記される |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #6-s03 | #5 未完了状態の挙動 (盲目リトライ無し): 構造化失敗 → 1 回試行で ⑥ へ落とす (#5 完了後は本シナリオを削除) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| (sub task 実装中に判明したら追記) |  |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
...
| #5 | #5-s01 | 不正答案 (final 区分なし) → CLI が `error.code="missing_final_section"` / `error.field` / `error.expected` を返す |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #5 | #5-s02 | 不正答案 (constraints の evidence_origin 不正値) → CLI が `error.code="invalid_evidence_origin"` / `error.field` を返す |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #5 | #5-s03 | 不正答案 (support_refs 型違反) → CLI が `error.code="invalid_support_refs_type"` を返す |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #5 | #5-s04 | 正常答案 → error block が含まれない RealignResult |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #6 | #6-s01 | 構造化失敗 (1 回目) → CLI error 詳細を Agent が読んで修正 → 2 回目で成功 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #6 | #6-s02 | 構造化失敗 → リトライ → 再失敗 → ⑥ 表示。出力に最後の答案と error 詳細が併記 |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #6 | #6-s03 | #5 未完了状態: 構造化失敗 → 1 回試行で ⑥ へ落とす (#5 完了後は本シナリオを削除) |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |
