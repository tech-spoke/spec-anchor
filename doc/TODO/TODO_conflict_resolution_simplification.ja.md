...
**最終更新**: 2026-05-30
**ステータス**: 実装・pytest 検証済み / 未完了（production E2E・人間レビュー未実施）
...
|---|---|---|---|---|---|
| #1〜#7 実装 + unit / integration pytest | PASS | none / fake / default | `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m pytest --skip-external -q -p no:cacheprovider` → 685 passed, 22 skipped | 22 skipped は、このコマンドで意図的に除外した外部サービス検証など。skip 件数だけで今回 TODO の残作業を増やさない | production E2E と人間レビューへ進む |
...
| production E2E | NOT-RUN | production E2E | 未実行 | 実運用経路の一巡検証をまだ実施していない | `/spec-core` → `/spec-inject` → dismiss → source hash 変更 → 再浮上を real provider で通す |
