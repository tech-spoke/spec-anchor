...
| 5 | T-template-dismiss-flow | コマンドテンプレに却下フロー（説明 vs 却下分離 / 証跡表示 / 実行前確認）と、realign の pending 時答案抑止（#1 (b) で CLI ゲートを置かずテンプレが担う）を記述 | 未着手 | — | 2026-05-30 | — |
...
| 7 | T-freshness-degraded-fold | `degraded_optional_artifact` を `failed_required_artifact` に畳み `degraded` status を廃止（軸1: 4→3、軸2: degraded 理由除去）。section_metadata 部分失敗も failed 扱い。中間 status 不採用で pending 変更と独立化したため独立 sub task へ昇格（2026-05-30、(い)） | 未着手 | — | 2026-05-30 | — |
| 8 | T-e2e-conflict-simplification | 既存 `tests/e2e/` 基盤を再利用し、本 TODO の挙動変更（pending 非ブロック・realign 提示停止・dismiss 一巡・degraded→failed）を E2E シナリオで検証。挙動変更で無効化される既存 snapshot（`#2-s06` 等）の除去・再生成も担う。3 段ゲート（pytest + LLM 自己確認 + 人間レビュー） | 未着手 | — | 2026-05-30 | — |
...
- `inject-conflicts` CLI の完全削除（`cli.py:165,297-298,461-471` / `inject.py:552-606,784`、外部設計書 §8.4 / draft §6.1.6 の記述も削除）— **確定済み（2026-05-30）**。調査結論: pending conflict の本文提示は freshness gate 経由（`inject.py:65` `_hydrate_pending_conflict_items` / `freshness.py:320` `pending_conflict_items`）で既に行われており、`inject-conflicts`（`inject.py:552-606` `run_inject_conflicts`）は `status == "resolved"` のみを返す path ④ 専用関数。path ④ 廃止で唯一の用途が消え、pending 提示へ転用する余地もないため完全削除でよい
...
- **確定（既存踏襲、2026-05-30）**: 却下時の証跡には conflict_id を出す（再参照用、現状も末尾に出している）。利用者向け本文では conflict_id を出さず内部保持する現テンプレの扱いは維持する
...
- ルール 4 / ルール 5 削除はオーナー承認済み（2026-05-30）。ルール番号は **欠番維持で確定**（`AGENTS.md` がルール 6〜12 を番号参照しているため繰り上げない。6〜19 は不変）
...
- 無効化 snapshot の処理: `#2-s06` を `scenarios.py` と live snapshot から除去（degraded 廃止）、`#3`/`#4` 系を新挙動で再生成・再検証
...
|---|---|---|---|---|---|---|
| #8-s01 | 多数 pending のプロジェクトで `/spec-inject` が blocked にならず inject-search 実行可・`pending_conflict_items` 返却 | tests/e2e/snapshots/#8-s01_*.md | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #8-s02 | `/spec-realign` テンプレートが pending 時に提示停止し、答案を生成せず答案付き `spec-anchor realign` を呼ばない（CLI ゲートではなくテンプレ/LLM が抑止） | tests/e2e/snapshots/#8-s02_*.md | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #8-s03 | `/spec-inject` と `/spec-realign` が pending 時に同一提示で停止 | tests/e2e/snapshots/#8-s03_*.md | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #8-s04 | dismiss CLI 実行 → dismissed → 以後提示されない | tests/e2e/snapshots/#8-s04_*.md | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #8-s05 | dismissed が source 変更（ハッシュ失効）で再 pending として再浮上（一巡） | tests/e2e/snapshots/#8-s05_*.md | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #8-s06 | 「説明だけ」で却下が永続化されない / 「却下明示」で CLI 実行+証跡表示（#5 連動） | tests/e2e/snapshots/#8-s06_*.md | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| #8-s07 | section_metadata 部分失敗で freshness status=failed 停止（#7、旧 `#2-s06` の置換） | tests/e2e/snapshots/#8-s07_*.md | `[ ]` | `[ ]` | `[ ]` | 未確認 |
| — | （sub task 実装中に判明したシナリオはここに追記） |  | `[ ]` | `[ ]` | `[ ]` | 未確認 |

snapshot ファイル名の `<短い識別子>` は各シナリオ実装時に決める（前回命名規約 `<scenario_id>_<短い識別子>.md` を踏襲）。
...
- `#3-s01〜s05` / `#4-s01/s02` が新挙動で再生成され、禁止用語横断チェックも pass
