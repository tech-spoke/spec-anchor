...
- 現フェーズ: Phase 11 stage timings / performance observability 完了
- 次フェーズ: Phase 12 production query path / artifact consistency / Graph RAG retrieval hardening
...
- ただし低レベル `build_injection()` には、`core_update` も `freshness_report` も渡されない場合に `run_core_update()` を実行する古い安全弁が残る
- `run_core_update()` は graph/vector/chunk/sidecar artifacts を途中で書き出すため、後段 stage が失敗すると artifact が部分更新され得る
- `retrieval.graph_expansion_hops` は config として存在するが、retrieval path の graph traversal は bounded BFS ではなく single-pass relation merge に近い
- BM25 / dense / graph retrieval は JSON 全体ロードと全件走査が多く、大量 source / chunk / relation 時の query latency が読みにくい
...
- [ ] artifact transaction / rollback を実装する
  - `.spec-grag/graph/.staging/<artifact_revision>/` に core artifacts を書き出す
  ...
  - `artifact_revision.json` などの commit marker を追加し、readiness report に active / staging / failed revision を出す
  - 途中失敗後に `spec-inject` / `spec-realign` が古い一貫した artifact を読める regression test を追加する
- [ ] `build_injection()` を read-only default にする
  ...
  - public API として「InjectionContext build は原則 read-only」という契約を `DESIGN.ja.md` に追記する
- [ ] entity embedding の入力表現を修正する
...
- [ ] graph expansion を `graph_expansion_hops` に従う bounded traversal にする
  - selected section / chunk / explicit target から seed graph nodes を作る
  - hop 上限、relation type allowlist、confidence threshold、max graph entities を config 化する
...
- [ ] BM25 / dense retrieval の差し替え境界を切る
...
- [ ] classification dedup / budget 挙動を修正する
...
- [ ] stable section / chunk / anchor ID の移行設計を入れる
...
- [ ] security / artifact privacy hardening を行う
  ...
  - source 内 instruction が extraction / classification / answer 方針を上書きしない regression test を追加する
- [ ] Phase 11 timing metrics を production tuning 用に拡張する
