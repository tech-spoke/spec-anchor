...
- [ ] classification candidate collection layer を追加する
...
- [ ] stable ID / classification key による dedup を分類前に行う
...
- [ ] `classification_priority_score` を実装する
...
- [ ] type 別 budget を config 化する
  - `max_items` を全体上限として維持する
  - `max_source_chunks`、`max_concepts`、`max_graph_entities`、`max_chapter_anchors`、`max_clusters` を追加する
  - type cap を超えた item は `classification_incomplete` ではなく、必要なら `classification_deprioritized` として区別するか検討する
  - config schema / template / `EXTERNAL_DESIGN.ja.md` / `DESIGN.ja.md` を同期する
- [ ] classification 実行順を priority sort 後に統一する
...
- [ ] degraded / fail policy を priority aware にする
  - high-priority item が未分類なら production / CI では degraded または fail-fast にする
  - low-priority item だけが未分類なら warning に留める
  - `spec-inject` と `spec-realign` で許容度を分ける。realign は回答生成に使う high-priority source が未分類ならより厳しく扱う
  - ResultEnvelope の `warnings` / `degraded_components` / review notes に、未分類の type と件数を出す
- [ ] observability を追加する
...
- [ ] production self E2E で再測定する
  ...
  - `status=ok` または low-priority warning のみの `degraded` に改善することを目標にする
