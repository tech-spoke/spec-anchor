...
- 次フェーズ: Phase 12 残タスク retrieval index / BM25 postings / stable ID / security hardening
...
- Phase 12 初回実装で `retrieval.graph_expansion_hops` は bounded traversal に接続済み。relation policy / confidence threshold / max entity cap の config 化は残る
...
- [x] artifact transaction / rollback を実装する
  ...
  - readiness report に active / staging / failed revision を出す連携は残タスクとする
...
- [x] `build_injection()` を read-only default にする
  ...
  - public API として「InjectionContext build は原則 read-only」という契約の `DESIGN.ja.md` 追記は documentation task に残す
...
- [x] graph expansion を `graph_expansion_hops` に従う bounded traversal にする
  ...
  - relation type allowlist、confidence threshold、max graph entities の config 化は残タスクとする
