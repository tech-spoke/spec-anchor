...
- [x] stable section identity の生成 / 引き継ぎ規則を実装する
...
- [x] manifest lookup / reconcile を stable identity 主体に移す
...
- [x] document chunks の主参照を `stable_chunk_uid` に移す
...
- [x] BM25 / dense / retrieval index を stable chunk / section key に寄せる
...
- [x] graph node / edge provenance に stable IDs を必須化する
...
- [x] AgenticSearchCandidate / citation / display の alias 解決を追加する
...
### Phase 14. classification priority / budget policy

目的: production E2E で見えた `classification_incomplete` を、単なる `classification.max_items` 増加ではなく、候補の重要度に基づく分類順序・予算配分・degraded/fail 判定として整理する。GraphRAG retrieval で拾った候補をすべて同列に LLM classification へ流すのではなく、raw source / Purpose / approved Concept / conflict 疑い / target 近傍を優先し、cluster / community summary など補助情報は後回しにする。

背景:

- Phase 12 で classification cache / dedup / budget 超過時の `classification_incomplete` 明示は実装済み
- ただし現状は分類が inline に実行されるため、graph entity / concept / chapter / cluster が先に budget を消費し、raw source や Purpose が後から `classification_incomplete` になり得る
- 2026-05-02 の production E2E では `classification.max_items = 8` に対し、`spec-inject` は 25 候補中 17 skipped、`spec-realign` は 27 候補中 19 skipped となり、どちらも `status=degraded`
- `classification.max_items` は 20 程度まで拡大してよいが、拡大だけでは重要候補が先に分類される保証がない

方針:

- retrieval 後に分類候補を一度集め、stable ID / classification key で dedup してから LLM classification に渡す
- vector / BM25 / graph の retrieval score を土台にしつつ、SPEC-grag 用の `classification_priority_score` を追加する
- semantic similarity だけでなく、根拠の強さ、制約らしさ、target 近傍、approved 状態、conflict 疑い、候補 type を priority に反映する
- global `max_items` だけでなく、候補 type ごとの budget / cap を導入する
- `classification_incomplete` は件数だけでなく、未分類候補の priority / type によって warn / degraded / fail を分ける

優先 tier:

- Tier 0: Purpose、明示 target source、approved Concept / KeyAnchor、既承認 Conflict など、原則として常に注入または最優先分類するもの
- Tier 1: raw source chunk、target file / target section 近傍、conflict 疑い、高 retrieval score source、approved Concept に強く紐づく source
- Tier 2: graph entity、chapter anchor、dependency 近傍、relation path 付きの候補
- Tier 3: cluster / community summary、chapter summary、低 score 候補、補助的な俯瞰情報
...
- [x] classification candidate collection layer を追加する
  - inline classification ではなく、source / purpose / concept / chapter / cluster / graph entity / agentic candidate を `ClassificationCandidate` として収集する
  - candidate には `classification_key`、`stable_section_uid`、`stable_chunk_uid`、`candidate_type`、`retrieval_score`、`source_strength`、`tier`、`priority_score` を持たせる
  - 既存 output schema への反映位置を保持するため、candidate から ConstraintContext / TargetContext へ戻す mapping を持つ
- [x] stable ID / classification key による dedup を分類前に行う
  - 同じ raw source が constraint / target の両方に出る場合も LLM classification は一度だけにする
  - `stable_chunk_uid`、`stable_section_uid`、`concept_chunk_id`、`chapter_anchor_id`、`cluster_id`、`entity_id` を優先 key にする
  - key がない legacy item は既存の excerpt digest fallback を使う
- [x] `classification_priority_score` を実装する
  - retrieval score、BM25 score、dense score、graph path confidence を正規化して土台にする
  - raw source / Purpose / approved Concept / KeyAnchor / target 近傍 / conflict 疑いを boost する
  - cluster / community summary、低 confidence relation、補助 summary を penalty にする
  - priority の内訳を run artifact の classification stage metrics に残す
- [x] type 別 budget を config 化する
  - `max_items` を全体上限として維持する
  - `max_source_chunks`、`max_concepts`、`max_graph_entities`、`max_chapter_anchors`、`max_clusters` を追加する
  - type cap を超えた item は `classification_budget_skip_reason = "type_budget_exhausted"` と `deprioritized_count_by_type` で説明する
  - config schema / template / project setup を同期する
- [x] classification 実行順を priority sort 後に統一する
  - Purpose / raw source が graph entity や cluster より後回しにならないことを regression test で固定する
  - budget 消費後の skipped item には `classification_source = "classification_incomplete"` と `classification_llm_skipped = "max_items_exhausted"` を維持する
  - cache hit は budget を消費しないことを確認する
- [x] degraded / fail policy を priority aware にする
  - high-priority item が未分類なら production / CI では degraded または fail-fast にする
  - low-priority item だけが未分類なら warning に留める
  - `spec-inject` / `spec-realign` とも high-priority incomplete は `fail_on_high_priority_incomplete=true` で failed にする
  - ResultEnvelope の `warnings` / `degraded_components` と stage metrics に、未分類の type と件数を出す
- [x] observability を追加する
  - `stage_timings.classification.metrics` に `candidate_count_by_type`、`classified_count_by_type`、`skipped_count_by_type`、`deprioritized_count_by_type`、`high_priority_skipped_count` を出す
  - selected / skipped の上位数件について priority reason を artifact に残す。ただし source本文の保存は privacy policy に従う
- [x] regression / E2E test を追加する
  - `max_items` が小さい場合でも raw source / Purpose が graph entity / cluster より先に分類される
  - 同一 source evidence が複数 context に出ても LLM call は一度だけ
  - high-priority incomplete は failed、medium/low-priority incomplete は degraded warning
  - type cap により cluster が source budget を食い潰さない
  - production fallback は引き続き silent rule-based へ落ちない
- [x] production self E2E で再測定する
  - `classification.max_items = 20` の repo-local config で `spec-inject` / `spec-realign` を再実行する
  - `classification_incomplete` が残る場合、未分類が Tier 2 / Tier 3 に偏っていることを確認する
  - `status=degraded` は残るが、`high_priority_skipped_count = 0` となり、未分類は graph entity / chapter anchor / cluster に偏った

実装結果:

- `classification.max_items = 20`、`max_source_chunks = 12`、`max_concepts = 4`、`max_graph_entities = 4`、`max_chapter_anchors = 2`、`max_clusters = 2` を schema / repo config / template / project setup に追加
- classification は retrieval 後に `ClassificationCandidate` として収集し、classification key dedup、priority sort、global/type budget 適用後に実行する
- `classification.batch_size = 5` を追加し、LLM classification を候補ごとの個別 subprocess ではなく batch structured output で実行する
- `classification.cache_enabled = true` / `cache_path = ".spec-grag/cache/classification_cache.json"` を追加し、`classification_key` と content fingerprint / provider / model / prompt policy が一致する場合は budget 消費前に persistent cache hit とする
- `classification_source = "classification_incomplete"`、`classification_llm_skipped = "max_items_exhausted"`、`review_required = true` の production 契約は維持
- metrics は `candidate_count_by_type`、`classified_count_by_type`、`skipped_count_by_type`、`deprioritized_count_by_type`、`high_priority_skipped_count`、selected/skipped summary を出す
- 2026-05-02 self production `spec-inject` batch 初回: total `72,489ms`、retrieval `19,104ms`、classification `53,085ms`、classification LLM calls 4、`high_priority_skipped_count=0`
- 2026-05-02 self production `spec-inject` persistent cache 再実行: total `44,670ms`、retrieval `14,124ms`、classification `30,277ms`、classification LLM calls 3、cache hit 15、`high_priority_skipped_count=0`
- 2026-05-02 self production `spec-realign` batch/cache 後: total `89,423ms`、retrieval `17,615ms`、classification `28,672ms`、answer `42,844ms`、classification LLM calls 2、cache hit 19、`high_priority_skipped_count=0`
- 残課題: query planner / answer generation も LLM latency の大きな比率になった。classification は batch/cache で改善済みだが、retrieval query planner cache や answer latency は別途改善余地あり。

推奨順:

1. config schema / template に type budget を追加
2. candidate collection / mapping を最小実装
3. dedup と cache reuse を分類前に移動
4. priority score と sort を追加
5. priority aware degraded / fail policy
6. metrics / run artifact 拡張
7. production self E2E 再測定

完了条件: `classification.max_items` を 20 程度に設定した production E2E で、Purpose / raw source / target 近傍 / conflict 疑い / approved Concept が優先的に LLM classification される。未分類が残る場合も high-priority 候補ではなく補助的な cluster / summary 等に偏り、ResultEnvelope には未分類の type / priority / 件数が説明可能な形で出る。
...
- `doc/PHASE13_REPORT.ja.md`: Phase 13 stable identity migration の実行報告
- `doc/PHASE14_REPORT.ja.md`: Phase 14 classification priority / budget policy の実行報告
