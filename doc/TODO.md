...
- 現フェーズ: Phase 10 watcher / GRAG readiness gate / Concept diff queue 化
- 次フェーズ: Phase 10 完了後、契約監査 / production readiness / E2E / GRAG 品質 / 障害系を `doc/AUDIT_TODO.ja.md` に従って実施する
...
現状: Phase 9 で production 主経路と provider policy は整備済み。no-change / format-only は semantic hash で数秒に短縮済み。ただし、日常運用では意味変更時の extraction / embedding / community refresh に時間がかかるため、watcher による background incremental が必要である。また pending Concept diff 中の追加変更を扱う queue / provisional cache / 単一 pending 制約は未実装である。
...
- [ ] runtime config / policy resolver を追加する
...
- [ ] watch state / queue schema を追加する
...
- [ ] watcher entrypoint を追加する
  - Source specs を watch し、debounce 後に background core incremental を実行する
...
- [ ] Concept diff 単一 pending / queued change を実装する
  - pending Concept diff がある間は新しい diff を多重生成しない
  - queued change は古い diff として保存せず、再評価対象として保存する
  - accept / reject / revise / apply 後に queued change を最新 Concept base hash で再評価する
  - base hash mismatch では apply を blocked にする
- [ ] provisional concept cache を実装する
  ...
  - rejected / suppressed 履歴で同一候補の再提案を抑制する
  - approved Concept / provisional / rejected を novelty gate で区別する
  - provisional cache が InjectionContext / Answer / ConflictNotes に混入しない regression を追加する
- [ ] Conflict candidate state を readiness gate に統合する
...
完了条件: local daily mode では watcher が Source specs の変更を background incremental で処理し、`/spec-inject` / `/spec-realign` は同期 core 更新を行わず readiness gate と承認フローだけを通る。Concept diff は単一 pending に制限され、pending 中の変更は queue / provisional cache で再評価される。CI / watcherなしでは foreground incremental が可能で、production では dirty / pending / stale が fail-fast する。
