...
同様に、セクションごとの要約、関連セクション、章単位の要点も単独では根拠にならない。制約として採用する場合は、Purpose / Core Concept / Source Specs 本文のいずれかを根拠として確認する。
...
§6.2 の 3 path は探索の起点であり上限ではない。Agent は根拠が不十分と判断した場合、別の検索キー生成・別 path への切り替え・上位章への hop を能動的に行う。探索の十分性 (もう追加探索は不要で答案構成へ進めるか) は Agent が判断し、CLI は path 数・hop 数の上限を強制しない。ただし根拠は Purpose / Core Concept / Source Specs に縛られ、CLI 道具 (`spec-anchor inject-*`) を介さずに Source Specs を直接 grep する経路は禁止する (検索の起点は必ず CLI 経由)。Source Specs 本文の `Read` は CLI で section_id を特定した後の補助確認としてのみ許可される。
...
| `spec-anchor realign` | `error` object がある場合のみ (config 不在等)。gate 停止 (`status="blocked"` / `"failed"`) では exit 0 |
| `spec-anchor-setup-system` | `status` が `error` / `failed` |
| `spec-anchor-setup-project` | `status` が `conflict` / `error` / `failed` |
| `spec-anchor inject-search` / `inject-section` / `inject-chapters` / `inject-purpose` | 常に 0 (停止状態は stdout JSON の `should_stop` / `blocking_reasons` / `status` で表現) |
| `spec-anchor-watch` | 常に 0 |

`inject-*` と `spec-anchor-watch` の呼び出し元は CLI exit code を見ずに stdout JSON を parse する責務を持つ。

全コマンド共通: CLI 引数の構文エラー (未知フラグ、必須引数の欠落等) は argparse が exit code 2 + stderr usage text で終了し、stdout JSON は出ない。上記の exit code 方針は argparse を通過してコマンド実装に到達した後の契約である。

### 3.4 失敗時の共通契約

`/spec-core` が `status="failed"` を返す場合、stdout JSON 内の `freshness_report.blocking_reasons` に `failed_required_artifact` が積まれ、`freshness_report.status` も `failed` になる。これにより後続の `/spec-inject` / `/spec-realign` は §12 freshness gate で停止する。`/spec-core` 自身は新しい canonical artifact を上書きせず、前回値を残す。他のコマンド (`setup-project` / `setup-system` / `watch`) の失敗は各コマンド章を参照。

### 3.5 人間向け表示契約 (停止時・正常完了時・リトライ)

slash command (`/spec-core` / `/spec-inject` / `/spec-realign`) が利用者へ返す応答は、利用者が SPEC-anchor の内部構造を知らない前提で読める言葉で書く。shell CLI が返す内部の状態名・enum 値・フィールド名・pipeline stage 名をそのまま会話へ貼ってはいけない。表示の具体フォーマットは各コマンドテンプレ (`.claude/commands/spec-*.md` / `.codex/skills/spec-anchor/SKILL.md`) が保持し、本節はその契約を定める。各コマンド章 (§5.2 / §6.2 / §7.2) の「正常時の出力」「停止時の出力形式」は本節の枠組みに従う。

#### 利用者向け本文に貼ってはいけない内部用語

利用者向け応答に出してはいけない内部用語 (control flag、freshness の停止理由 enum、pipeline stage 名、CoreResult の件数フィールド名、内部 result path、Conflict Review Item の生フィールド名、CLI が default で返す英語自然文) は `tests/e2e/forbidden_terms.py` の `FORBIDDEN_TERMS` を単一の真実とする。本文書はこの一覧を再掲しない (再掲すると二重管理になり乖離する)。Agent は CLI の内部値を人間向けの見出し・説明へ写像してから提示する。

#### 停止時の表示カテゴリ

コマンドが処理を進められず停止したとき、状況を次の利用者視点カテゴリへ写像し、該当カテゴリの見出しと「次の操作」だけを表示する。該当しないカテゴリは表示しない。

- ① 初期設定が完了していない: 設定ファイル、Purpose / Core Concept、対象仕様のいずれかが見つからない。`spec-anchor-setup-project` の実行やファイル作成を促す。
- ② 外部サービスへの接続が必要: 検索基盤やモデル呼び出しに必要なサービスへ接続できない。サービス起動 / 接続情報の確認を促す。
- ③ 保持物の更新が必要: 仕様または設定が変更され保持情報が古い。`/spec-core` の実行を促す。
- ④ 保持物の更新中・待機: 別プロセスが更新中、または更新待ちがある。完了を待つよう促す。
- ⑤ 人間判断が必要な仕様の衝突: 未解決の衝突がある。各衝突を人間向けの見出しで本文展開し、人間が矛盾の意味を判断（説明、または矛盾でなければ却下）できるよう提示する。衝突の本文展開と却下フローの契約は §6.2 / §7.2 の pending conflict 出力および §11 が定める (本節では展開しない)。
- ⑥ ツール側のエラー: 想定外のエラーで、利用者操作では解消できない。開発元への報告を促す。
- ◇ 情報通知 (続行可能): 補助的な保持情報のみが古いが処理は続行できる。停止せず、通常出力の末尾に参考情報を 1 行添える。
- ✕ 非表示: 回答候補の組み立てをコマンド側が待っている内部状態 (`/spec-realign` で回答候補がまだ構成されていないとき)。利用者には見せず、コマンドを実行している Agent が回答候補を組み立てて自動的に再実行する。

#### 正常完了時の表示

正常完了でも内部の指標名・状態名をそのまま貼らない。

- `/spec-core`: 「保持物の更新が完了しました」とともに、更新があった仕様 (変更が無ければ「変更ありませんでした」)、人間判断が必要な衝突 (あれば ⑤ で展開)、過去の衝突解消判断のうち仕様変更で見直し余地が出たものの再確認候補 (過去の判断は 採用 / 却下 / 修正 へ翻訳)、次の操作を表示する。詳細は §5.2。
- `/spec-inject`: §6.2 の 5 セクション (今回守る制約 / 今回見るべき対象 / 関連セクションとして確認したもの / 採用しなかったもの / 不確実性・人間確認) で表示し、各制約の根拠種別・参照補助・適用範囲・確認すべき点を人間向けの見出しで示す。
- `/spec-realign`: §7.2 の 4 区分 (今回守る制約 / 今回扱う修正候補または検討対象 / 競合・不確実性・人間レビュー / 課題への回答または修正案) で表示し、制約は `/spec-inject` と同じ人間向け見出しで示す。

#### 回答候補の形式不備とリトライ

`/spec-realign` に渡した回答候補の形式が不備のとき、shell CLI はどの項目が期待とどう違うかを項目単位で返す。これを受けて Agent は、指摘された項目だけを直して 1 回だけ再実行する。それでも整わない場合は ⑥ (ツール側のエラー) として扱い、最後に送った回答候補と「期待された形式との差分」を人間語で併記して表示する。リトライの途中経過や内部の項目名は利用者へ出さない。
...
| 環境変数 | 用途 |
|---|---|
| `SPEC_ANCHOR_FAKE_LLM` | **開発・検証用**。truthy (`1` / `true` / `yes` / `on`) で `/spec-core` が `[llm.providers.<id>].command` を起動せず in-process fake を使う。本運用の動作保証対象外 |
| `SPEC_ANCHOR_FAKE_RETRIEVAL` | **開発・検証用**。truthy で Qdrant + FlagEmbedding BGE-M3 の実構築を伴うコード経路を block する。本運用の動作保証対象外 |
...
```text
spec-anchor core [--all | -a] [--rebuild] [--verify-index] [--llm-provider <id>] [--dismiss-conflict <conflict_id> --reason "<text>"]
```
...
| `--dismiss-conflict <conflict_id>` | 任意 | (未指定) | 指定した Conflict Review Item を「矛盾ではない」として却下する | 対象 item を `dismissed` にし、却下時点の source hash を記録 (§11)。`--reason` 必須 |
| `--reason <text>` | `--dismiss-conflict` 使用時は必須 | (未指定) | 却下理由 | 空文字は受け付けない。`--dismiss-conflict` なしで指定しても効果はない |
...
| field | 型 | 必須 | 意味 |
|---|---|---|---|
| `status` | string | 必須 | `updated` / `failed` / `error`。Section Metadata 生成の失敗 (全失敗・部分失敗いずれも) は必須保持物の失敗として扱われ、`freshness_report.status` が `failed` になり後続コマンドを停止させる |
...
| `stale_dismissal_count` | int | 必須 | 却下根拠セクションの hash 変化により却下が失効 (`stale_dismissal`) した項目数 |
...
- CLI exit code: 0 (`status="updated"`)
- stdout JSON: 上記 field がすべて出力される
- 後続コマンド: `/spec-inject` / `/spec-realign` が実行可能 (freshness が `fresh`)

#### 警告付き続行

- 発火条件: Section Metadata の LLM 生成が一部 Section で失敗、ただし必須 artifact は揃う
- CLI exit code: 0
- Section Metadata の生成失敗時: `warnings[]` に Section ごとの失敗説明、`status="failed"`、`freshness_report.status="failed"`、`freshness_report.blocking_reasons=["failed_required_artifact"]`、`freshness_report.diagnostics.failed_required_artifacts=["section_metadata"]` (全失敗・部分失敗いずれも停止)
...
| 検索基盤に到達できない (`[vector_store].provider="qdrant"` 設定済み) | `status="failed"`、`retrieval_index_status="failed"`、`related_sections_status="failed"`、`diagnostics` に接続失敗の詳細 (診断用)、`warnings[]` に失敗理由 | 接続失敗理由 | 1 | サービス起動後 `/spec-core --rebuild` |
...
- Conflict Review Item の status を人間判断なしに `dismissed` に変えない (`spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` 以外の書き込み口を持たない)
...
利用者向けの表示 (停止カテゴリ・禁止内部用語・正常完了の見せ方) は §3.5 人間向け表示契約に従う。
...
#### 停止時の出力形式（利用者が観測できる内容）

利用者が観測できる出力は Agent が自然言語に整形した内容である。`<path>` は実際のプロジェクトパスに読み替える。

**設定ファイル不在:**
> `/spec-core` を実行しましたが、プロジェクトの設定ファイルが見つかりません（`<path>/.spec-anchor/config.toml`）。先に `spec-anchor-setup-project --target <path>` を実行してプロジェクトを初期化してから、`/spec-core` を再実行してください。

**Purpose ファイル不在:**
> `/spec-core` は失敗しました。Purpose ファイルが見つかりません（`<path>/docs/core/purpose.md`）。ファイルを作成してから `/spec-core` を再実行してください。Purpose の内容は人間が記述します。

**Core Concept ファイル不在:**
> `/spec-core` は失敗しました。Core Concept ファイルが見つかりません（`<path>/docs/core/concept.md`）。ファイルを作成してから `/spec-core` を再実行してください。Core Concept の内容は人間が記述します。

**仕様ファイル 0 件:**
> `/spec-core` は失敗しました。設定ファイルの `[sources].include` に一致する仕様ファイルが 1 件もありません。仕様ファイルを作成するか、設定の glob パターンを修正してから `/spec-core` を再実行してください。

**一部セクションのメタデータ生成失敗（保持データは揃った）:**
> `/spec-core` は完了しましたが、一部のセクションのメタデータ生成に失敗しました（例: 12 件中 2 件。失敗セクション: `docs/spec/api.md#0003-payments`、`docs/spec/auth.md#0001-session`）。必須の保持データは揃っているため `/spec-inject` / `/spec-realign` は実行可能です。失敗セクションを再生成する場合は `/spec-core --all` を実行してください。

**章単位の要点生成失敗:**
> `/spec-core` は失敗しました。3 章分の章単位の要点生成に失敗しました。前回の要点データは保持されているため既存の情報は引き続き参照可能ですが、最新の仕様変更は未反映です。`/spec-core --all` で再試行してください。

**検索基盤サービス接続失敗:**
> `/spec-core` は失敗しました。関連セクション検索の基盤サービス（`http://localhost:6333`）に接続できません。サービスを起動してから（例: `systemctl --user start qdrant`）、`/spec-core --rebuild` を実行してください。

**検索インデックス更新 / 検証失敗:**
> `/spec-core` は失敗しました。セクション検索インデックスの更新または検証に失敗しました。`/spec-core --rebuild` でインデックスを作り直してください。前回のインデックスは保持されているため、再構築完了まで既存の検索は利用可能です。
...
- shell CLI の `status` / reason を要約で書き換えて利用者へ伝達しない (`failed` / `blocked` を「続行できる」かのように弱めて伝えない)
...
pending な Conflict Review Item (矛盾) は freshness の停止理由ではない。保持物が最新で freshness が `fresh` のとき、各サブコマンドは Agentic Search を実行可能な状態で、課題に関係する矛盾本文を `pending_conflict_items[]` (各 item は §11.5 の object 構造)、件数を `pending_conflict_count` として返す。`blocking_reasons` には矛盾起因の理由は積まれない。矛盾の提示と停止は Agent 側 (§6.2) が行う。
...
| `hits[].related_sections[]` | list[object] | 必須 | 関連先一覧。各 object に `target_section_id` / `relation_hint` (`depends_on` / `impacts` / `prerequisite` / `same_policy` / `see_also`) を含む。`relation_hint` に `conflicts_with` は出現しない。Related Sections は Conflict Review Item (§11) の入力経路ではない |
...
#### 6.1.6 共通副作用として行わないこと

- Source Specs / Purpose / Core Concept を書き換えない
- 自動で `/spec-core` を実行しない (保持物が古い場合は停止のみ。Agent / 利用者が `/spec-core` を明示実行する)
- 自動探索 / 多段 traversal を実行しない (各サブコマンドは単発の retrieval / payload lookup / 章 anchor 取得 / Purpose 取得のみ提供)
- Conflict Review Item の status を変更しない (却下は `spec-anchor core --dismiss-conflict` 経由のみ)
...
§6.2 末尾の 3 path に従い、課題の性質に応じて次の組み合わせで `spec-anchor inject-*` を呼ぶ (Agentic Search)。
...
**3 path は探索の起点であり上限ではない。** Agent は 3 path 通過後でも、課題への根拠が不十分と判断した場合、別の検索キー生成・別 path への切り替え・上位章への hop を能動的に行う。探索の十分性 (これ以上の追加探索は不要で答案構成へ進めるか) は Agent が判断する。CLI は path 数・hop 数の上限を強制せず、`spec-anchor inject-*` には自動探索 / 多段 traversal を行うコマンドも含まれない (CLI は単発の retrieval / payload lookup / 章 anchor 取得 / Purpose 取得のみを提供し、Section 間の再帰的 lookup は Agent が行う)。「3 path 通過 = 終了」を機械的に解釈しない。ただし根拠は引き続き Purpose / Core Concept / Source Specs に縛られ、CLI 道具を介さずに Source Specs を直接 grep する経路は禁止する。

#### 根拠と入力の区別

会話区間 (現在のユーザー発話、直近の会話、明示された課題プロンプト、進行中の作業対象) は検索キー生成と制約生成の入力であり、仕様上の根拠ではない。最終根拠は Purpose / Core Concept / Source Specs のいずれかに由来し、どれに由来するかを `evidence_origin` で区別する。
...
freshness が `fresh` の場合、次の 5 セクション構造で利用者へ提示する。

```text
今回守る制約
  - <制約>
    根拠: Purpose / Core Concept / Source Specs のいずれか
...
|---|---|---|
| `statement` | 必須 | 今回守る制約 |
| `evidence_origin` | 必須 | `Purpose` / `Core Concept` / `Source Specs` のいずれか |
| `evidence_ref` | 必須 | 文書 path、`source_section_id`、source span、または Core Concept の項目 |
| `support_refs` | 任意 | セクションごとの要約 / 関連セクション / 章単位の要点 などの参照補助 |
| `applicability` | 任意 | 今回の課題でどこに効く制約か |
| `uncertainty` | 任意 | 根拠不足 / 衝突 / 人間確認が必要な点 |

`statement` / `evidence_origin` / `evidence_ref` の 3 field は欠かさない。検索キー / セクションごとの要約 / 関連セクション・章単位の要点だけを根拠に制約を確定しない (根拠は常に Purpose / Core Concept / 仕様本文のいずれか)。
...
| pending Conflict Review Item | `pending_conflict_items[]` の各項目を §11.4 の構造で提示 (`conflict_id` / `severity` / `conflict_points` / `why_conflicting` / `why_llm_cannot_decide` / `source_refs` / `recommended_next_action`) | freshness の停止理由ではない。Agent は制約情報と併せて提示し、矛盾ではないと人間が明示した場合だけ `spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` を実行 |
...
利用者が観測できる出力は Agent が自然言語に整形した内容である。`<path>` は実際のプロジェクトパスに読み替える。停止時は制約検索（Agentic Search）を実行したかどうかを必ず伝える。

**設定ファイル不在:**
> `/spec-inject` は実行できません。プロジェクトの設定ファイルが見つかりません（`<path>/.spec-anchor/config.toml`）。先に `spec-anchor-setup-project --target <path>` を実行し、次に `/spec-core` を実行してから `/spec-inject` を再実行してください。制約検索は実行していません。

**仕様ファイルの変更が未反映:**
> `/spec-inject` は停止しました。仕様ファイルに変更がありますが、保持データがまだ更新されていません。`/spec-core` を実行してから `/spec-inject` を再実行してください。制約検索は実行していません。

**自動更新実行中 / 処理待ちキュー残:**
> `/spec-inject` は停止しました。保持データの自動更新が進行中です（または処理待ちの変更が残っています）。自動更新の完了を待ってから `/spec-inject` を再実行してください。制約検索は実行していません。

**人間判断が必要な仕様矛盾（Conflict Review Item）:**
> `/spec-inject` は、今回守る制約とともに人間判断が必要な仕様の矛盾を N 件提示しました。Agentic Search は実行済みです（pending な矛盾は停止理由ではなく、注入すべき情報です）。
>
> ## 今回守る制約
> - …（Agentic Search で集めた制約）
>
> ## 人間判断が必要な矛盾（N 件）
> 矛盾 1（重要度: 高）
> 対立する主張: 「A は同期処理」vs「A は非同期処理」
> なぜ矛盾するか: §3.2 と §5.1 が矛盾している
> LLM が判断できない理由: 両方に同等の根拠がある
> 参照箇所: docs/spec/api.md の §0003、docs/spec/queue.md の §0001
> 次のアクション: 矛盾の意味を会話で人間に伝えてください（`Ask a human to decide this conflict.`）
>
> 矛盾ではないと人間が判断した場合は `spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` で却下できます。却下は強制されません。

**必須の保持データが欠損:**
> `/spec-inject` は停止しました。直前の `/spec-core` で必須の保持データ（章単位の要点、関連セクション、検索インデックスのいずれか）の生成に失敗しています。警告: 1 章分の章単位の要点生成が失敗しました。`/spec-core --all` で保持データを再構築してから `/spec-inject` を再実行してください。制約検索は実行していません。
...
| Agent が構成した answer candidate | 必須 | Agent が §6.2 の 3 path で抽出した制約と回答案を 4 区分構造で構成したもの。CLI option 経由で渡す | 不在時は `status="fresh"`、`stop_reason="needs_agent_answer"`、`should_stop=true`、`recommended_next_action="provide an Agent-generated answer candidate for /spec-realign"`、CLI exit code 0 |
...
| `pending_conflict_items[]` | list | pending な Conflict Review Item がある時任意 | §11 |
...
利用者向けの表示 (停止カテゴリ・禁止内部用語・正常完了の見せ方・リトライ) は §3.5 人間向け表示契約に従う。
...
`/spec-inject` と同じ §6.2 の 3 path で制約を抽出した後、`spec-anchor realign --answer-json '<json>'` (または `--answer-text` / `--answer-file`) を呼ぶ。
...
CLI が回答候補をまだ受け取っていない状態 (`spec-anchor realign` を回答候補なしで呼んだとき返る `stop_reason="needs_agent_answer"`) は §3.5 の ✕ カテゴリ (非表示) に該当する。これは停止として利用者へ見せない。`/spec-realign` を実行している Agent 自身が §6.2 の 3 path で制約を抽出して回答候補を 4 区分で構成し、`spec-anchor realign --answer-json` を自動的に再実行する。Agent が課題の中心を特定できず回答候補を構成できない場合のみ、上表の「課題の中心を特定できなかった」として利用者へ追加情報を求める。

#### 停止時の出力形式（利用者が観測できる内容）

利用者が観測できる出力は Agent が自然言語に整形した内容である。`<path>` は実際のプロジェクトパスに読み替える。停止時は常に「回答の整形は実行していません」を明示する。

**設定ファイル不在:**
> `/spec-realign` は実行できません。プロジェクトの設定ファイルが見つかりません（`<path>/.spec-anchor/config.toml`）。先に `spec-anchor-setup-project --target <path>` を実行し、次に `/spec-core` を実行してから `/spec-realign` を再実行してください。回答の整形は実行していません。

**仕様ファイルの変更が未反映 / 自動更新実行中:**
> `/spec-realign` は停止しました。仕様ファイルに変更がありますが、保持データがまだ更新されていません（または更新が進行中です）。`/spec-core` を実行してから（または自動更新の完了を待ってから）`/spec-realign` を再実行してください。回答の整形は実行していません。

**人間判断が必要な仕様矛盾（Conflict Review Item）:**
> `/spec-realign` は、今回守る制約とともに人間判断が必要な仕様の矛盾を N 件提示し、回答の生成は行いませんでした。矛盾がある間はどちらの主張に沿うかを暗黙に選ばないため、回答を整形しません（Agentic Search は実行済みです）。
>
> ## 今回守る制約
> - …（Agentic Search で集めた制約）
>
> ## 人間判断が必要な矛盾（N 件）
> 矛盾 1（重要度: 高）
> 対立する主張: 「retry は 3 回」vs「retry は 5 回」
> なぜ矛盾するか: §4.1 と §6.2 が矛盾している
> LLM が判断できない理由: 両方が現行仕様ファイルに存在する
> 参照箇所: docs/spec/policy.md の §0004、docs/spec/retry.md の §0001
> 次のアクション: 矛盾の意味を会話で人間に伝えてください（`Ask a human to decide this conflict.`）
>
> 矛盾ではないと人間が判断した場合は `spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` で却下できます。却下は強制されません。

**必須の保持データが欠損:**
> `/spec-realign` は停止しました。直前の `/spec-core` で必須の保持データの生成に失敗しています。`/spec-core --all` で保持データを再構築してから `/spec-realign` を再実行してください。回答の整形は実行していません。

回答候補がまだ構成されていない状態 (`stop_reason="needs_agent_answer"`) は §3.5 の ✕ カテゴリであり、上記の利用者向け停止メッセージに含めない。Agent は 3 path で制約を抽出して回答候補を構成し、自動的に再実行する。回答候補を構成できないのは課題の中心を特定できないときに限り、その場合は次節「Agent が追加確認を求める条件」に従い利用者へ追加情報を求める。
...
- `status="fresh"` で `stop_reason="needs_agent_answer"` → §3.5 の ✕ カテゴリ。利用者へは見せず、Agent が 3 path で回答候補を構成して自動再実行する (課題の中心が特定できないときのみ利用者へ追加確認)
...
| `production_readiness.blocking_reasons[]` | list[string] | 必須 | `qdrant_service_unavailable` / `flagembedding_missing` / `qdrant_client_missing` / `agent_cli_unavailable` / `console_script_missing` 等 |
...
| `FlagEmbedding` / `qdrant_client` package import 不可 | `blocking_reasons=["flagembedding_missing"]` / `["qdrant_client_missing"]` | 不在 package 名 | 0 (warning) | package 導入 |
| Agent CLI (`codex` / `claude`) が PATH 上に無い | `blocking_reasons=["agent_cli_unavailable"]` | 不在 CLI 名 | 0 (warning) | Agent CLI 導入 |
| console script が PATH 上に無い | `blocking_reasons=["console_script_missing"]` (不在 script 1 件につき 1 reason) | 不在 script 名 | 0 (warning) | SPEC-anchor 再インストール |
...
| オプション | 必須 | 既定値 | 意味 | 観測効果 |
|---|---|---|---|---|
| `--target <path>` | 任意 | `.` (カレントディレクトリ) | 配置先 project root | 配置対象 path |
...
- 候補生成: `/spec-core` (Source Specs の Section ペア候補を LLM なしで作り、Section ペア conflict judge が未解消の矛盾だけを記録)
- 却下: 人間 (`spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` 経由。LLM が人間の指示を受けて代行実行する)

矛盾は「解決すべきゲート」ではなく「注入すべき情報」である。課題に関係する矛盾が提示された時点で目的 (LLM の仕様ドリフト防止) は達成されており、SPEC-anchor のデータ上で矛盾を「解決済み」にマークする仕組みは持たない。人間が矛盾に取れる操作は、説明 (会話で意味を伝える。状態は変えない) と却下 (矛盾ではないと判断して以後提示しない) の 2 つだけである。

Conflict 判定は、保存済みの候補ファイルを入力にしない。`/spec-core` は full run または変更がある incremental run で Section ペア候補をメモリ上で作り、Section ペア conflict judge に渡す。変更のない incremental run では候補生成と judge をスキップし、前回の Conflict Review Items を保持する。候補数、打ち切り件数、再確認件数は CoreResult の `diagnostics.section_pair_candidate_generation` に残る。

### 11.2 status 値とその遷移

| status | 意味 | 遷移元 | 遷移先 |
|---|---|---|---|
| `pending` | LLM が解決できず提示対象 | (新規) / 却下が失効した dismissed | `dismissed` (`--dismiss-conflict` 経由)、`pending` のまま (説明だけのとき) |
| `dismissed` | 「矛盾ではない」と人間が判断して却下した (以後提示しない) | `pending` | `pending` (却下根拠セクションの hash 変化で却下が失効した場合、§11.6) |

pending な矛盾は freshness の停止理由ではない。保持物が最新なら freshness は続行を許可し、CLI は矛盾本文を `pending_conflict_items[]` として返す。課題に関係する矛盾の提示と停止は Agent (§6.2 / §7.2) が行う。

### 11.3 却下の渡し方

矛盾の却下は `spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` の 1 つだけで行う。これ以外に矛盾の状態を変える操作・CLI option はない。

| 引数 | 必須 | 意味 |
|---|---|---|
| `--dismiss-conflict <conflict_id>` | — | 却下対象の Conflict Review Item の id |
| `--reason <text>` | 必須 | 却下理由 (証跡)。空文字は受け付けない |

- 却下対象が pending でない (既に dismissed) 場合、存在しない conflict_id の場合、`--reason` 不在 / 空の場合はエラーになる。
- 却下時、対象 item の `source_refs[]` が指す Source Specs の出典と、却下時点の Source Specs / Purpose / Core Concept のハッシュが `base_source_hashes` として記録される。
- 却下は人間が代行 LLM へ指示したことを表す human attestation を伴って記録され、ハッシュ失効による自動再 pending (§11.6) と区別される。

### 11.4 Conflict Review Item の object 構造
...
| `status` | 必須 | §11.2 の enum (`pending` / `dismissed`) |
...
| `section_pair` | 必須 | `section_pair_id` / `left_section_id` / `right_section_id` / `candidate_origin` を含む Section ペア情報 |
| `conflict_points[]` | 必須 | `left_excerpt` / `right_excerpt` / `why_conflicting` / `severity` を含む抜粋ペア |
...
| `base_source_hashes[]` | 必須 | 却下時に参照した source の hash 配列 (却下の失効判定に使う) |
| `valid_scope` | 必須 | `"section_pair"` |
| `stale_dismissal` | 必須 | boolean。dismissed item の却下根拠 source hash が変化した場合 `true` (§11.6) |
| `created_at` | 必須 | ISO 8601 UTC 形式 (例: `2026-05-22T03:00:00Z`)。item 作成時刻 |
| `updated_at` | 必須 | ISO 8601 UTC 形式。却下適用時を含む最終更新時刻 |
| `resolution` | dismissed 時任意 | 却下記録。`reason` / 参照した source refs / human attestation を含む |

### 11.5 stale_dismissal flag の発火条件と再 pending

却下された Conflict Review Item は `base_source_hashes[]` を持つ。却下根拠にした source (Source Specs / Purpose / Core Concept) の hash が変化すると `stale_dismissal` flag が `true` になる。`/spec-core` 再生成時、`stale_dismissal` になった dismissed item は却下が破棄され、`status="pending"` に戻って再び提示対象になる (再浮上は許容: 「矛盾を解消できない修正をしただけ」と解釈する)。失効はハッシュベース (粗い) であり、矛盾と無関係な軽微な編集でも却下が一旦失効して再判定される。

### 11.6 副作用として行わないこと

- 矛盾を「解決済み」としてマークしない (`resolved` 状態は存在しない)
- 却下以外に矛盾の状態を変えない (片方優先 / 条件分岐 / 課題内一時判断 / 差し戻し / 保留などの操作は存在しない)
- 人間の明示指示を経由せず却下しない (`--dismiss-conflict` は人間が代行 LLM に指示したときだけ実行される)
- 却下や矛盾の内容を Purpose / Core Concept / Source Specs に自動反映しない (反映は人間が直接編集する)
...
| `freshness_report.counts` | 任意 | 診断用。reason ごとの件数 (`required_artifact_problem_count` / `pending_conflict_count` / `watcher_queue_count`) |
| `freshness_report.diagnostics` | 任意 | 診断用。reason ごとの詳細。`failed_required_artifacts[]` / `missing_required_artifacts[]`。内部 field の追加・改名は本書中で影響範囲を明示する |
...
| `flagembedding_missing` | `setup-system` | FlagEmbedding 未導入 |
| `qdrant_client_missing` | `setup-system` | qdrant-client 未導入 |
| `agent_cli_unavailable` | `setup-system` | Agent CLI (codex / claude いずれも) 未導入 |
| `console_script_missing` | `setup-system` | console script 未導入 |
...
| `stale_dismissal` | `core` | dismissed Conflict Review Item の base source hash が変化 |
