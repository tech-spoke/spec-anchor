...
| pending Conflict Review Item | `pending_conflict_items[]` の各項目を §11.4 の構造で提示 (`conflict_id` / `severity` / `claims` / `why_conflicting` / `why_llm_cannot_decide` / `source_refs` / `recommended_next_action`) | freshness の停止理由ではない。Agent は制約情報と併せて提示し、矛盾ではないと人間が明示した場合だけ `spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` を実行 |
...
Conflict 判定は全 SpecClaim pair の総当たりではない。Claim Retrieval が dense retrieval、sparse retrieval、conflict probe retrieval を融合して候補 SpecClaim pair を絞り込み、LLM triage が `send_to_review = true` と判定した pair だけが Conflict Review Item の作成対象になる。上限により送られなかった候補は CoreResult の `diagnostics` に残る。
...
- 却下時、対象 item の claims が指す Source Specs の出典が `referenced_source_refs` として記録され、却下時点の Source Specs / Purpose / Core Concept のハッシュが `base_source_hashes` として記録される。
...
| `resolution` | dismissed 時必須 | 却下記録。`reason` / `referenced_source_refs[]` / human attestation を含む |

### 11.5 stale_dismissal flag の発火条件と再 pending

却下された Conflict Review Item は `base_source_hashes[]` を持つ。却下根拠にした source (Source Specs / Purpose / Core Concept) の hash が変化すると `stale_dismissal` flag が `true` になる。`/spec-core` 再生成時、`stale_dismissal` になった dismissed item は却下が破棄され、`status="pending"` に戻って再び提示対象になる (再浮上は許容: 「矛盾を解消できない修正をしただけ」と解釈する)。失効はハッシュベース (粗い) であり、矛盾と無関係な軽微な編集でも却下が一旦失効して再 triage が走る。
