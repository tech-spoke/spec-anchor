...
- ⑤ 人間判断が必要な仕様の衝突: 未解決の衝突がある。各衝突を人間向けの見出しで本文展開し、人間が採用案を決めるよう促す。衝突の本文展開と却下フローの契約は §6.2 / §7.2 の pending conflict 出力および §11 が定める (本節では展開しない)。
...
| `reopened_dismissal_count` | int | 必須 | 失効した却下が pending に戻され再び提示対象になった項目数 |
...
**未解決の仕様矛盾（Conflict Review Item）:**
> `/spec-inject` は停止しました。解決されていない仕様の矛盾が N 件あります。
>
> 矛盾 1（重要度: 高）
...
> 次のアクション: この矛盾を人間が判断して解決してください
>
> 各矛盾を解決してから `/spec-inject` を再実行してください。制約検索は実行していません。
...
**未解決の仕様矛盾（Conflict Review Item）:**
> `/spec-realign` は停止しました。解決されていない仕様の矛盾が N 件あります。
>
> 矛盾 1（重要度: 高）
...
> 次のアクション: この矛盾を人間が判断して解決してください
>
> 各矛盾を解決してから `/spec-realign` を再実行してください。回答の整形は実行していません。
