...
**回答案が未提示（`/spec-realign` 固有）:**
> `/spec-realign` には回答案が必要ですが、提示されていません。先に `/spec-inject` で制約セットを取得し、それをもとに回答案（今回守る制約 / 修正候補 / 競合・不確実性 / 課題への回答の 4 区分）を作成してから `/spec-realign` を再実行してください。
