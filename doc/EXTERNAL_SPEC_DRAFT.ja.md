...
| field | 型 | 必須 | 意味 |
|---|---|---|---|
| `status` | string | 必須 | `updated` / `failed` / `error`。任意 artifact の部分失敗 (degraded) は top-level status には反映されず、`freshness_report.status` で表現される |
...
CLI が回答候補をまだ受け取っていない状態 (`spec-anchor realign` を回答候補なしで呼んだとき返る `stop_reason="needs_agent_answer"`) は §3.5 の ✕ カテゴリ (非表示) に該当する。これは停止として利用者へ見せない。`/spec-realign` を実行している Agent 自身が §6.2 の 4 path で制約を抽出して回答候補を 4 区分で構成し、`spec-anchor realign --answer-json` を自動的に再実行する。Agent が課題の中心を特定できず回答候補を構成できない場合のみ、上表の「課題の中心を特定できなかった」として利用者へ追加情報を求める。
...
回答候補がまだ構成されていない状態 (`stop_reason="needs_agent_answer"`) は §3.5 の ✕ カテゴリであり、上記の利用者向け停止メッセージに含めない。Agent は 4 path で制約を抽出して回答候補を構成し、自動的に再実行する。回答候補を構成できないのは課題の中心を特定できないときに限り、その場合は次節「Agent が追加確認を求める条件」に従い利用者へ追加情報を求める。
...
- `status="fresh"` で `stop_reason="needs_agent_answer"` → §3.5 の ✕ カテゴリ。利用者へは見せず、Agent が 4 path で回答候補を構成して自動再実行する (課題の中心が特定できないときのみ利用者へ追加確認)
