...
realign = stdout_json("realign.stdout")
...
checks = {
    ...
    "exitcodes": {
        ...
        "realign": exitcode("realign.exitcode"),
    },
    "core_degraded": {
    ...
    "inject_continues_on_degraded": {
        ...
        "should_stop_absent_or_false": inject.get("should_stop") in (None, False),
        "warnings_present": bool(inject.get("warnings")),
        "warning_mentions_degraded_optional": "degraded_optional_artifact" in json.dumps(
            inject.get("warnings") or [],
            ensure_ascii=False,
        ),
        "hits_present": bool(inject.get("hits")),
    },
    "realign_continues_on_degraded": {
        "status_degraded": realign.get("status") == "degraded",
        "should_stop_false": realign.get("should_stop") is False,
        "can_continue_true": realign.get("can_continue") is True,
        "blocking_reasons_degraded_optional": realign.get("blocking_reasons")
        == ["degraded_optional_artifact"],
        "warnings_present": bool(realign.get("warnings")),
        "warning_mentions_degraded_optional": "degraded_optional_artifact" in json.dumps(
            realign.get("warnings") or [],
            ensure_ascii=False,
        ),
        "answer_present": bool(realign.get("answer")),
    },
...
checks["passed_realign_degraded_continue"] = all(checks["realign_continues_on_degraded"].values())
checks["passed_all"] = (
    ...
    and checks["passed_realign_degraded_continue"]
)
