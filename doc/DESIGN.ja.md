...
- [x] §5 / §5.5 / §5.8 / §3.4 から `possible_conflict` field / `conflict_pair_max_per_section` / Related Sections 由来 conflict routing を完全削除する (SCD-032, Phase 5)
  ...
  - **Phase 5 完了 (2026-05-29, 本 commit)**: `git grep -nE "possible_conflict|legacy_related_possible_conflict|legacy_possible_conflict_mode|conflict_pair_max_per_section" spec_anchor tests` で 0 件を確認。本体 7 ファイル (`spec_anchor/config.py` -7 行 / `spec_anchor/conflict_review.py` -265 行 / `spec_anchor/core.py` -154 行 / `spec_anchor/llm_provider.py` -11 行 / `spec_anchor/related_sections.py` -24 行 / `spec_anchor/section_metadata.py` -4 行 / `spec_anchor/templates/.spec-anchor/config.toml` -1 行) と test 7 ファイル (`tests/test_conflict_review.py` -288 行 + 新規 2 test / `tests/test_related_sections.py` -6 行 + 新規 2 test / `tests/test_spec_core.py` / `tests/test_config_keys_acceptance.py` -1 行 / `tests/test_config_loader.py` -2 行 / `tests/test_section_metadata_generation.py` -1 行 / `tests/test_spec_core_acceptance.py` -78 行) で 465 行の本体削除 + 298 行の test 削除。新規 4 test (`test_related_sections_output_has_no_retired_conflict_flag_field` / `test_related_sections_prompt_does_not_request_conflict_judgment` / `test_conflict_review_accepts_only_spec_claim_pair_input` / `test_core_does_not_route_related_sections_to_conflict_review`) で grep 0 件と Phase 5 後の入力境界を検証。`python3 -m pytest -q --skip-external` は 604 passed, 22 skipped, 1 failed (`tests/test_spec_inject.py::test_review_pending_conflict_items_are_loaded_from_real_context_artifact` が `PendingConflictSpecCoreProvider` の旧経路依存で失敗 → `@pytest.mark.skip` で一時 skip + `doc/TODO.ja.md` T-spec-inject-pending-conflict-fixture-update に追従 task 記録)。Phase 1-3 の独立 test (24 件) は影響なし。
...
`reflection_status` は `unreflected | reflected | not_required` の内部 metadata とする。
