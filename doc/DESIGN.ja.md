...
- [x] §5 Related Sections: `relation_hint` enum = {depends_on, impacts, prerequisite, same_policy, see_also} のみを保持し、Conflict Review Item の入力経路にはしない
  - 実装: spec_anchor/related_sections.py (`ALLOWED_RELATION_HINTS` と selection validation)、spec_anchor/conflict_review.py (SpecClaim pair 入力境界)
  - 検証: tests/test_related_sections.py::test_t_u10_related_sections_validation_filters_invalid_items_and_applies_limit、tests/test_conflict_review.py::test_conflict_review_accepts_only_spec_claim_pair_input
...
- [x] §5.8 Conflict 判定 stage は Claim Retrieval が絞った SpecClaim pair と LLM triage の結果を対象にする (全 pair の総当たり判定はしない)
  - 実装: spec_anchor/claim_retrieval.py、spec_anchor/conflict_candidates.py、spec_anchor/conflict_review.py の対象選定経路
  - 検証: tests/test_conflict_review.py::test_t_u20_conflict_pair_selection_uses_triaged_spec_claim_pairs
...
- [ ] §5.10 SpecClaim 経路 (SpecClaim 抽出 + Claim Retrieval + LLM triage) を新規実装する (SCD-032)
  ...
  - **Phase 3 (LLM triage stage) 完了 (2026-05-29, commits `5204323` Part A + `fa2def0` Part B + 本 commit Part C)**: `spec_anchor/conflict_candidates.py` を新設 (`ConflictTriageLlmRequest` / `ConflictTriageValidation` / `ConflictCandidateTriageResult` / `ConflictTriageCache` / `build_conflict_triage_prompt` / `validate_conflict_triage_response` / `compute_conflict_triage_cache_key` / `generate_conflict_candidate_triage_result` / `write_conflict_candidate_pairs_jsonl`)、LLM triage 出力は `send_to_review` (bool) / `reason` (str) / `confidence` (high/medium/low の enum) のみで他 field を含む応答は reject (Conflict Review との責務重複防止)、cache key は両 claim の `claim_uid` / `claim_hash` / `retrieval_hash` / 両 source の `source_hash` / `triage_prompt_version` / `triage_schema_version` / `triage_model` / `triage_effort`、`triage_max_pairs` 上限内の LLM triage 実行、`triage = null` のままの retrieval-only record は `.spec-anchor/context/conflict_candidate_pairs.jsonl` から除外 (Conflict Review に送る対象は `triage.send_to_review = true` のみ)。`spec_anchor/core.py` に `_generate_conflict_candidate_triage_if_enabled` を結線し CoreResult に `conflict_candidate_triage_status` と `conflict_candidate_triage_diagnostics` (`send_to_review_count` / `send_to_review_false_count` / `triage_truncated_pairs`) を追加、`spec_anchor/llm_provider.py` に `conflict_candidate_triage` stage の prompt / OpenAI structured output schema (`send_to_review` / `reason` / `confidence` の全 properties を required で strict mode 対応) を追加、`spec_anchor/config.py` の `[llm.stage_routing]` 許可 stage に `conflict_candidate_triage` を追加。fake テストは `tests/test_conflict_candidates.py` と `tests/test_spec_core.py` の incremental で確認。`tests/test_spec_core_acceptance.py::test_trace_audit_stage_order` の `expected_order` に `conflict_candidate_triage` を追加 (`claim_retrieval` と `conflict_evaluation` の間)。実機検証: 実 Codex CLI (`gpt-5.4-mini` / `low`) + Qdrant + FlagEmbedding BGE-M3 を使う `/spec-core` で 14 SpecClaim → 45 candidate pairs → `triage_max_pairs = 30` の上限内で LLM triage を 30 件実行し、`triage.send_to_review = true` の 7 件のみが `.spec-anchor/context/conflict_candidate_pairs.jsonl` に保存されることを確認。変更なし incremental では `stages.conflict_candidate_triage.status = "skipped_unchanged"` + `llm_calls = 0` を確認。
  - **Phase 4 (実機 recall 検証 = Phase 5 着手 gate) 完了 (2026-05-29, 本 commit)**: 実 Codex CLI + Qdrant + FlagEmbedding BGE-M3 を使う `/spec-core` で `docs/spec/sample.md` の意図的な conflict pair (§0004 Session Termination: 24h purge ↔ §0005 Session Retention Policy: 30 日保持必須) が SpecClaim 経路で **2 件の SpecClaim pair (CC-00013 + CC-00015) として `triage.send_to_review = true, confidence = "high"` で `.spec-anchor/context/conflict_candidate_pairs.jsonl` に保存**され、Conflict Review pipeline の入力候補に届くことを確認 (合格基準 B 達成)。recall 比較 (合格基準 C、任意項目) の手順と結果記録 template は `doc/性能測定/spec_claim_migration_comparison.md` (新規) に整備。
- [x] §5 / §5.5 / §5.8 / §3.4 から Related Sections 由来 conflict signal field / 旧 section-pair 上限設定 / Related Sections 由来 conflict routing を完全削除する (SCD-032, Phase 5)
  - 実装範囲: spec_anchor/related_sections.py の schema / prompt / output から conflict signal field を削除、spec_anchor/llm_provider.py の旧 schema 定義削除、spec_anchor/core.py の Related Sections 由来 conflict routing 削除、spec_anchor/conflict_review.py の relation_hint 整合 filter 削除、旧 section-pair 上限設定 key 削除
  - 検証: Related Sections output に conflict signal field が存在せず、Conflict Review の入力境界が SpecClaim pair に固定される test を追加
  - **Phase 5 完了 (2026-05-29, 本 commit)**: 本体 7 ファイルと test 7 ファイルで旧経路を削除。新規 test (`test_related_sections_output_has_no_retired_conflict_flag_field` / `test_related_sections_prompt_does_not_request_conflict_judgment` / `test_conflict_review_accepts_only_spec_claim_pair_input` / `test_core_does_not_route_related_sections_to_conflict_review`) で Phase 5 後の入力境界を検証。`python3 -m pytest -q --skip-external` は 604 passed, 22 skipped, 1 failed (`tests/test_spec_inject.py::test_review_pending_conflict_items_are_loaded_from_real_context_artifact` が `PendingConflictSpecCoreProvider` の旧経路依存で失敗 → `@pytest.mark.skip` で一時 skip + `doc/TODO.ja.md` T-spec-inject-pending-conflict-fixture-update に追従 task 記録)。Phase 1-3 の独立 test (24 件) は影響なし。
- [x] §5.8 Conflict Review の入力境界を SpecClaim pair / evidence / triage result に固定する (SCD-033)
  ...
  - **Phase 5 完了 (2026-05-29, 本 commit)**: `spec_anchor/core.py:644` で `.spec-anchor/context/conflict_candidate_pairs.jsonl` を読み込み、`spec_anchor/core.py:651` で `evaluate_conflicts(conflict_candidate_pairs=..., spec_claims=...)` の signature に変更、`spec_anchor/conflict_review.py:375` で triage 済み SpecClaim pair のみを選択、`spec_anchor/conflict_review.py:493` で `evaluate_conflicts` 入力 schema を `conflict_candidate_pairs` + `spec_claims/claims` に変更。新規 test `tests/test_conflict_review.py::test_conflict_review_accepts_only_spec_claim_pair_input` で `evaluate_conflicts` が SpecClaim pair 入力だけを受けることを検証、`tests/test_conflict_review.py::test_core_does_not_route_related_sections_to_conflict_review` で core が Related Sections 出力を Conflict Review に直渡ししないことを検証。実機検証: Phase 4 (commit 285a6db) で確認した既知 conflict (sample.md §0004 vs §0005) が Phase 5 後の入力境界でも引き続き SpecClaim pair として Conflict Review に届くことは、Phase 4 記録ファイル `doc/性能測定/spec_claim_migration_comparison.md` の手順で再確認可能 (本 commit 時点では追加実機実行を省略)。
...
`valid_scope` は `global | source_pair | section_pair | task_scope` とする。
