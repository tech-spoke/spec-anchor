...
**【2026-04-28 改訂】案 A 破棄、Phase 1 入り口で方向 1 / 方向 2 を再評価**:
...
- **本 §1.4 の「Python + LlamaIndex 系エコシステム採用」は方向確定後に再記述する**:
  - 方向 1（GRAG をちゃんと使う）採用 → 本記述を維持、SchemaLLMPathExtractor / kg_extractors を活用する設計に書き換え
  - 方向 2（制御性最優先）採用 → 「Python + 自前 canonical graph + optional LlamaIndex adapter」に書き換え
- 上記の落とし穴対応（`safe_delete_by_section` / `kg_extractors=[ImplicitPathExtractor()]` / `load_index_from_storage` 不使用）は **方向 1 採用時のみ適用**。方向 2 採用なら graph store / index を自前管理するため、これらの落とし穴自体が存在しない
- PGRetriever fusion / 4 軸付与を Orchestrator 側でやる原則、Claude `--bare` 不使用は **方向 1 / 方向 2 共通**
- Phase 1 入り口で方向 1 / 方向 2 を SURVEY/SUMMARY.md §3.8 の波及先発見品質評価軸 5 観点で対称比較し、確定後に §1.1〜§1.9 / §2 / §3 / §4 を再記述する
