...
`apply_conflict_decision` は status を `dismissed` に変更する場合、`decision_payload.human_acknowledgement = true` を必須とする。EXTERNAL_DESIGN §2.8 / §7.2.2 に従い、却下は人間の判断であることを caller が明示的に attest する。Agent / LLM が独断で dismiss を実行する経路はサポートしない。却下以外に矛盾の状態を変える decision は存在しない (`DECISIONS = {"dismiss"}`)。
...
`apply_conflict_decision` が受ける `decision` は次の 1 値だけである (`DECISIONS = {"dismiss"}`)。矛盾を「解決済み」にする decision (`prefer_a` / `prefer_b` / `conditional` / `task_scope_resolution` / `needs_source_update` / `defer`) は廃止済みで存在しない。
