...
Section ペア候補生成の処理量制御は `[conflict_candidate_detection]` の `small_section_all_pairs_threshold`、`section_pair_top_k`、`global_pair_cap`、`min_dense_score`、`allow_same_source_file_pair`、`allow_same_section_pair` で行う。上限により候補が切られた場合は CoreResult の `diagnostics.section_pair_candidate_generation.truncated_count` に残る。
...
Section Pair Conflict Detection 専用の上限は `[conflict_candidate_detection]` で管理する (`small_section_all_pairs_threshold = 12` / `section_pair_top_k = 8` / `global_pair_cap = 80` / `min_dense_score = 0.55` 等)。詳細は `doc/EXTERNAL_DESIGN.ja.md` §10.2 を参照。
...
```text
all_pairs       # Section 数が small_section_all_pairs_threshold 以下
retrieval_cap   # Section 数が threshold を超える場合
```

`all_pairs` mode は Section の i<j 組み合わせを列挙する。`retrieval_cap` mode は section-level hybrid retriever を使い、各 Section から `section_pair_top_k` 件まで近傍 Section を残し、`global_pair_cap` で通常候補数を制限する。`allow_same_source_file_pair = false` の場合、同じ Source Specs file 内の Section ペアは候補から除く。
