...
- [x] §5 Related Sections: `relation_hint` enum = {depends_on, impacts, prerequisite, same_policy, see_also} のみを保持し、Conflict Review Item の入力経路にはしない
  - 実装: spec_anchor/related_sections.py (`ALLOWED_RELATION_HINTS` と selection validation)、spec_anchor/conflict_review.py (`evaluate_section_pair_conflicts` の Section ペア入力境界)
  - 検証: tests/test_related_sections.py::test_t_u10_related_sections_validation_filters_invalid_items_and_applies_limit、tests/test_conflict_review.py::test_evaluate_section_pair_conflicts_pending_item_with_conflict_points
...
- [x] §5.8 Conflict 判定 stage は Section ペア候補生成が返す Section ペアを対象にする。候補生成は LLM を呼ばず、Conflict Review judge が唯一の LLM 判定である
  - 実装: spec_anchor/section_pair_candidates.py、spec_anchor/conflict_review.py、spec_anchor/core.py の section_pair_candidate_generation 経路
  - 検証: tests/test_section_pair_candidates.py、tests/test_conflict_review.py::test_core_does_not_route_related_sections_to_conflict_review
- [x] §9 Freshness: pending Conflict Review Item は `/spec-inject` / `/spec-realign` の保持物鮮度 blocker にしない
  - 実装: spec_anchor/freshness.py、spec_anchor/inject.py、spec_anchor/conflict_review.py
  - 検証: tests/test_freshness.py::test_t_u03_pending_conflict_does_not_block
...
- [x] §8 `/spec-inject` CLI 拡張 (`inject-search` / `inject-section` / `inject-chapters` / `inject-purpose`)
  - 実装: Phase R-6 で spec_anchor/inject.py に `run_inject_search` / `run_inject_section` / `run_inject_chapters` / `run_inject_purpose` を追加、spec_anchor/cli.py に対応する subparser + dispatcher を追加。Qdrant / FlagEmbedding 不可時は structured warning fallback。F-C 採用後、各 inject-* の冒頭に freshness gate を組み込み、gate probe 専用 subcommand (`spec-anchor inject`) は撤去。矛盾解決軽量化後の Agentic Search は 3 path である
  - 検証: tests/test_inject_cli_extension.py
- [x] §5.10 Section Pair Conflict Detection は候補 artifact を作らず、full run または変更がある incremental run ごとに候補をメモリ上で作り直す
  - 実装: spec_anchor/section_pair_candidates.py (`generate_section_pair_candidates`)、spec_anchor/core.py の section_pair_candidate_generation 経路
  - 検証: tests/test_section_pair_candidates.py、tests/test_spec_core_acceptance.py::test_trace_audit_stage_order
- [x] §5.8 Conflict Review Item は Section ペア ID を `conflict_id` に使い、本文の対比は `conflict_points[]` で保持する
  - 実装: spec_anchor/conflict_review.py (`section_pair_id`、`_build_section_pair_conflict_item`、`CONFLICT_REVIEW_ITEM_FIELDS`)
  - 検証: tests/test_conflict_review.py::test_build_section_pair_conflict_item_matches_schema_and_validates、tests/test_conflict_review.py::test_evaluate_section_pair_conflicts_pending_item_with_conflict_points
...
`/spec-core` は `[llm.providers.<id>]` 設定の command / model / effort / timeout_sec / max_retries を使って、Section Summary、Section Search Keys、Related Sections の選定、Conflict Review (LLM が既存根拠で解消できない conflict の判定)、Chapter Key Anchor を生成・実行する。Section ペア候補生成は `generate_section_pair_candidates` が LLM を呼ばずに行い、Conflict Review judge だけが Section ペア conflict path の LLM stage である。`SPEC_ANCHOR_FAKE_LLM` env var が truthy のときは provider 設定を無視して in-process FakeLlmProvider を使う (test / smoke 専用)。`SPEC_ANCHOR_FAKE_RETRIEVAL` が truthy のときは Qdrant + FlagEmbedding の実構築を block する (`allow_real_provider=True` 経路を除く)。
...
Section ペア候補生成は Source Specs の section を対象にし、小規模では全 Section ペア、大規模では section-level retrieval と上限設定で絞った Section ペアを作る。候補は永続化せず、full run または変更がある incremental run のたびにメモリ上で作り直す。Related Sections の生成とは独立に実行され、Related Sections 由来 pair や Related Sections として採用されなかった「高リスク候補」を Conflict Review に送る経路は持たない。

stage 単位での model / effort 切替は `[llm.stage_routing]` で指定する。許可される stage key は `section_metadata` / `related_sections` / `conflict_review` / `chapter_key_anchor` の 4 つ。Section ペア候補生成は LLM を呼ばないため stage_routing 対象外。stage_routing で指定された provider が `[llm.providers.<id>]` に存在しない場合、または stage key が許可外の場合は `ConfigError` で reject する。stage_routing 未指定の stage は `[llm.providers.<id>]` の先頭定義へフォールバックする。`select_llm_provider_config(stage=...)` の解決優先順は `provider_id (CLI 引数 / env)` → `stage_routing[stage]` → `[llm.providers.<id>]` の先頭定義の順である。

Section ペア候補生成の処理量制御は `[conflict_candidate_detection]` の `conflict_candidate_mode`、`section_pair_top_k`、`global_pair_cap`、`min_dense_score`、`allow_same_source_file_pair`、`allow_same_section_pair` で行う(budget-first: cross-pair 総数が `global_pair_cap` 以下なら全件 judge、超なら retrieval で上位だけ)。上限により候補が切られた場合は CoreResult の `diagnostics.section_pair_candidate_generation.truncated_count` に残る。

変更のない incremental run では、`core.py` が Section hash と Purpose / Core Concept hash の変化なしを検出し、`section_pair_candidate_generation_status = "skipped_unchanged"` として Section ペア候補生成と Conflict Review judge を両方スキップする。既存の Conflict Review Items は保持される。
...
Section Pair Conflict Detection 専用の上限は `[conflict_candidate_detection]` で管理する (`conflict_candidate_mode = "budget"` / `section_pair_top_k = 8` / `global_pair_cap = 80` / `min_dense_score = 0.55` / `judge_batch_size = 5` 等)。詳細は `doc/EXTERNAL_DESIGN.ja.md` §10.2 を参照。
...
`conflicts_with` は本 stage の出力 enum から **削除** されている。Related Sections は conflict 判定を持たない。仕様上の矛盾候補抽出は Section Pair Conflict Detection (§5.10) が独立して扱う。これにより Related Sections の軽量な分類タスクと厳密な矛盾判定の evidence 厳密度を分離し、Related Sections のモデル選択が conflict recall を左右しない構造にする。
...
Section ペア候補生成で選ばれた Section ペアについて、CLI は両 Section の本文、出典、関連する Purpose / Core Concept を LLM に渡して Conflict Review judge を実行する。Related Sections の `relation_hint` や旧 Related Sections conflict referral 由来 pair は本 stage の入力にしない。Conflict Review judge は Section Pair Conflict Detection で唯一の LLM call で、Purpose / Core Concept / Source Specs grounding を必須とする。

`conflict_review.evaluate_section_pair_conflicts` は `section_pairs` と `conflict_judge` callable を引数として受け取る薄い orchestration である。Purpose / Core Concept / Source Specs を実際に grounding として LLM に渡す責務は呼び出し側が用意する judge 実装が担う。core.py の通常経路では `_EvidenceGroundedConflictJudge` ラッパーが Purpose と Core Concept のテキストとハッシュを毎 request に付与する。`evaluate_section_pair_conflicts` を単体で呼ぶ caller (テストや外部ツール) も同等の grounding を持つ judge を渡す責任を負う。

Source Specs grounding は section_a / section_b の本文を judge prompt に含める形で行う。周辺 section や全 corpus を judge に渡すことはしない。judge が追加 context を必要とする場合は、`evidence_terms` を経由した snippet 抽出のみを使い、Source Specs 全文の盲目的注入 (EXTERNAL_DESIGN §3.4 の生テキスト投入制限に反する) は行わない。

`apply_conflict_dismissal` は status を `dismissed` に変更する場合、`human_acknowledgement = true` を必須とする。EXTERNAL_DESIGN §2.8 / §7.2.2 に従い、却下は人間の判断であることを caller が明示的に attest する。Agent / LLM が独断で dismiss を実行する経路はサポートしない。却下以外に矛盾の状態を変える CLI 経路は存在しない。
...
Conflict Review Item の `status = pending` は freshness の停止理由を作らない。pending な矛盾は `pending_conflict_items` として情報返却され、freshness report の `blocking_reasons[]` には矛盾起因の理由を積まない。課題に関係する矛盾を提示してから停止するのは Agent 側挙動 (EXTERNAL_DESIGN §8.6) である。

`dismissed` になった item は、却下理由・参照 source refs・human attestation を resolution に保持する。`status` は `pending` / `dismissed` の 2 値だけで、矛盾を「解決済み」にする状態 (`resolved`) は存在しない。

`base_source_hashes` は却下時に参照した Purpose、Core Concept、Source Specs の hash を保持する。却下根拠 source が変わった場合、dismissed item は `stale_dismissal = true` になり、`/spec-core` 再生成で却下が破棄され `status="pending"` に戻る (再 pending)。

現行の自動検出で作る item は `valid_scope = "section_pair"` とする。

却下は `spec-anchor core --dismiss-conflict <conflict_id> --reason "..."` の 1 経路で受ける。外部 slash command は増やさず、Agent / LLM が人間の却下指示を CLI フラグに渡す。

`apply_conflict_dismissal` が受ける操作は `dismiss` だけである。矛盾を「解決済み」にする構造化判断や、仕様更新予定・保留を Conflict Review Item の状態として保存する経路は廃止済みで存在しない。
...
### 5.10 Section Pair Conflict Detection

`spec_anchor.section_pair_candidates.generate_section_pair_candidates` は、Conflict Review judge に渡す Section ペア候補を返す。戻り値は `SectionPairCandidateResult(candidates, diagnostics)` で、`candidates` は `{left_section_id, right_section_id, candidate_origin}` の list、`diagnostics` は `section_count` / `mode` / `generated_count` / `truncated_count` / `recheck_count` を持つ。

候補生成 mode (budget-first。Section 数による段差は無い):

```text
all_pairs       # cross-pair 総数 <= global_pair_cap (conflict_candidate_mode="budget")、
                #   または conflict_candidate_mode="exhaustive"
retrieval_cap   # cross-pair 総数 > global_pair_cap (budget)
```

mode 決定は `conflict_candidate_mode`(既定 `"budget"`)と `global_pair_cap` で行う。`budget` では cross-pair 総数(`allow_same_source_file_pair=false` の除外後)が `global_pair_cap` 以下なら `all_pairs`、超なら `retrieval_cap`。`exhaustive`(検証用)は cap を無視して常に `all_pairs`。`all_pairs` mode は Section の i<j 組み合わせを列挙する。`retrieval_cap` mode は section-level hybrid retriever を使い、各 Section から `section_pair_top_k` 件まで近傍 Section を残し、`global_pair_cap` で通常候補数を制限する。`allow_same_source_file_pair = false` の場合、同じ Source Specs file 内の Section ペアは候補から除く。

`allow_same_section_pair = true` の場合、各 Section の self-pair を cap-exempt として加える。既存 Conflict Review Item の `base_source_hashes[]` と現在の section hash が一致しない場合、その Section ペアは `existing_conflict_recheck` として cap-exempt で再判定対象に加える。

候補は保存しない。`core.py` は full run または変更がある incremental run で `generate_section_pair_candidates` を呼び、直後に `evaluate_section_pair_conflicts(cand.candidates, ...)` を呼ぶ。変更のない incremental run では `section_pair_candidate_generation_status = "skipped_unchanged"` とし、候補生成と judge を両方スキップする。

`candidate_origin` の現行値:
...
generate section_pair candidates in memory
run section_pair conflict judge
update Conflict Review Items from unresolved section_pair judge results
...
slash command は Agent / LLM に探索手順を指示する。CLI は次の参照操作を提供する。`evidence_origin` enum (Purpose / Core Concept / Source Specs) を 3 path がカバーする。各 `inject-*` および `realign` は内部で freshness gate / watcher gate を通すので、Agent が事前 probe を呼ぶ必要はない (F-C 採用後の構造)。pending Conflict Review Item は freshness の停止理由ではなく、CLI が情報として返し、Agent が課題関連性を判断して提示する。
...
Agent / LLM はこれらを使い、課題の性質に応じて 3 path を組み合わせる。
...
```text
status: fresh | blocked | failed
blocking_reasons[]
warnings[]
```

status は `STATUSES = {fresh, blocked, failed}` の 3 値。`degraded` status は廃止済みで、section_metadata の部分失敗も `failed_required_artifact` に畳む。`can_continue` は `status == fresh` のときだけ true。

`blocking_reasons[]` は次の 5 値だけで、表示優先は次の順にする。pending な矛盾は停止理由ではなく注入情報なので、ここには積まれない。
...
`/spec-inject` と `/spec-realign` は、`status != fresh` の場合に自動更新しない。`blocking_reasons[]` に dirty / stale / watcher 系理由がある場合は `/spec-core` または watcher が先に保持物を更新する。pending な矛盾は freshness を blocked にしないが、課題に関係する矛盾は Agent が制約とともに提示してから停止する (EXTERNAL_DESIGN §8.6)。
