...
ResultEnvelope:
  ...
  result_type: CoreResult | InjectionContext | RealignResult | ConceptApprovalRequiredResult | NeedMoreContextResult | ErrorResult
  payload: object
  execution:
...
経路 3: /spec-inject（GRAG を信じすぎず、Agentic search 併用）
  ...
  2. Orchestrator: Concept diff が未承認なら InjectionContext を生成せず、
     ConceptApprovalRequiredResult を返して停止:
       task_prompt / CoreResult.concept_diff / required_action: accept|reject|revise / warnings
     （EXTERNAL_DESIGN.ja.md §5.3: 未承認時は確認要求だけを出力する）
     - これはデッドロックではなく安全停止点である。ユーザーはチャット上で
       Concept を手動修正するか、accept / reject / revise 相当の判断を行い、
       その後同じコマンドを再実行する
...
4. rejected / pending hunk は適用しない
5. revised hunk は revision_instruction に基づき再生成し、再度 hunk 承認に戻す
