...
- production full build / approval / E2E:
  ...
  - `spec-inject`: exit 0、elapsed `0:51.04`、`status=degraded`、`context_ready=true`、`llm_calls=9`、warning `classification_incomplete`
  - `spec-realign`: exit 0、elapsed `99.899s`、`status=degraded`、`context_ready=true`、`llm_calls=10`、answer 生成まで完了、warning `classification_incomplete`
  - finding: production E2E は動くが、classification budget 8 件を超えて degraded になる。品質監査では classification budget / item prioritization / query planner latency を重点確認する
...
- [ ] Phase 14 後の E2E 判定を反映する
  - 対象: C / Full Build 実施メモの `spec-inject` / `spec-realign`
  - 見直し: `classification.max_items=8` 起因の古い `RISK` は stale。Phase 14 後は `classification_medium_priority_incomplete` 残りとして再判定する
...
- [ ] medium / low priority incomplete の production policy を決める
  - 選択肢: degraded 維持、warning のみ、answer 使用直前の deferred classification
...
| injection | `spec-inject` | InjectionContext | `2026-05-02T052715Z` run / wall `0:51.04` | RISK | `context_ready=true` だが `classification_incomplete` で degraded |
| realign | `spec-realign` | RealignResult または NeedMoreContext | `2026-05-02T052918Z` run / wall `99.899s` | RISK | answer 生成完了。`classification_incomplete` で degraded |
...
- [!] production E2E `spec-inject`
  ...
  - finding: `classification_budget = 8`、skipped 17、warning `classification_incomplete`
...
- [!] production E2E `spec-realign`
  ...
  - finding: `classification_budget = 8`、skipped 19、warning `classification_incomplete`
...
| 2026-05-02 | production inject / realign E2E | RISK。`spec-inject` 51.04s、`spec-realign` 99.899s。どちらも `context_ready=true` だが `classification_incomplete` で degraded | classification budget 8 件で source / concept / chapter / cluster の一部が未分類 | budget設定、優先順位、分類cache粒度、degraded時のanswer policyを確認 |
