...
- [x] medium / low priority incomplete の production policy を決める
  ...
  - deferred classification: answer 使用直前に実際に使う medium / low 候補だけ追加分類する案は有効だが、別実装タスクとして残す
...
| 2026-05-02 | retrieval query set / type budget 監査 | OK/PARTIAL。5 query の初回評価で Source evidence は概ね妥当。`Concept にないが Source specs にある制約` で tier 0 graph entity が type budget で落ちる不具合を修正し、再実行で high priority skipped 0 | medium incomplete は degraded 維持。Concept 巨大 chunk 混入、query planner 語過多、answer 40秒台は残る | QueryPlan cache / Concept chunking / deferred classification を検討 |
