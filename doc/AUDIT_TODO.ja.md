...
- [ ] Phase 15 architecture audit hardening を進める
  ...
  - 2026-05-03 実装済み: atomic write 共通化、LLM factory 共通化、priority constants、`tests/conftest.py`、`safe_delete_by_sections`、optional numpy dense similarity、`[logging]` config / watcher file logging、inject / realign 共通 pipeline 抽出
  - 先行残: `injection.py` concern 分割、`core.py` stage 分割、watcher / foreground lock 方針監査、BM25 broad candidate 追加監査、Concept index incremental embedding reuse
  - 検証: focused regression `32 passed in 15.23s`、追加 focused regression `67 passed in 69.05s`、full regression `243 passed in 210.62s`
