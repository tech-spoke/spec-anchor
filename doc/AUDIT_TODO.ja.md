...
- [ ] Phase 15 architecture audit hardening を進める
  ...
  - 2026-05-03 実装済み: atomic write 共通化、LLM factory 共通化、priority constants、`tests/conftest.py`、`safe_delete_by_sections`、optional numpy dense similarity、Concept index embedding reuse、BM25 strong-term candidate prune、`[logging]` config / watcher file logging、foreground core update の `WatchLock` 共有、inject / realign 共通 pipeline 抽出
  - 先行残: `injection.py` concern 分割、`core.py` stage 分割、query set 5本での BM25 broad candidate 再実測
  - 検証: focused regression `32 passed in 15.23s`、追加 focused regression `67 passed in 69.05s`、Concept / retrieval focused `19 passed in 12.27s`、CLI focused `26 passed in 59.47s`、full regression `246 passed in 201.81s`
