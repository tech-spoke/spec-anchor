...
### 第14回・min_dense_score キャリブレーション + BGE-M3 load 回数 再確認（follow-up）

測定日: 2026-05-31　目的: 旧 `#2`（detection_pipeline_simplify、close 済み）の非ブロッキング follow-up（`min_dense_score` 実 Qdrant キャリブレーション / `#9-s10` BGE-M3 load 回数再確認）を実施。

#### 計測条件

`docs/spec/sample.md`（6 section）は all_pairs mode で `min_dense_score` を**行使しない**（dense gate は retrieval_cap mode でのみ効く）。そこで temp project（`テスト用ドキュメント/` の `29_振る舞い層API一覧`(8 sec) + `30_テスト用矛盾例`(5 sec・意図的矛盾 fixture) + `23_フィールドパーツ一覧`(8 sec) = 21 section、別 Qdrant collection）で retrieval_cap mode を踏ませ、候補生成の `QdrantHybridRetriever.search` 全 hit の dense cosine を instrument 計測した。conflict judge は fake 化（候補生成は judge 前に走るため dense gate 評価に影響しない）。section_metadata は実 LLM 生成で production 同等の metadata 込み埋め込みを使用。

#### dense_score 分布（441 hit）

| min | p25 | median | p75 | max |
|---|---|---|---|---|
| 0.438 | 0.580 | 0.657 | 0.721 | 1.000 |

`min_dense_score=0.55` で keep=360 / drop=81（drop は dense が非 None かつ <0.55 のみ。**sparse-only hit（dense_score=None）は gate せず keep** し recall を保護）。同一プロジェクト仕様で全 section が意味的に近く、dense baseline が高い（median 0.66）。

#### 既知矛盾ペアの recall（30_矛盾例 ↔ 29_API）

第8回で検出された意図的矛盾（30 が 29 の同名 API と矛盾）について、30 の各 section が対応 29 section を dense gate 通過で候補化できるか:

| 30_矛盾例 source | 29_API target | dense_score（2 pass） | 0.55 判定 |
|---|---|---|---|
| Store 初期値の変更禁止 | 1. Store | 0.65 / 0.69 | KEEP ✓ |
| StoreGroup 全体差し替えの禁止 | 2. StoreGroup | 0.68 / 0.67 | KEEP ✓ |
| Action 全体差し替えの必須化 | 5. Action / 全体操作 | 0.62 / 0.67 | KEEP ✓ |

3 主要矛盾ペアは全て 0.55 を上回り候補化（第8回の検出 recall と整合）。ノイズ（`23_フィールドパーツ一覧` ↔ API/矛盾）は 0.44〜0.54 で大半 drop。

#### 判定: `min_dense_score = 0.55` は妥当（default 維持）

- 既知の真矛盾ペアを keep し、無関係ノイズを drop できている。
- ただし dense baseline が高いドメインでは 0.55 は分布の下位中央に位置し、弱い対応（例: bindContext 系）は 0.54 付近で borderline。コーパスの意味的広がりで最適値が動くため、`[conflict_candidate_detection].min_dense_score` を config で per-project 調整可能にしてある。現状の値で本サンプル + テスト用矛盾 fixture は recall 維持。
- sparse-only hit は gate しないため語彙一致のみのペアも候補化され recall を底上げ。
- 限界: 本計測は候補生成のみ（judge fake）。「候補化された矛盾が judge で実際に pending になるか」は別。大規模実コーパスで recall 不足が観測されれば閾値再調整する。

#### BGE-M3 load 回数（#9-s10 再確認）

同 probe で all_pairs（sample 6 sec）/ retrieval_cap（21 sec）とも BGE-M3 weight load = **1 回**（shared provider が候補生成にも注入）。詳細は `tests/e2e/snapshots/#9-s10_flagembedding_load_count_real_run.md`「2026-05-31 再計測」節。
