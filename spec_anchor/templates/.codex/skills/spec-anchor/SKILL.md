...
core 実行後は、後続作業に必要な CoreResult field を報告する: `updated_sources`, `failed_sources`, `failed_sections`, `retrieval_index_status`, `freshness_report`, `pending_conflict_count`, `conflict_review_items`, `stale_dismissal_count`。ユーザーが workflow として依頼していない限り、core の一部として inject / realign を実行しない。
