...
* `.spec-anchor/context/`  — human-facing read API (chapter_anchors,
  conflict_review_items)
* `.spec-anchor/state/`    — `/spec-core` execution state (section_manifest,
...
# Artifact names that live in `.spec-anchor/state/` rather than
# `.spec-anchor/context/`.
...
class ContextArtifactStore:
    """Resolve and atomically write the `.spec-anchor/context/` and
    `.spec-anchor/state/` artifacts produced by `/spec-core`.

    `state_dir` defaults to the sibling `.spec-anchor/state/` directory of the
...
def build_section_manifest(sections: list[Any]) -> dict[str, Any]:
    ...
    is ``spec_anchor.core._section_manifest_entry`` which adds audit
