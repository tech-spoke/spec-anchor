"""Persist `/spec-core` stage progress to `.spec-anchor/state/core_progress.json`.

This module makes `spec-anchor core` runs observable when they take long or
...
def progress_file_path(root: Path | str) -> Path:
    return Path(root) / ".spec-anchor" / "state" / PROGRESS_FILENAME
