"""Polling watcher for lightweight SPEC-anchor projects."""
...
from spec_anchor.core import run_spec_core_for_watcher
...
from spec_anchor.freshness import (
...
DEFAULT_STATE_FILE = ".spec-anchor/state/watch_state.json"
DEFAULT_QUEUE_FILE = ".spec-anchor/state/watch_queue.json"
SCHEMA_VERSION = 1
...
def run_watcher_once(
    project_root: str | Path = ".",
    *,
    # SIGTERM handling: the `spec-anchor-watch` entry point in cli.py:watch_main()
...
def run_watcher_cycle(
    ...
    **kwargs: Any,
    ...
    """Scan once and run at most one internal SPEC-anchor core update."""
    ...
    lock_attempt = acquire_core_update_lock(
        ...
        stale_lock_ms=int(settings.stale_lock_sec * 1000),
        now_ms=now_ms,
        metadata={"entrypoint": "spec-anchor-watch"},
    )
...
def load_watcher_settings(
    ...
    *,
    ...
    context_value = _optional_str(context, "context", "storage") or ".spec-anchor/context"
...
run_watch = run_spec_anchor_watch
watch = run_spec_anchor_watch
spec_anchor_watch = run_spec_anchor_watch
run_watch_once = run_watcher_once
watch_once = run_watcher_once
run_watcher = run_spec_anchor_watch
