"""Source Retrieval Index primitives for lightweight SPEC-anchor.
...
default `spec_anchor_section`). 1 section = 1 vector.
...
DEFAULT_SECTION_COLLECTION = "spec_anchor_section"
...
class FlagEmbeddingBgeM3Provider:
    ...
    weights. When the env var ``SPEC_ANCHOR_FAKE_RETRIEVAL`` is truthy
        ...
        **model_kwargs: Any,
        ...
        if _fake_retrieval_enabled() and not allow_real_provider:
            raise RuntimeError(
                ...
                "SPEC_ANCHOR_FAKE_RETRIEVAL is set. Pass allow_real_provider=True "
                "for the standard /spec-core path, or unset SPEC_ANCHOR_FAKE_RETRIEVAL "
...
def _fake_retrieval_enabled() -> bool:
    return os.environ.get("SPEC_ANCHOR_FAKE_RETRIEVAL", "").lower() in _TRUE_VALUES
