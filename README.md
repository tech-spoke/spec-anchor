...
SPEC-anchor is a lightweight specification context tool. SPEC-anchor は、LLM が作業中に次を見失わないよう支援する軽量な仕様コンテキストツールである。
...
Purpose と Core Concept は人間が維持する。SPEC-anchor does not update them automatically.
...
| LLM (`/spec-core` 用) | `.spec-anchor/config.toml` の `[llm]` で設定。Agent 側 LLM とは別 |

Ollama は標準の sparse-vector 経路ではない。

## 環境構築

### 1. SPEC-anchor 本体と retrieval 依存のインストール
...
`spec-anchor core` / `/spec-inject` / `/spec-realign` が Qdrant に依存するため、log-in セッションで自動起動・自動再起動するように systemd ユーザーサービス化するのが扱いやすい。手動 `qdrant --disable-telemetry` を毎回起動するより事故が少ない。

```bash
# 永続化される storage / runtime ディレクトリを先に作る
mkdir -p ~/.local/share/spec-anchor/qdrant/storage ~/.local/share/spec-anchor/qdrant/runtime

# ユーザーサービス unit を作成
mkdir -p ~/.config/systemd/user
cat > ~/.config/systemd/user/qdrant.service <<'UNIT'
[Unit]
Description=Qdrant Vector Database (for spec-anchor)
...
Environment=QDRANT__STORAGE__STORAGE_PATH=%h/.local/share/spec-anchor/qdrant/storage
ExecStartPre=/bin/mkdir -p %h/.local/share/spec-anchor/qdrant/storage %h/.local/share/spec-anchor/qdrant/runtime
WorkingDirectory=%h/.local/share/spec-anchor/qdrant/runtime
...
```bash
mkdir -p ~/.local/share/spec-anchor/qdrant/storage
QDRANT__SERVICE__HTTP_PORT=6333 \
QDRANT__STORAGE__STORAGE_PATH=~/.local/share/spec-anchor/qdrant/storage \
...
```bash
spec-anchor --help
spec-anchor-setup-system --check-only
```

`spec-anchor-setup-system --check-only` の出力で、Qdrant / FlagEmbedding / Agent CLI の導入状態と不足要素を確認できる。

## プロジェクトへの導入

```bash
# 対象プロジェクトにセットアップ（Claude Code + Codex 両対応）
spec-anchor-setup-project --target /path/to/project --agent both

# コンテキスト artifact を初回生成
cd /path/to/project
# 必要に応じて .spec-anchor/config.toml の [vector_store].url を実 Qdrant URL にする
...
- Codex 用: `~/.codex/skills/spec-anchor/SKILL.md`（user install、既定）
...
セットアップ後、Agent 環境に配置された入口（Claude Code の `/spec-core`、Codex の spec-anchor skill）から利用できる。
...
`/spec-core --all` の section_metadata / related_sections 生成は batch ごとに LLM を呼ぶ。`.spec-anchor/config.toml` の `[limits].llm_batch_concurrency` で同時実行数を指定できる:
...
| `spec-anchor-setup-system --check-only` | SPEC-anchor 本体の導入状態と外部依存を確認する |
...
| CLI / SPEC-anchor | 設定読込、section hash / freshness 管理、保持物生成、検索 API、参照 API。探索方針や最終回答は決めない |

CLI は探索や回答の主体にならない。Agent / LLM が CLI の提供する検索結果と保持物を使って判断する。

## Agent 入口

`spec-anchor-setup-project` は Agent 環境ごとに対応する形式の入口を配置する。

| Agent 環境 | 形式 | 配置先 |
|---|---|---|
| Claude Code | command | `<project>/.claude/commands/spec-{core,inject,realign}.md` |
| Codex CLI | skill | `<project>/.codex/skills/spec-anchor/SKILL.md` |
