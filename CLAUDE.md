...
4. `doc/TODO.ja.md` - 開放 task 一覧。実装着手前に該当 task の検証条件・scope・依存を確認する
...
**2 回目** (B-4 task): その後 B-4 task 1 つだけを Codex に投げた時も、約 12 分で同じ症状 (forwarder completed・Codex 本体 kill・Final report 不在) が再発した。Claude main は監査で逸脱 2 ファイル (`spec_grag/retrieval_index.py` / `spec_grag/section_payload.py` への scope 外 fallback shim 追加。CODEX が pytest を venv 外で実行して見た幻の 8 件 ImportError を「修正」した) を検出し revert した。同時に、Bash tool のタイムアウトを「仕様上 600000ms = 10 分が hard cap、変更不可」と誤認していたことも user 指摘で発覚した。
