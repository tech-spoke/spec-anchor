...
### ルール 15: 外部設計書はソースコードを読んだことがない読者に通じる言葉で書く
...
- 通じる → 外部設計書に書いてよい
- 通じない → 内部設計書 (`doc/DESIGN.ja.md`) に書く。外部設計書では読者が体験する動作 (何が起きるか、何を見るか、何をすればよいか) で書き直す
...
外部設計書の書き換え時、契約内容 (出力構造、field 名、設定項目、エラー条件、判断肢) を削除・変更してはいけない。書き換えてよいのは、同じ契約内容を内部用語から外部用語へ表現し直すことだけである。契約内容の追加・削除・変更はユーザーの明示指示が必要。
