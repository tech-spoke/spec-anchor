"""Project config loader for lightweight SPEC-grag."""
...
def load_config(
    ...
    *,
    ...
    try:
        ...
        raise ConfigError(f"TOML parse error in .spec-grag/config.toml: {exc}") from exc
...
def _load_context(root: Path, raw_value: Any) -> ContextConfig:
    ...
    storage = _optional_str(table, "context", "storage") or ".spec-grag/context"
