...
6. CLI で検証する: `spec-grag inject --constraints '<json-array>'`。CLI は Agent-generated constraints を検証するだけで、fallback constraints を生成しない。検証に失敗した場合、constraints を直すか blocker として報告する。
...
4. CLI で検証する: `spec-grag realign --constraints '<json-array>' --answer-json '<json-object>'`。返却 payload を constraint-checked answer として扱う。検証に失敗した場合、answer を直すか blocker として報告する。
