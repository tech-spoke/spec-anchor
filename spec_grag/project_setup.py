"""Setup helpers for SPEC-grag project and system entrypoints."""
...
def run_smoke_checks() -> dict[str, Any]:
    ...
    help_result = subprocess.run(
        [sys.executable, "-m", "spec_grag", "--help"],
        cwd=repo_root,
        text=True,
        capture_output=True,
        check=False,
    ...
    with tempfile.TemporaryDirectory(prefix="spec-grag-smoke-") as tmp:
...
def _template_text(relative_path: str) -> str:
    resource = importlib.resources.files("spec_grag").joinpath(
...
def _core_paths_from_config_template() -> dict[str, Path]:
    raw = tomllib.loads(_template_text(".spec-grag/config.toml"))
...
def _config_text_after_planned_operations(
    ...
    for operation in operations:
        if operation["path"] == ".spec-grag/config.toml":
...
def _purpose_placeholder() -> str:
    return (
        ...
        "\n"
        "SPEC-grag never updates this file automatically.\n"
    )
        ...
        result["diagnostics"].append(
            {
                ...
                "severity": "info",
                "message": (
                    "Codex uses skill entrypoints under <project>/.codex/skills/spec-grag; "
