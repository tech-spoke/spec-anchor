...
> 最新の実装状態・検証結果・次作業は `doc/HANDOFF.ja.md` と `doc/TODO.md` を優先して参照する。本ファイル下部の古いセッション引き継ぎは履歴として読む。
...
| `doc/DESIGN.ja.md` | **現行版（2026-04-29 置換済）**。旧 draft が GPT + Claude レビューを経て DESIGN.ja.md に昇格。4 entity / 6 relation、Orchestrator 側実装、Core Concept index、cluster sidecar、ConceptApprovalRequiredResult、spike fallback ladder を含む |
| `BAK/doc/DESIGN.ja.md.pre-draft-20260429` | 旧 DESIGN.ja.md（Phase 0 以前の重い設計、11 entity / 案 A/B/C 比較議論含む）。BAK に退避済 |
...
| `doc/TODO.md` | Phase 1 spike 計画（spike 05-13）。DESIGN.ja.md の確定内容に合わせた再構成が必要 |
...
3. **EXTERNAL_DESIGN.ja.md は不変**。要件は**すべて維持**。軽量化されるのは **graph schema（4 entity / 6 relation）と実装手段のみ**（CONCEPT entity を廃止し ANCHOR に統合、2026-04-29）
...
1. **DESIGN.ja.md は置換済（2026-04-29）**。GPT + Claude の複数ラウンドレビューを経て draft → 現行版に昇格
   - CONCEPT entity 廃止 → 4 entity / 6 relation
   - Hierarchical Cluster sidecar、Core Concept index（§3.7）、ConceptApprovalRequiredResult、spike fallback ladder（§4.9）を追加
   - 冒頭の「draft」「書き直し draft」表記は残っている → 正式版として冒頭を書き換えるかはユーザー判断
2. **TODO.md / SUMMARY.md の波及修正**（DESIGN.ja.md 確定内容に合わせて）
...
DESIGN.ja.md（当時は draft）で「軽量 MVP では対象外」「Phase 3 以降で再評価」「MVP では区別なしで運用も可」と書いて、EXTERNAL_DESIGN.ja.md にない概念（Custom schema / kind / Project Custom Schema）を持ち込みつつ先送りした。ユーザーから「MVP という言葉で逃げてる？」と指摘。一掃。
