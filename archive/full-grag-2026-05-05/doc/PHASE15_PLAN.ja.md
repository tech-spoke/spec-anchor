...
- [x] BM25 broad candidate 問題を追加監査する
  ...
  - 残り: q1〜q3 は `227`〜`281` / `407` documents までしか縮んでいないため、field weighting / required term gate / query type 別 fallback を継続検討する
...
- [ ] embedding throughput を監査する
  - 対象: full build で `SECTION` / `ANCHOR` / raw chunks を約 2,000 件逐次 embedding していること、Ollama `bge-m3` の single-text HTTP call、batch / parallel / graph entity embedding 対象削減の余地
  - 受け入れ条件: raw chunk only hybrid retrieval、graph entity embedding、Concept embedding の件数と時間を分離し、短期改善策を後続実装タスクへ落とす
- [ ] staging `copytree` の最適化要否を判断する
  - 対象: diagnostics は追加済みだが、artifact size 増加時に full directory copy が支配的になるかは未判定
  - 受け入れ条件: production artifact size と staging prepare timing から、現状維持 / incremental copy / hardlink / rsync 相当のいずれかを判断する
...
- P15-D 追補: staging prepare stage に active / staging directory の file count、dir count、bytes を記録する diagnostics を追加。最適化要否の判断は残件。
- P15-D 残件追補: BM25 は q4 / q5 では大きく縮んだが、q1〜q3 はまだ broad。full build embedding は約 2,000 件の逐次 Ollama call が支配しているため、embedding throughput 監査を追加。
